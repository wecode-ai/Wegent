...
class TestGetContainerPortsHostMode:
    """Test cases for get_container_ports with host network mode support"""

    @patch("subprocess.run")
    def test_get_container_ports_bridge_mode(self, mock_run):
        """Test getting ports for bridge network mode (with port mappings)"""
        # Mock check_container_ownership and get ports
        mock_run.side_effect = [
            MagicMock(stdout="test-container\n", returncode=0),  # ownership check
            MagicMock(stdout="0.0.0.0:8080->8080/tcp\n", returncode=0),  # port mappings
        ]
    ...
    @patch("subprocess.run")
    def test_get_container_ports_host_mode_from_env(self, mock_run):
        """Test getting ports for host network mode (from environment variables)"""
        # Mock check_container_ownership, no port mappings, then environment variables
        mock_run.side_effect = [
            MagicMock(stdout="test-container\n", returncode=0),  # ownership check
            MagicMock(stdout="\n", returncode=0),  # no port mappings (host mode)
            MagicMock(
                stdout="PATH=/usr/bin\nPORT=8080\nHOME=/root\n", returncode=0
            ),  # env vars
        ]
    ...
    @patch("subprocess.run")
    def test_get_container_ports_host_mode_no_port_env(self, mock_run):
        """Test getting ports for host mode when PORT env var is missing"""
        # Mock check_container_ownership, no port mappings, no PORT env var
        mock_run.side_effect = [
            MagicMock(stdout="test-container\n", returncode=0),  # ownership check
            MagicMock(stdout="\n", returncode=0),  # no port mappings
            MagicMock(
                stdout="PATH=/usr/bin\nHOME=/root\n", returncode=0
            ),  # no PORT env var
        ]
        ...
        assert result["status"] == "success"
        assert len(result["ports"]) == 0  # No port information available

    @patch("subprocess.run")
    def test_get_container_ports_not_owned(self, mock_run):
        """Test getting ports for container not owned by executor_manager"""
        ...
        assert result["status"] == "failed"
        assert "not owned" in result["error_msg"]
        assert len(result["ports"]) == 0

    @patch("subprocess.run")
    def test_get_container_ports_multiple_ports(self, mock_run):
        """Test getting multiple port mappings"""
        # Mock check_container_ownership and multiple port mappings
        mock_run.side_effect = [
            MagicMock(stdout="test-container\n", returncode=0),  # ownership check
            MagicMock(
                stdout="0.0.0.0:8080->8080/tcp, 0.0.0.0:9090->9090/tcp\n",
                returncode=0,
            ),
        ]
        ...
        assert result["ports"][0]["host_port"] == 8080
        assert result["ports"][1]["host_port"] == 9090
