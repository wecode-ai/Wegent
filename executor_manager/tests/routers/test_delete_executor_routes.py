...
async def test_delete_executor_by_task_id_success(mocker):
    ...
    result = await routers.delete_executor_by_task_id(
        request=routers.DeleteExecutorByTaskIdRequest(task_id=1234),
        http_request=http_request,
    )
...
async def test_delete_executor_by_task_id_not_found(mocker):
    ...
    result = await routers.delete_executor_by_task_id(
        request=routers.DeleteExecutorByTaskIdRequest(task_id=9999),
        http_request=http_request,
    )
...
async def test_delete_executor_by_task_id_unsupported_executor(mocker):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await routers.delete_executor_by_task_id(
            request=routers.DeleteExecutorByTaskIdRequest(task_id=1234),
            http_request=http_request,
        )
