...
class MockExecutor(Executor):
    ...
    def get_container_address(self, executor_name, executor_namespace=None):
...
class TestExecutorDispatcher:
    ...
    def test_load_custom_executor_from_config(self):
        ...
        config = '{"custom": "executor_manager.tests.wecode.executors.test_dispatcher.MockExecutor"}'
        # Patch config before importing dispatcher
    ...
    def test_load_multiple_executors(self):
        ...
        config = json.dumps(
            {
                "docker": "executor_manager.tests.wecode.executors.test_dispatcher.MockExecutor",
                "custom": "executor_manager.tests.wecode.executors.test_dispatcher.MockExecutor",
            }
        )
        with patch("executor_manager.config.config.EXECUTOR_CONFIG", config):
            from executor_manager.executors.dispatcher import ExecutorDispatcher
...
def test_mock_executor_implements_required_interface():
    executor = MockExecutor()

    assert executor.submit_executor({}) == {"status": "success"}
    assert executor.get_current_task_ids() == {"task_ids": []}
    assert executor.delete_executor("pod") == {"status": "success"}
    assert executor.get_executor_count() == {"count": 0}
    assert executor.get_container_address("pod") == {
        "status": "success",
        "base_url": "http://localhost:10001",
    }
    assert executor.get_container_status("pod") == {
        "exists": True,
        "status": "running",
        "oom_killed": False,
        "exit_code": 0,
