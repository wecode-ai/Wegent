...
@pytest.mark.integration
def test_initial_k8s_dispatch_times_out_once_against_real_http_server(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    request_count = 0
    ...
    monkeypatch.setenv("EXECUTOR_INITIAL_DISPATCH_MAX_RETRIES", "1")
    monkeypatch.setenv("EXECUTOR_INITIAL_DISPATCH_RETRY_INTERVAL", "0")
    monkeypatch.setenv("EXECUTOR_INITIAL_DISPATCH_TIMEOUT", "0.1")
    executor = object.__new__(K8sExecutor)
    executor.requests = requests

    try:
        started_at = time.monotonic()
        with pytest.raises(RuntimeError, match="Failed to dispatch initial task"):
            executor._dispatch_initial_task_to_instance(
                {"task_id": 123},
                "executor-e2e",
                host,
                port,
