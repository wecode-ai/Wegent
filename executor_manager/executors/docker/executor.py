...
class DockerExecutor(Executor):
    ...
    def _execute_in_persistent_container(
        ...
        if not user_id or not shell_id:
            logger.warning(
                f"Missing user_id ({user_id}) or shell_id ({shell_id}) for persistent container, falling back to ephemeral"
            )
        ...
        if container_result.get("status") == "error":
            raise RuntimeError(
                container_result.get(
                    "error_message", "Failed to get/create persistent container"
                )
            )
        ...
        if container_result.get("reused"):
            logger.info(
                f"Reusing persistent container {container_name} for task {task_info['task_id']}"
            )
        else:
            logger.info(
                f"Created new persistent container {container_name} for task {task_info['task_id']}"
            )
        ...
        if port:
            ...
            raise RuntimeError(
                f"Could not get port for persistent container {container_name}"
            )
    ...
    def _wait_for_container_ready(
        ...
        logger.info(
            f"Waiting for container {container_name} to be ready on port {port}..."
        )
    ...
    def _add_workspace_mount(self, cmd: List[str]) -> None:
        ...
        if executor_workspace:
            ...
            logger.warning(
                "EXECUTOR_WORKSPACE environment variable not set, workspace will not be mounted. "
