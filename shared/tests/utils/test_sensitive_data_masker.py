...
"""
Tests for shared/utils/sensitive_data_masker.py (migrated from test_sensitive_data_masker.py)
"""

import pytest

from shared.utils.sensitive_data_masker import SensitiveDataMasker, mask_sensitive_data, mask_string
...
class TestSensitiveDataMasker:
    ...
    @pytest.fixture
    def masker(self):
        """Create SensitiveDataMasker instance"""
    ...
    def test_mask_list_with_sensitive_data(self, masker):
        """Test masking list with sensitive data"""
        data_list = [
            "github_pat_FAKETOKEN123",
            "normal_value",
            {"key": "sk-FAKEKEY123"}
        ]

        masked = masker.mask_list(data_list)

        assert "****" in masked[0]
        assert masked[1] == "normal_value"
        assert "****" in masked[2]["key"]

    def test_mask_nested_structures(self, masker):
        """Test masking deeply nested data structures"""
        data = {
            "level1": {
                "level2": {
                    "level3": {
                        "token": "github_pat_DEEPNESTED123"
                    }
                }
            }
        }
        ...
        # Should mask even deeply nested values
        assert "****" in masked["level1"]["level2"]["level3"]["token"]
        assert "github_pat_DEEPNESTED123" not in str(masked)
