import type { BrowserAnnotationContextData } from "./browser-annotation";
...
): "browser_annotation" | "code_selection" {
  return context.source === "browser_annotation" ||
    context.filePath.startsWith("browser:")
    ? "browser_annotation"
    : "code_selection";
}
...
function serializedCodeCommentContexts(contexts: CodeCommentContext[]): string {
  const payload = contexts.map((context, index) => {
    ...
    return {
      ...
      adjustments: context.adjustments?.length
        ? context.adjustments
        : undefined,
...
export function appendCodeCommentContexts(
  ...
  return trimmedMessage
    ? [trimmedMessage, "", contextBlock].join("\n")
    : contextBlock;
}
...
function browserTargetFromSelectedText(
  ...
  try {
    ...
    if (
      ...
      return (parsed as { target: BrowserAnnotationContextData["target"] })
        .target;
    }
  } catch {
...
function deserializeCodeCommentContext(
  item: unknown,
  index: number,
): CodeCommentContext | null {
  if (!item || typeof item !== "object") return null;
  const record = item as Record<string, unknown>;
  if (
    typeof record.commentNumber !== "number" ||
    (record.source !== "browser_annotation" &&
      record.source !== "code_selection") ||
  ...
  const selectedText =
    typeof record.selectedText === "string" ? record.selectedText : "";
...
export function parseCodeCommentContexts(
  ...
  const blockMatch =
    /<workspace_comment_context>[\s\S]*?<\/workspace_comment_context>\s*$/.exec(
      content,
    );
