...
export function mergeRuntimeConversationTurn(
  ...
  return {
    ...
    fileChanges:
      ...
      local.fileChanges.workspace_path === snapshot.fileChanges.workspace_path &&
...
export function isTerminalTurnStatus(status: RuntimeConversationTurn['status']): boolean {
  return status === 'done' || status === 'failed' || status === 'cancelled'
}

export function isUnsettledTurnStatus(status: RuntimeConversationTurn['status']): boolean {
...
export function mergeRuntimeConversationItem(
  ...
  const snapshotBlock = preserveRequestUserInputResponse(local.block, snapshot.block)
...
export function isTerminalProcessingBlockStatus(status: ProcessingBlock['status']): boolean {
