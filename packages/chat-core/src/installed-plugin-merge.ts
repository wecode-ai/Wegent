...
import { isWegentCloudMarketplaceId as isWegentCloudMarketplace } from './plugin-marketplace-identity'
...
export function installedPluginSourceLabel(
  ...
  if (item.spec.origin === 'created' || item.spec.source.type === 'local') {
    ...
    if (status === 'approved') {
      return translate('workbench.plugins_created_source_approved')
    }
    if (status === 'rejected') {
      return translate('workbench.plugins_created_source_rejected')
    }
    if (item.spec.sourcePayload?.submissionId || status === 'pending') {
      return translate('workbench.plugins_created_source_pending')
    }
    return translate('workbench.plugins_created_by_me')
  }

  const marketplace = marketplaceKey(item)
  if (isPersonalMarketplaceId(marketplace)) {
    return translate('workbench.plugins_source_personal_share')
  }
  if (isOpenAiOfficialMarketplaceId(marketplace)) {
    return translate('workbench.plugins_source_openai_official')
  }
  if (isInternalDeviceMarketplaceId(marketplace)) {
    if (item.spec.sourceProvider === 'codex') {
      return translate('workbench.plugins_source_codex_mirror')
    }
    if (item.spec.sourceProvider === 'user') {
      return translate('workbench.plugins_source_community')
    }
    return translate('workbench.plugins_source_wegent_official')
  }
