...
export function getRuntimeMessageActiveThinking(
  message: WorkbenchMessage,
): string {
  if (message.role !== "assistant" || message.status !== "streaming") return "";

  return (
    message.streamingThinkingContent?.trim() ||
    getLatestActiveThinkingBlock(message.blocks)
  );
}

export function getLatestRuntimeLiveActivity(
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    return populatedRuntimeLiveActivityOrEmpty({
      active: true,
      thinking: compactLiveActivityText(
        getRuntimeMessageActiveThinking(message),
      ),
      processText: compactLiveActivityText(
        getLatestProcessTextBlock(message.blocks),
      ),
...
export function getLatestRuntimeLiveActivityFromTurns(
  ...
  for (let index = turns.length - 1; index >= 0; index -= 1) {
    ...
    if (!turn || (turn.status !== "pending" && turn.status !== "streaming"))
      continue;

    let lastUserIndex = turn.items.length - 1;
    while (
      lastUserIndex >= 0 &&
      turn.items[lastUserIndex]?.type !== "user_message"
    ) {
      lastUserIndex -= 1;
    }
    return populatedRuntimeLiveActivityOrEmpty(
      runtimeLiveActivityFromTurn(turn, lastUserIndex + 1),
    );
  }
  return EMPTY_RUNTIME_LIVE_ACTIVITY;
}
...
function populatedRuntimeLiveActivityOrEmpty(
...
function getLatestProcessTextBlock(
...
function getLatestToolActivities(
  blocks: ProcessingBlock[] | undefined,
): RuntimeLiveToolActivity[] {
  if (!blocks?.length) return [];
  const tools: RuntimeLiveToolActivity[] = [];
  for (let index = blocks.length - 1; index >= 0; index -= 1) {
    const block = blocks[index];
...
function getLatestActiveThinkingBlock(
  blocks: ProcessingBlock[] | undefined,
): string {
  if (!blocks?.length) return "";
...
function compactLiveActivityToolInput(
  ...
  for (const [key, value] of Object.entries(input)) {
    ...
    if (
      ...
      const nested = compactLiveActivityToolInput(
