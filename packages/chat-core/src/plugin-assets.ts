...
export function createPluginAssetResolver(localFileUrl: (path: string) => string) {
  ...
  function isNeutralPluginIconUrl(value?: string | null): boolean {
  ...
  function isRelativePluginAssetPath(value?: string | null): boolean {
  ...
  function resolvePluginAssetUrl(value?: string | null): string {
    const source = value?.trim()
    if (!source) return ''
    if (!isLocalAssetPath(source)) return source

    return localFileUrl(localPathFromFileUrl(source))
  }
  ...
  function currentPluginLogoAppearanceMode(): ResolvedAppearanceMode {
    if (typeof document === 'undefined') return 'light'
    return document.documentElement.dataset.theme === 'dark' ? 'dark' : 'light'
  }

  function resolvePluginLogoUrl(options: {
  ...
  function installedPluginHasRelativeLogo(plugin: InstalledPlugin): boolean {
  ...
  function resolveInstalledPluginLogoUrl(
   ...
   */
  function resolvePluginLogo(options: {
    pluginKey?: string | null
    logo?: string | null
    logoDark?: string | null
    composerIcon?: string | null
   ...
   */
  function resolvePreferredPluginLogo(options: {
    pluginKey?: string | null
    appearanceMode?: ResolvedAppearanceMode
