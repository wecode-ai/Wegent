  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (character !== "\u001b") {
      if (character >= " ") {
        ...
        writeText(
          value.slice(index, Math.min(end, index + MAX_TERMINAL_COLUMNS)),
        );
        index = end - 1;
      }
      continue;
    ...
    if (command === "A") moveToRow(row - amount, true);
      ...
      column = Math.min(column + amount, MAX_TERMINAL_COLUMNS - 1);
