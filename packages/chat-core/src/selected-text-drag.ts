  ...
  window.dispatchEvent(
    new CustomEvent<SelectedTextChangedDetail>(SELECTED_TEXT_CHANGED_EVENT, {
      detail: {
        source,
        text: normalizedText,
        rect: normalizedText ? rect : null,
      },
    }),
  );
}

export function writeSelectedTextDragData(
...
function selectionBelongsToDragSource(
  selection: Selection,
  ...
  const composedContains = (ancestor: Node, descendant: Node) => {
    ...
    while (current) {
      ...
      current =
        current.parentNode ??
        (root instanceof ShadowRoot && root !== current ? root.host : null);
    }
    return false;
  };
