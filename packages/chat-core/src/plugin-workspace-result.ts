import type { WorkbenchMessage } from "./runtime-conversation";
...
export function parsePluginWorkspaceResults(
  content: string,
): PluginWorkspaceResult[] {
  const results: PluginWorkspaceResult[] = [];
  for (const line of content.split(/\r?\n/)) {
    const markerIndex = line.indexOf(PLUGIN_WORKSPACE_RESULT_MARKER);
    if (markerIndex < 0) continue;
    const payload = line
      .slice(markerIndex + PLUGIN_WORKSPACE_RESULT_MARKER.length)
      .trim();
    try {
...
export function latestPluginWorkspaceResult(
  ...
  for (
    ...
    for (
      ...
      if (!normalizedTaskId || result.taskId === normalizedTaskId)
        return result;
    }
  }
  return null;
...
export function pluginWorkspaceManifestPath(
  ...
  return normalizedWorkspace
    ? `${normalizedWorkspace}/${relativeManifest}`
    : relativeManifest;
}
