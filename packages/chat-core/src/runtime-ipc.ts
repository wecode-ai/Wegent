...
export interface CloudRuntimeIpcClient {
  ...
  subscribe: (handler: (event: RuntimeEvent) => void) => Promise<() => void>;
...
export function createCloudRuntimeIpcClient(
  ...
  return {
    ...
    async subscribe(
      ...
      const runtimeHandler = (event: RuntimeEvent) => {
        handler(event);
      };
...
async function emitRuntimeRequest<T>(
  ...
  const requestId = `cloud-runtime-${globalThis.crypto.randomUUID()}`;
  ...
  const relayTimeoutSeconds = resolveRelayTimeoutSeconds(
    method,
    params,
    timeoutMs,
  );
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    client.socket.emit(
      ...
      (ack: RuntimeIpcAck<T> | undefined) => {
        ...
        if (!ack) {
          console.warn(
            "[Wework] Cloud runtime RPC returned an empty acknowledgement",
            {
              request_id: requestId,
              method,
              device_id: targetDeviceId,
        ...
        void decodeRuntimeIpcResult<T>(ack.result ?? null).then(
          ...
          (error) => {
            console.warn(
              "[Wework] Cloud runtime RPC response decoding failed",
              {
                request_id: requestId,
                method,
                device_id: targetDeviceId,
...
function deviceIdFromParams(
  params: Record<string, unknown>,
): string | undefined {
  const direct =
    stringField(params, "deviceId") ?? stringField(params, "device_id");
