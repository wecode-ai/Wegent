...
export function workspaceProjectMatchesQuery(
  ...
  return (
    !query || project.name.toLowerCase().includes(query.trim().toLowerCase())
  );
}
...
>({
  ...
  const allItemEntries = Object.entries(projectItems).flatMap(
    ([spaceKey, items]) => items.map((item) => ({ item, spaceKey })),
  );
  ...
  return {
    stats: {
      ...
      completedCount: allItems.filter((item) => item.status === "completed")
        .length,
      ...
      inProgressCount: allItems.filter((item) => item.status === "in_progress")
        .length,
    },
    ...
    myTodos: myWork
      ...
      .slice(0, MAX_MY_TODOS),
    sortedProjects: [...visibleProjects].sort((left, right) =>
      right.updated_at.localeCompare(left.updated_at),
    ),
    projectByKey: new Map(
      projects.map((project) => [workspaceProjectKey(project), project]),
    ),
  };
}
