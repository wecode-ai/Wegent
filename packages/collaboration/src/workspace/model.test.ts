...
function item(id: string, overrides: Partial<WorkspaceHomeItem> = {}): WorkspaceHomeItem {
  return {
    id,
    title: id,
    status: 'pending',
...
describe('workspace home model', () => {
  it('computes cross-project statistics, activity, todos, and project ordering', () => {
    ...
    expect(snapshot.myTodos.map(workItem => workItem.id)).toEqual(['in-progress'])
    expect(snapshot.sortedProjects.map(project => project.id)).toEqual(['newer', 'older'])
  })
