...
function memberNameById(members: WorkspaceHomeMember[], userId: number | null): string | null {
...
function ProjectSpaceRow<Project extends WorkspaceHomeProject, Member extends WorkspaceHomeMember>({
  project,
  members,
  host,
  openLabel,
  ...
  const LocationIcon = project.location === 'local' ? host.icons.HardDrive : host.icons.Cloud
  ...
  return (
    ...
    >
      ...
      <span className="flex items-center">
        ...
        <span className={classNames('flex items-center', onManage && 'group-hover:hidden')}>
              ...
              {members.slice(0, 2).map((member, memberIndex) => (
                <span
                  ...
                  className={classNames(
                    ...
                    MEMBER_AVATAR_CLASSES[memberIndex % MEMBER_AVATAR_CLASSES.length],
                    memberIndex > 0 && '-ml-1'
                  )}
                >
...
>({
  ...
  const manageProjects = useMemo(
    () =>
      snapshot.sortedProjects.filter(project => workspaceProjectMatchesQuery(project, manageQuery)),
  ...
  const renderProjectRow = (project: Project, options?: { onOpen(): void; onManage(): void }) => {
    ...
    return (
      <ProjectSpaceRow
        ...
        taskCountLabel={host.translate('todo.home_task_count', '{{count}} 任务', {
          count: projectCounts[key] ?? 0,
        })}
        memberCountLabel={host.translate('todo.home_member_count', '{{count}} 人', {
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-8 py-7">
      <div className="mx-auto w-full max-w-[880px]">
        <div className="flex items-start">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">
              {host.translate('todo.projects_home_subtitle', '跨项目的个人工作台与项目空间概览')}
            </p>
          </div>
              ...
              snapshot.recentActivity.map(({ item, spaceKey }, itemIndex) => {
                ...
                const actionLabel =
                  item.status === 'completed'
                    ? host.translate('todo.home_activity_completed', '完成了任务')
                    : host.translate('todo.home_activity_updated', '更新了任务')
                return (
                  <button
                  ...
                  >
                    <span
                      className={classNames(
                        ...
                        MEMBER_AVATAR_CLASSES[itemIndex % MEMBER_AVATAR_CLASSES.length]
                      )}
                    >
        ...
        <section className="mt-8">
          ...
          >
            {snapshot.sortedProjects.map(project => renderProjectRow(project))}
          </div>
        </section>
              ...
              <div className="flex min-h-0 flex-1 flex-col px-5 pb-5 pt-4">
                <div className="relative shrink-0">
                  ...
                  <input
                    ...
                    placeholder={host.translate('todo.home_manage_search', '搜索项目空间')}
                ...
                <div className="mt-4 min-h-0 flex-1 overflow-y-auto border-t border-border">
                  {manageProjects.length === 0 ? (
                    <p className="px-2 py-6 text-center text-sm text-text-muted">
                      {host.translate('todo.home_manage_empty', '没有匹配的项目空间')}
                    </p>
                  ) : (
