...
let highlightCodeModulePromise: Promise<
  typeof import("./highlightCode")
> | null = null;
...
export function MarkdownCodeBlock({
  ...
  const { onCopy, translate: t } = useMarkdownServices();
  const [copied, setCopied] = useState(false);
  const text = String(children).replace(/\n$/, "");
  const language = normalizeLanguage(lang);
  const canToggleWrap = language === "markdown";
  const wrapStateKey = canToggleWrap
    ? getMarkdownWrapStateKey(text, compact)
    : "";
  const [wrapState, setWrapState] = useState(() => ({
    key: wrapStateKey,
    value: wrapStateKey
      ? (markdownWrapStateByKey.get(wrapStateKey) ?? false)
      : false,
  }));
  const storedWrapLines = wrapStateKey
    ? (markdownWrapStateByKey.get(wrapStateKey) ?? false)
    : false;
  const effectiveWrapLines =
    canToggleWrap &&
    (wrapState.key === wrapStateKey ? wrapState.value : storedWrapLines);
  const displayLanguage = formatDisplayLanguage(language);
  const wrapButtonLabel = effectiveWrapLines
    ? t("code.wrap_off")
    : t("code.wrap_on");
  ...
  const highlightedPrefix =
    highlightedCode && text.startsWith(highlightedCode.code)
      ? highlightedCode
      : null;
        ...
        <div className="flex items-center gap-1">
            ...
            >
              {effectiveWrapLines ? (
                <TextWrap
                  ...
                  data-testid="markdown-code-wrap-enabled-icon"
                />
              ) : (
                <ArrowRightToLine
                  className="h-3.5 w-3.5"
          ...
          >
            {copied ? (
              <CopyCheck
                className="h-3.5 w-3.5"
                data-testid="markdown-code-copy-success-icon"
              />
            ) : (
              <Copy
                className="h-3.5 w-3.5"
                data-testid="markdown-code-copy-icon"
              />
            )}
          </button>
        </div>
        ...
        <pre style={codeStyle}>
          <code {...codeProps}>
            <HighlightedCodeContent
              text={text}
              highlightedCode={highlightedPrefix}
            />
          </code>
        </pre>
      </MarkdownCodeScrollArea>
...
function useThrottledHighlightedCode(
  text: string,
  language: string,
  ...
  useEffect(() => {
    ...
    const startHighlight = () => {
      ...
      const modulePromise = (highlightCodeModulePromise ??=
    ...
    <>
      {lines.map((html, index) => (
        <HighlightedCodeLine
          key={index}
          html={html}
          appendLineBreak={index < lines.length - 1}
        />
      ))}
      {pendingTail}
    </>
