...
describe('KanbanColumnDropzone', () => {
  it('preserves the Wework dropzone classes, test id and active drag hint', () => {
    ...
    expect(element.props['data-testid']).toBe('cloud-todo-column-dropzone-pending')
    ...
    expect(element.props.className).toContain('rounded-xl bg-muted ring-1 ring-inset ring-focus/50')
    expect(children[0].props['data-testid']).toBe('cloud-todo-column-drag-hint-pending')
...
describe('KanbanBoard', () => {
  it('owns the DndContext, column container and DragOverlay structure', () => {
    ...
    expect(columns.props.className).toBe('flex h-full min-h-0 items-start gap-3.5 px-6')
