...
export function isVisibleModelSelectorControl(
...
export function isCodexDefaultModel(model: UnifiedModel | null): boolean {
  return Boolean(
    model &&
    isCodexOfficialModel(model) &&
    normalizedModelId(model) === CODEX_DEFAULT_MODEL_ID,
  );
}

export function findCodexDefaultModel(
  models: UnifiedModel[],
): UnifiedModel | null {
  return (
    models.find(
      (model) => !model.compatibilityDisabled && isCodexDefaultModel(model),
    ) ?? null
  );
}
...
function resolvePowerSettings(
  ...
  return definitions.flatMap((definition) => {
    ...
    const reasoningControl = getControlsForModel(model).find(
      (control) => control.id === "reasoning",
    );
    if (
      !reasoningControl?.options.some(
        (option) => option.value === definition.reasoningEffort,
      )
    ) {
      return [];
    }
...
export function getCodexModelPowerSettings(
  models: UnifiedModel[],
): ModelPowerSetting[] {
  const primarySettings = resolvePowerSettings(
    CODEX_PRIMARY_POWER_SETTINGS,
    models,
  );
  if (primarySettings.length >= 4) return primarySettings;

  const terraSettings = resolvePowerSettings(
    CODEX_TERRA_POWER_SETTINGS,
    models,
  );
...
export function isSelectedPowerSetting(
  ...
  return Boolean(
    ...
    normalizeModelOptionValue("reasoning", reasoningEffort) ===
...
export function selectedControlOption(
  ...
  const requestedValue = normalizeModelOptionValue(
    control.id,
    options[control.id],
  );
  return (
...
export function desktopFastModeState(
  ...
  const selectedValue = control
    ? selectedControlOption(control, options)?.value
    : undefined;
...
export function modelCompatibilityDisabledMessage(
  ...
  if (reason === "unavailable") {
    return resolveLabel(
      "workbench.model_disabled_unavailable",
...
export function isCloudModel(model: UnifiedModel): boolean {
  return model.provider !== "local";
}

export function codexProviderId(
  model: UnifiedModel | null,
): string | undefined {
