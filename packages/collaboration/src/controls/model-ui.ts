...
export function isSameModelSelection(
  ...
  return (
    modelConfigString(left, "codexProviderId") ===
      modelConfigString(right, "codexProviderId") &&
...
function getCompatibilityRuntimeGroup(
  ...
  if (family === "claude" || provider === "claude" || protocol === "claude")
    return "claude";
  return family;
}
...
export function areModelCompatibilityFamiliesCompatible(
  ...
  return (
    getCompatibilityRuntimeGroup(currentFamily) ===
    getCompatibilityRuntimeGroup(nextFamily)
  );
}
...
function getExplicitSupportedControls(
...
function modelInterfaceReasoningOptions(
  values: string[],
): ModelControlOption[] {
...
export function getModelUiMetadata(model: UnifiedModel): ModelUiMetadata {
  ...
  const familyLabel =
    typeof ui.familyLabel === "string" && ui.familyLabel.trim()
      ? ui.familyLabel.trim()
      : undefined;
...
export function getModelDisplayLabel(
  ...
  const regionLabel = metadata.region
    ? (REGION_LABELS[metadata.region] ?? metadata.region)
    : "";
...
export function normalizeModelOptionAliases(
  options: ModelOptions = {},
): ModelOptions {
...
export function normalizeModelOptions(
  ...
  const normalized = Object.fromEntries(
    getControlsForModel(model)
      .filter(
        (control) =>
          control.persistDefault !== false || options[control.id] !== undefined,
      )
...
function extractVersionParts(label: string): number[] {
  return [...label.matchAll(/\d+/g)].map((match) =>
...
export function groupModelsByFamily(models: UnifiedModel[]) {
  const groups = new Map<
    string,
    { models: UnifiedModel[]; familyLabel?: string }
  >();
