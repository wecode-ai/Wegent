...
import { type ModelControlConfig, normalizeModelOptionValue } from "./model-ui";
import type { ModelOptions } from "@wegent/chat-core/models";
...
export function ReasoningSlider({
  ...
  const requestedValue = normalizeModelOptionValue(
    control.id,
    selectedModelOptions[control.id],
  );
  ...
  const selectFromClientX = (track: HTMLElement, clientX: number) => {
    ...
    const optionIndex =
      steps.length === 1 ? 0 : Math.round(ratio * (steps.length - 1));
  ...
  const handlePointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (
      Math.abs(event.clientX - dragStartXRef.current) >= POINTER_DRAG_THRESHOLD
    ) {
    ...
    >
      ...
      >
        ...
        >
          {ultraSelected ? (
            <span
              data-testid="reasoning-ultra-burst"
              className={styles.ultraBurst}
            />
          ) : null}
          {ultraSelected
