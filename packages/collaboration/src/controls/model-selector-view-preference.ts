export const MODEL_SELECTOR_VIEW_CHANGED_EVENT =
  "wework:model-selector-view-changed";
...
export function readModelSelectorPowerViewPreference(): boolean {
  ...
  try {
    return (
      window.localStorage.getItem(MODEL_SELECTOR_VIEW_STORAGE_KEY) === "power"
    );
  } catch {
    return false;
  }
...
export function writeModelSelectorPowerViewPreference(
  ...
  window.dispatchEvent(
    new CustomEvent<boolean>(MODEL_SELECTOR_VIEW_CHANGED_EVENT, {
      detail: powerViewOpen,
    }),
  );
}
