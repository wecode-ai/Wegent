  ...
  return recordArray(value).map((item, index) => ({
    ...
    fileConstraints: isRecord(item.fileConstraints)
      ? {
          ...
          min_files:
            typeof item.fileConstraints.min_files === "number"
              ? item.fileConstraints.min_files
              : 1,
          max_files:
            typeof item.fileConstraints.max_files === "number"
              ? item.fileConstraints.max_files
              : 1,
        }
      : null,
  }));
...
function normalizeStoredStep(
  ...
  const kind =
    item.kind === "dynamic" || item.kind === "loop" || item.kind === "branch"
      ? item.kind
      : "task";
  const nodeType =
    item.nodeType === "event" ||
    ...
    item.nodeType === "loopEnd"
      ...
      : kind === "loop"
        ...
        : kind === "branch"
          ...
          : "task";
  const executionMode =
    item.executionMode === "manual" ? "manual" : "automatic";
  const executionEnvironment =
    item.executionEnvironment === "cloud" ? "cloud" : "local";
  ...
  const step: AutomationUiStep = {
    ...
    name:
      typeof item.name === "string"
        ? item.name
        : inherited?.name || "未命名执行节点",
    prompt:
      typeof item.prompt === "string" ? item.prompt : inherited?.prompt || "",
    kind,
    dependencies: stringArray(item.dependencies),
    dependencyContext: isRecord(item.dependencyContext)
      ? Object.fromEntries(
          Object.entries(item.dependencyContext).map(
            ([dependencyId, sources]) => [
              dependencyId,
    ...
    executionEnvironment,
    executionDeviceId:
      typeof item.executionDeviceId === "string"
        ? item.executionDeviceId
        : null,
    runtimeProfileId:
      typeof item.runtimeProfileId === "string" ? item.runtimeProfileId : null,
    ...
    approvalPolicy:
      item.approvalPolicy === "automatic" ? "automatic" : "required",
    nodeType,
    ...
    branchConditions: recordArray(item.branchConditions).map((condition) => ({
      sourceType:
        normalizeBranchSourceType(
          condition.sourceType ?? condition.source_type,
        ) || "github",
      eventType:
        typeof condition.eventType === "string" ? condition.eventType : "",
    ...
    eventWait: (() => {
      ...
      const collectionMode = normalizeBranchCollectionMode(
        stored?.collectionMode ??
          stored?.collection_mode ??
          legacyCondition?.collectionMode,
      );
  ...
  if (kind === "dynamic") {
    ...
    step.subgraph = {
      nodes: subgraphNodes.map((node, childIndex) =>
        normalizeStoredStep(node, childIndex),
      ),
    };
  }
  return step;
...
function storedFlow(
...
): NormalizedAutomationFlowV2 | null {
    ...
    return {
      ...
      description:
        typeof candidate.description === "string"
          ...
          : rule.prompt,
      graph: {
        nodes: candidate.graph.nodes.map((step, index) =>
          normalizeStoredStep(step, index),
        ),
      },
    };
  }
  ...
  return {
    version: 2,
    description:
      typeof legacy.description === "string" ? legacy.description : rule.prompt,
    graph: {
...
function parseCron(expression: string | null) {
  ...
  if (/^[0-6]$/.test(dayOfWeek)) {
    ...
    return {
      ...
      weekday: weekdays[Number(dayOfWeek)],
      time,
    };
  }
    ...
    environment:
      rule.executionEnvironment === "cloud"
        ...
        : rule.executionEnvironment === "managed"
          ...
          : "本机",
    executionEnvironment:
      rule.executionEnvironment === "cloud" ? "cloud" : "local",
...
function normalizeBranchSourceType(value: unknown): BranchSourceType | "" {
  return typeof value === "string" &&
    BRANCH_SOURCE_TYPES.has(value as BranchSourceType)
    ? (value as BranchSourceType)
    : "";
}

function normalizeBranchCollectionMode(
  value: unknown,
): BranchCollectionMode | "" {
  return typeof value === "string" &&
    BRANCH_COLLECTION_MODES.has(value as BranchCollectionMode)
...
function normalizeTriggerSource(
  ...
  if (
    ...
    TRIGGER_SOURCE_TYPES.has(stored as ProjectEventSourceType)
  ) {
...
function normalizeCollectionMode(
  ...
  const stored =
    rule.eventConfig.collection_mode ?? rule.eventConfig.collectionMode;
  if (stored === "webhook" || stored === "poll" || stored === "hybrid")
    return stored;
  return "webhook";
}

export function automationRuleFromBackend(
  rule: ProjectAutomationRule,
): AutomationUiRule {
  const flow = storedFlow(rule);
  const schedule = parseCron(rule.cronExpression);
  const startMode =
    rule.eventType === "task.status_changed" ? "status" : "immediate";
  const subscriptionId =
    typeof rule.eventConfig.subscription_id === "string"
      ? rule.eventConfig.subscription_id
      : typeof rule.eventConfig.subscriptionId === "string"
  ...
  return {
    ...
    trigger: {
      ...
      event:
        externalEvent ??
        (startMode === "status" ? "status_changed" : "created"),
      tags: Array.isArray(rule.eventConfig.tags)
        ? rule.eventConfig.tags.filter(
  ...
  const depth = node.depends_on.length
    ? Math.max(
        ...node.depends_on.map((dependencyId) => {
          ...
          return dependency
            ? workflowNodeDepth(dependency, nodes, memo) + 1
            : 0;
        }),
      )
    : 0;
  ...
  const toStep = (
    ...
    return {
      ...
      branchConditions: (node.branch_conditions ?? []).map((condition) => ({
        sourceType:
          normalizeBranchSourceType(condition.source_type) || "github",
      ...
      eventWait:
        node.node_type === "branch"
          ...
          : null,
      dependencies: [...node.depends_on].filter(
        (dependencyId) => dependencyId !== "start",
      ),
      ...
      executionEnvironment: "local",
      executionDeviceId:
        config?.execution_device_id ??
        referencedRule?.executionDeviceId ??
        null,
      runtimeProfileId:
        config?.runtime_profile_id ?? referencedRule?.runtimeProfileId ?? null,
  ...
  for (const node of definition.nodes) {
    ...
    if (node.node_type === "loop") {
      ...
      steps.push({
        ...toStep(node),
        subgraph: {
          nodes: bodyNodes.map((bodyNode, index) => toStep(bodyNode, index)),
        },
      });
      continue;
    }
...
export function automationRuleFromLegacyWorkflow(
  ...
  const stageMode =
    definition?.stage_mode ?? (definition?.nodes.length ? "dag" : "none");
      ...
      ? [
          {
            ...
            name: "AI 动态分配",
            prompt:
              definition.coordinator_prompt || "根据 Issue 动态拆解并分配任务",
            x: 440,
            y: 226,
...
function executionConfigFromUiNode(
  ...
  return {
    ...
    project_plugins: node.projectPlugins.flatMap((plugin) => {
      ...
      const pluginName =
        typeof plugin.pluginName === "string" ? plugin.pluginName : "";
      const marketplaceId =
        typeof plugin.marketplaceId === "string" ? plugin.marketplaceId : "";
      const displayName =
...
function workflowNodeFromUi(
  ...
  return {
    ...
    body_node_ids:
      node.nodeType === "loop" ? [...(node.bodyNodeIds ?? [])] : undefined,
    loop_config:
    ...
    execution_config: includeExecutionConfig
      ...
      : null,
    execution_config_override:
      includeExecutionConfig && node.executionConfigOverride,
  };
}
...
export function legacyWorkflowFromAutomationRule(
  ...
  const dynamicNode =
    rule.steps.length === 1 && rule.steps[0]?.kind === "dynamic"
      ? rule.steps[0]
      : null;
  ...
  if (dynamicNode) {
    return {
      ...
      approval_policy:
        dynamicNode.approvalPolicy ?? previous?.approval_policy ?? "required",
      ai_automation_rule_id: dynamicNode.automationRuleId,
      execution_config: executionConfigFromUiNode(dynamicNode),
      nodes: (dynamicNode.subgraph?.nodes ?? []).map((node) =>
        workflowNodeFromUi(node, false),
      ),
    };
  }
    ...
    return [
      {
        ...
        depends_on:
          step.dependencies.length > 0 ? [...step.dependencies] : ["start"],
      },
    ];
  });
  ...
  const rewired = topLevelNodes.map((node) => ({
    ...node,
    depends_on:
      node.node_type !== "loop" && node.depends_on.length === 0
        ? ["start"]
        : node.depends_on,
  }));
  return {
...
function storedStageConstraint(
  ...
  return {
    ...
    dependencyContext: Object.fromEntries(
      Object.entries(node.dependencyContext ?? {}).map(
...
function storedStepFromUi(node: AutomationUiStep): Record<string, unknown> {
  return {
    ...
    projectPlugins: (node.projectPlugins ?? []).map((plugin) => ({
      ...plugin,
    })),
  ...
  const describeNodes = (nodes: AutomationUiStep[], depth = 0): string[] =>
    nodes.map((step, index) => {
      ...
      const dependencies = (step.dependencies ?? []).length
        ...
        : "";
      const subgraph =
        (step.kind === "dynamic" || step.kind === "loop") &&
        step.subgraph?.nodes.length
...
export function automationInputFromUi(
  ...
  const isAiDynamicWorkflow =
    rule.steps.length === 1 && rule.steps[0]?.kind === "dynamic";
  const directStep =
    !isAiDynamicWorkflow &&
    rule.steps.length === 1 &&
    rule.steps[0]?.executionMode === "automatic"
      ? rule.steps[0]
      : null;
  const directConfig = directStep
    ? executionConfigFromUiNode(directStep)
    : null;
  ...
  return {
    ...
    executionEnvironment: directAgentId
      ? null
      : (directStep?.executionEnvironment ?? null),
    executionDeviceId: directAgentId
      ? null
      : (directConfig?.execution_device_id ?? null),
    roleSource: directAgentId ? "agent" : "generic",
    runtimeSource: directAgentId
      ? "agent_default"
      : (rule.runtimeSource ??
        (runtimeProfileId !== null ? "fixed_profile" : "runtime_user")),
...
export function automationRunFromBackend(
  ...
  return {
    ...
    issue:
      run.taskTitle ||
      run.taskId ||
      (run.trigger === "scheduled" ? "计划运行" : "手动运行"),
    status: run.status,
