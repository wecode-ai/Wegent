...
export class RuntimeConversationQueue<Lifecycle = unknown> {
  ...
  enqueue(
    message: RuntimePaneQueuedMessage,
    blockedLifecycle?: { value: Lifecycle },
  ) {
  ...
  async pump(port: RuntimeConversationQueuePort<Lifecycle>, busy: boolean) {
    ...
    if (
      this.awaitingNextTurn &&
      port.lifecycleChanged(this.awaitingNextTurn.lifecycle)
    ) {
      this.awaitingNextTurn = null;
    }
    if (busy || this.messages.some((message) => message.status === "sending"))
      return;
  ...
  async send(
    ...
    if (
      !message ||
      this.awaitingNextTurn ||
      this.messages.some((item) => item.status === "sending")
    )
      return false;
    ...
    try {
      ...
      this.rejectSend(
        id,
        result.error || port.sendFailedText,
        port,
        lifecycleBeforeSend,
      );
    } catch (cause) {
  ...
  async guide(
    ...
    try {
      ...
      this.fail(
        id,
        cause instanceof Error ? cause.message : port.guidanceFailedText,
      );
    }
    return false;
  }
  ...
  private patch(id: string, patch: Partial<RuntimePaneQueuedMessage>) {
    if (this.messages.some((item) => item.id === id))
      this.update((messages) =>
        messages.map((item) => (item.id === id ? { ...item, ...patch } : item)),
      );
  }
...
export function runtimeQueuedMessageRequest(
  ...
  return {
    ...
    ...(message.modelId
      ? { modelId: message.modelId, modelType: message.modelType }
      : {}),
...
export function isRuntimeQueueBusyError(
  error: string | null,
