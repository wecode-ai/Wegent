...
const address = { deviceId: 'device-1', taskId: 'original-session', runtime: 'codex' }
const root = {
  messageId: 'root',
  content: 'Work',
  metadata: {},
...
function setup() {
  ...
  const input = {
    ...
    reply: { id: 'queued-id', content: 'Continue', createdAt: '', status: 'queued' as const },
...
describe('shared PC card reply dispatch', () => {
  it('continues the activity-owned session even when no runtime-work snapshot is available', async () => {
    ...
    expect(await dispatchTaskCardReply(input)).toEqual({ ok: true, persisted: true })
  ...
  it('reports whether the reply was saved when the runtime rejects continuation', async () => {
    ...
    runtime.sendRuntimePaneMessage.mockImplementationOnce(async (_request, options) => {
  ...
  it('uses the Wegent continuation operation for a Wegent agent', async () => {
    const { input, client, runtime } = setup()
    expect(
      await dispatchTaskCardReply({ ...input, agent: { ...input.agent, runtime: 'wegent' } })
    ).toEqual({ ok: true, persisted: true })
    expect(client.continueWegentTask).toHaveBeenCalledWith(
      expect.objectContaining({ triggerMessageId: trigger.messageId, agentId: 'agent' })
    )
  ...
  it('closes the custom-manager response when its runtime send throws', async () => {
    const { input, client, runtime } = setup()
    runtime.sendRuntimePaneMessage.mockRejectedValueOnce(new Error('Connection lost'))
    expect(
      await dispatchTaskCardReply({
        ...
        card: {
          root: {
            ...run,
            metadata: { executor_type: 'automation_manager', manager_type: 'custom' },
          },
          replies: [],
        },
      })
    ...
    expect(client.failAgentResponse).toHaveBeenCalledWith(
      expect.objectContaining({ messageId: run.messageId, error: 'Connection lost' })
    )
  })
})
