...
export interface WorkspaceMessageImportInput {
  sourceTaskId: number
  subtaskIds?: number[]
  target: { kind: 'new_issue'; title: string } | { kind: 'existing_issue'; issueId: string }
  note?: string
}
...
export interface SharedWorkspaceAutomationExecutionCatalogApi {
  ...
  loadPlugins(projectId: string, deviceIds: string[]): Promise<WorkspaceAutomationPlugin[]>
}
...
export interface SharedWorkspaceProjectsApi {
  ...
  update(projectId: string, input: WorkspaceProjectUpdateInput): Promise<CollaborationProject>
  archive(projectId: string, version: number): Promise<void>
  listExecutionEnvironments(projectId: string): Promise<CollaborationExecutionEnvironment[]>
  addExecutionEnvironment(
    projectId: string,
    deviceId: number
  ): Promise<CollaborationExecutionEnvironment>
  removeExecutionEnvironment(projectId: string, deviceId: number): Promise<void>
  ...
  listCollaborationGroups?(projectId: string): Promise<import('../types').CollaborationGroup[]>
...
export interface SharedWorkspaceIssuesApi {
  list(projectId: string, filters?: WorkspaceIssueListFilters): Promise<CollaborationIssue[]>
  listPage(
    projectId: string,
    input: WorkspaceIssuePageInput
  ): Promise<WorkspacePage<CollaborationIssue> & { taskBindings: WorkspaceTaskBinding[] }>
  getBoardSnapshot(projectId: string): Promise<WorkspaceBoardSnapshot>
  get(issueId: string): Promise<CollaborationIssue>
  create(projectId: string, input: WorkspaceIssueCreateInput): Promise<CollaborationIssue>
  update(issueId: string, input: WorkspaceIssueUpdateInput): Promise<CollaborationIssue>
  assign(
    projectId: string,
    issueId: string,
    input: WorkspaceIssueAssignmentInput
  ): Promise<CollaborationIssue>
  approveRun(projectId: string, issueId: string, version: number): Promise<CollaborationIssue>
  rejectRun(
    projectId: string,
    issueId: string,
    version: number,
...
export interface SharedCollaborationWorkspacesApi {
  ...
  getNavigationContext?(workspaceId: string): Promise<CollaborationWorkspaceNavigationContext>
  ...
  listCollaborationGroups(workspaceId: string): Promise<import('../types').CollaborationGroup[]>
  createCollaborationGroup(
    workspaceId: string,
    input: CollaborationGroupCreateInput
  ): Promise<import('../types').CollaborationGroup>
  ...
  listExecutionEnvironments(workspaceId: string): Promise<CollaborationExecutionEnvironment[]>
  addExecutionEnvironment(
    workspaceId: string,
    input: WorkspaceExecutionEnvironmentCreateInput
  ): Promise<CollaborationExecutionEnvironment>
  removeExecutionEnvironment(workspaceId: string, deviceId: number): Promise<void>
  initializeExecutionEnvironment(
    workspaceId: string,
    input: { deviceId: number; version: number }
...
export interface SharedWorkspaceGitRepositoriesApi {
  ...
  listBranches(repository: WorkspaceGitRepositoryRef): Promise<WorkspaceGitBranch[]>
}

export interface SharedWorkspaceAttachmentsApi {
  list(issueId: string): Promise<CollaborationAttachment[]>
  listProjectTaskAttachments(projectId: string): Promise<CollaborationAttachment[]>
  upload(issueId: string, file: File): Promise<CollaborationAttachment>
  importContexts(issueId: string, contextIds: number[]): Promise<CollaborationAttachment[]>
...
export interface SharedWorkspaceFilesApi {
  ...
  upload(projectId: string, file: File, path?: string): Promise<CollaborationFile>
  access(fileId: string): Promise<WorkspaceBinaryAccess>
  read(fileId: string): Promise<Blob>
  move(fileId: string, path: string, version: number): Promise<CollaborationFile>
...
export interface SharedWorkspaceDeliveriesApi {
  ...
  addAsset(deliveryId: string, file: File, relativePath: string): Promise<WorkspaceDeliveryAsset>
  finalize(
    deliveryId: string,
...
export interface SharedWorkspaceAutomationsApi {
  ...
  create(projectId: string, input: Record<string, unknown>): Promise<WorkspaceAutomationRule>
  migrateWorkflow(
    projectId: string,
  ...
  runNow(projectId: string, automationId: string): Promise<WorkspaceAutomationRun>
  runWorkflowNode(
    projectId: string,
    issueId: string,
    workflowNodeId: string,
  ...
  listRuns(projectId: string, automationId: string): Promise<WorkspaceAutomationRun[]>
...
export interface SharedWorkspaceIncomingHooksApi {
  ...
  create(projectId: string, input: Record<string, unknown>): Promise<WorkspaceIncomingHook>
  update(
    projectId: string,
    hookId: string,
    input: Record<string, unknown> & { version: number }
...
export interface SharedWorkspaceAgentsApi {
  ...
  create(projectId: string, input: Record<string, unknown>): Promise<WorkspaceProjectAgent>
  update(
    projectId: string,
    agentId: string,
    input: Record<string, unknown>
...
export interface SharedWorkspaceRuntimeApi extends RuntimeConversationClient {
  ...
  checkDeviceAccess?(deviceIds: string[]): Promise<Record<string, RuntimeDeviceAccess>>
  ...
  listDevices(): Promise<import('@wegent/chat-core/execution-project').DeviceInfo[]>
  subscribeChatStream(
  ...
  >
  listModels(deviceId: string): Promise<import('@wegent/chat-core/models').UnifiedModel[]>
  uploadAttachment(
    file: File,
...
export interface WeworkWorkspaceRuntimePort {
  findIssueForTask(task: WorkspaceRuntimeTaskAddress): Promise<CollaborationIssue>
  ...
  updateTrackedTaskStatus(
    task: WorkspaceRuntimeTaskAddress,
    executionStatus: 'queued' | 'running' | 'succeeded' | 'failed' | 'cancelled' | 'archived'
...
export interface SharedWorkspaceHostPort {
  ...
  navigate(location: { projectId: string | null; issueId: string | null; view: string }): void
  openExternal(url: string): void
  notify(message: string, kind: 'success' | 'error'): void
}
