...
>({
  ...
  renderProviderSettings?(context: ProjectManageExtensionContext<Project>): React.ReactNode
  onProjectUpdated?(project: Project): void
}) {
  const { Check, GitBranch, LockKeyhole, Pencil, Search, Trash2, X } = host.icons
  ...
  useEffect(() => {
    ...
    void Promise.all([api.listMembers(project.id), api.listItems(project.id)])
      ...
      .catch(cause => active && reportError(cause, '加载项目失败'))
    return () => {
      active = false
    }
  }, [api, project.id])
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void api.searchUsers(query).then(response => {
        if (!active) return
  ...
  async function updateProject(values: Omit<ProjectManageUpdate, 'version'>): Promise<Project> {
    const updated = await api.updateProject(project.id, { ...values, version })
  ...
  async function saveVisibility(next: ProjectManageVisibility) {
    ...
    try {
      const updated = await updateProject({ visibility: next })
      setVisibility(updated.visibility ?? next)
      host.trackCompleted('update')
    } catch (cause) {
      host.trackFailed()
      setVisibility(previous)
      reportError(cause, '更新项目权限失败')
    } finally {
      setVisibilityBusy(false)
    }
  }

  async function saveDisplay(key: keyof ProjectManageCardDisplay, checked: boolean) {
  ...
  async function addMember(user: User) {
    ...
    try {
      const member = await api.addMember(project.id, user.id, memberRole)
      setMembers(current => [...current, member])
      setMemberQuery('')
      host.trackCompleted('member_invite')
    } catch (cause) {
      host.trackFailed()
      reportError(cause, '添加成员失败')
    } finally {
      setSavingUserId(null)
    }
  }

  async function updateMember(member: Member, role: Exclude<ProjectManageRole, 'Owner'>) {
    try {
      const updated = await api.updateMember(project.id, member.user_id, { role })
      setMembers(current =>
  ...
  async function removeMember(member: Member) {
    if (!host.confirm(`从项目中移除“${member.user_name}”？`)) return
    try {
      ...
      host.trackFailed()
      reportError(cause, '移除成员失败')
    }
  }

  async function persistTags(next: string[]) {
    const updated = await updateProject({ tags: next })
  ...
  async function deleteTag(tag: string) {
    ...
    if (!host.confirm(`删除标签“${tag}”${count ? `，并从 ${count} 个任务上移除` : ''}？`)) {
      return
    }
    setTagBusy(true)
    try {
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-8 py-7">
      <div className="mx-auto max-w-[900px]">
            ...
            <div className="mt-4 space-y-1 rounded-xl bg-muted p-1.5">
              {members.map(member => (
                ...
                >
                  ...
                  <input
                    ...
                    aria-label={host.translate('workbench.project_member_capability_label', '', {
                    ...
                    <>
                      <select
                        ...
                        onChange={event =>
                          void updateMember(
                            member,
                            event.target.value as Exclude<ProjectManageRole, 'Owner'>
                          )
                        }
                      ...
                      {host.renderTooltip({
                        label: host.translate('todo.remove_member', '移除 {{name}}', {
                          name: member.user_name,
                        }),
                        align: 'end',
                        children: (
                          <button
                            ...
                            aria-label={host.translate('todo.remove_member', '移除 {{name}}', {
                              name: member.user_name,
                            })}
                          >
              ...
              <div className="flex gap-2 rounded-lg bg-background/70 p-2">
                ...
                <select
                  ...
                  value={memberRole}
                  onChange={event => setMemberRole(event.target.value as ProjectManageRole)}
                  ...
                  {host.renderTooltip({
                    ...
                    children: (
                      <button
                        ...
                        aria-label={host.translate('todo.confirm_rename', '确认重命名')}
                      >
                  ...
                  {host.renderTooltip({
                    ...
                    children: (
                      <button
                        ...
                        aria-label={host.translate('todo.cancel_rename', '取消重命名')}
                      >
        ...
        {renderProviderSettings?.({ updateProject, reportError })}
