...
export function getAssistantReferences(
  ...
  const explicitReferences =
    references?.filter(
      (reference) => reference.path && reference.path.trim(),
    ) ?? [];
...
export function getDisplayCodexReferences(
...
function normalizeCodexReference(
...
function mergeCodexReferences(
  ...
  const path = isMoreSpecificReferencePath(
...
function isMoreSpecificReferencePath(
...
function normalizeReferencePath(path: string): string {
  ...
  for (const segment of normalized
    .slice(drive?.length ?? (absolute ? 1 : 0))
    .split("/")) {
...
function filterCodexDocumentReferences(
  references: CodexReference[],
): CodexReference[] {
  return references.filter((reference) =>
    isCodexDocumentReferencePath(reference.path),
  );
}
