...
>({
  ...
  TItemElement
> {
  const observedOffsetCallbackRef = useRef<
  ...
  const normalizedPositionKeyRef = useRef<
    string | number | null | typeof UNINITIALIZED_POSITION
  >(UNINITIALIZED_POSITION);
  useLayoutEffect(() => {
    if (!bottomOrigin || normalizedPositionKeyRef.current === positionKey)
      return;
...
>(
  ...
  return Math.min(
    ...
    Math.max(
      0,
      getVirtualizerMaximumOffset(instance, element) + element.scrollTop,
    ),
  );
}
