  ...
  return (
    ...
    >
      <div
        ...
        onPointerEnter={
          diffPreviewDisabled ? undefined : openDiffPreviewAfterDelay
        }
      ...
      >
        <button
          ...
          aria-label={t("file_changes.preview_file_label", {
            path: displayPath,
          })}
        >
          <span
...
export function FileChangeSummaryTrigger({
  file,
  summary,
  disabled,
  diffPreviewDisabled = false,
...
export function FileChangesStats({
  additions,
  deletions,
}: {
  additions: number;
  ...
  const openAfterDelay = () => {
    ...
    openTimerRef.current = window.setTimeout(() => {
      ...
      if (rect) {
        ...
        const nextPlacement =
          spaceAbove < DIFF_PREVIEW_ESTIMATED_HEIGHT && spaceBelow > spaceAbove
            ? "below"
            : "above";
        ...
        const maxLeft =
          window.innerWidth - width - DIFF_PREVIEW_VIEWPORT_GUTTER;
        const left = Math.min(
          Math.max(
            rect.left - DIFF_PREVIEW_HORIZONTAL_OFFSET,
            DIFF_PREVIEW_VIEWPORT_GUTTER,
          ),
          maxLeft,
        );
        const top =
          nextPlacement === "above"
            ? Math.max(
                DIFF_PREVIEW_VIEWPORT_GUTTER,
                rect.top - DIFF_PREVIEW_VERTICAL_GAP,
              )
...
function FileChangeDiffPreview({
  ...
  const content = (
    <div
      ...
      className={`pointer-events-auto fixed z-[9999] max-w-[calc(100vw-4rem)] select-text overflow-hidden rounded-xl border border-border bg-popover text-left text-text-primary shadow-2xl ${portalTheme.className}`}
    ...
    >
      ...
      <div className="max-h-[min(24rem,calc(100vh-9rem))] overflow-auto py-1 font-mono text-xs leading-5">
        ...
        {preview.truncated ? (
          <div className="px-4 py-1 text-xs text-text-muted">...</div>
        ) : null}
      </div>
    </div>
  );
