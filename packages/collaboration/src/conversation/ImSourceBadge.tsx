...
import {
  ...
  isRecord,
  type ImSourceLike,
} from "@wegent/chat-core/im-source";

export interface ImSourceBadgeProps {
  source: ImSourceLike;
  className?: string;
  testId?: string;
}
...
export function ImSourceBadge({
  ...
  const channelLabel = isRecord(source)
    ? getTrimmedString(source.channel_label)
    : undefined;
  ...
  const label =
    channelLabel ?? (labelKey ? t(labelKey) : t("workbench.im_source_label"));
