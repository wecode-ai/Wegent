...
const CONTEXT_COMPACTION_TOOLS = new Set([
  "context_compaction",
  "contextcompaction",
]);
...
function matchesToolName(name: string, names: Set<string>): boolean {
  ...
  if (
    doubleUnderscoreIndex >= 0 &&
    names.has(normalized.slice(doubleUnderscoreIndex + 2))
  ) {
    return true;
  }
...
export function getFileInputPath(
  block: Pick<ToolBlock, "toolInput">,
): string | undefined {
  return getFileInputPaths(block)[0];
}

export function getFileInputPaths(
  block: Pick<ToolBlock, "toolInput">,
): string[] {
...
function getDirectFileInputPath(
...
function getPatchFilePaths(
  input: Record<string, unknown> | undefined,
): string[] {
  if (!input) return [];
  const patchText = getStringField(
    input,
    "patch",
    "input",
    "arguments",
