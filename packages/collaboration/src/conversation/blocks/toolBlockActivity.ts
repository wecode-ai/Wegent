...
import type { FileChangesBlock, ProcessingBlock, ToolBlock } from "./types";
import {
  getFileInputPaths,
  getInputField,
  isCommandToolName,
...
export function buildProcessingDisplayRows(
  ...
  const hasFileChangesBlock = blocks.some(
  ...
  const flushFileChanges = () => {
    ...
    rows.push({
      ...
      id: block.id,
      block,
      sourceBlockIds: consecutiveFileChanges.map(
...
function mergeConsecutiveFileChanges(
  blocks: FileChangesBlock[],
): FileChangesBlock {
  const first = blocks[0];
  const latest = blocks[blocks.length - 1];
  const summaries = blocks.map((block) => block.fileChanges);
  const files = mergeFileChangeItems(
    summaries.flatMap((summary) => summary.files),
  );
...
export function summarizeToolBlocks(blocks: ToolBlock[]): string {
  ...
  if (stats.files > 0)
    parts.push(`已读取 ${formatCount(stats.files, "个文件")}`);
  if (stats.searches > 0) parts.push("已搜索代码");
  if (stats.creates > 0)
    parts.push(`已新增 ${formatCount(stats.creates, "个文件")}`);
  if (stats.edits > 0)
    parts.push(`已编辑 ${formatCount(stats.edits, "个文件")}`);
  if (stats.guidance > 0) parts.push("已引导对话");
  if (stats.commands > 0)
    parts.push(`已运行 ${formatCount(stats.commands, "条命令")}`);
  if (stats.tools > 0)
    parts.push(`已调用 ${formatCount(stats.tools, "个工具")}`);
  if (stats.failedCommands > 0) {
    parts.push(`运行失败 ${formatCount(stats.failedCommands, "条命令")}`);
  }
  if (stats.failedTools > 0)
    parts.push(`执行失败 ${formatCount(stats.failedTools, "个工具")}`);

  return parts.length > 0
    ? parts.join(" ")
    : `已调用 ${formatCount(blocks.length, "个工具")}`;
}
...
export function getToolActivityKind(
  ...
  if (isCommandToolName(name)) {
    return getCommandActivityKind(
      getInputField(block, "command", "cmd", "commandLine"),
    );
  }
  return "tool";
}
...
export function getToolActivitySearchItem(
  ...
  const name = block.toolName.toLowerCase();
  if (isWebSearchToolName(name) || getToolActivityKind(block) !== "search")
    return undefined;

  if (isCommandToolName(name)) {
    const command = getInputField(block, "command", "cmd", "commandLine");
    const summary = getSearchCommandSummary(
      command,
      getCommandWorkingDirectory(block),
    );
...
export function getToolActivityGroupKind(
  blocks: ToolBlock[],
): ToolActivityKind {
...
function getSearchCommandSummary(
  ...
  if (PATTERN_SEARCH_COMMANDS.has(executable)) {
    ...
    return {
      query: parsed.query,
      scope: formatSearchScope(parsed.scopes, cwd),
    };
  }
  return undefined;
}
...
function getGitSearchCommandSummary(
  ...
  const searchArgs =
    separatorIndex === -1 ? grepArgs : grepArgs.slice(0, separatorIndex);
  const pathspecs =
    separatorIndex === -1 ? [] : grepArgs.slice(separatorIndex + 1);
  const parsed = getPatternSearchArguments(searchArgs);
  if (!parsed?.query) return undefined;
  return {
    query: parsed.query,
    scope: formatSearchScope(
      pathspecs.length > 0 ? pathspecs : parsed.scopes,
      cwd,
    ),
  };
}
...
function getPatternSearchArguments(args: string[]):
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg.startsWith("-")) {
      ...
      if (
        !arg.includes("=") &&
        SEARCH_COMMAND_OPTIONS_WITH_VALUES.has(optionName)
      )
        index += 1;
      continue;
    }
...
function formatSearchScope(
  ...
  const scope =
    scopes.find((item) => item && item !== "." && item !== "./") ??
    cwd ??
    scopes[0];
...
function getCommandWorkingDirectory(
  block: Pick<ToolBlock, "toolInput">,
): string | undefined {
...
function getSedInputPaths(args: string[]): string[] {
  const nonOptionArgs = getPathArguments(
    args,
    ...
    if (
      ...
      const commandOptionIndex = words.findIndex(
        (word, index) =>
          ...
          ["-c", "-command"].includes(word.toLowerCase()),
      );
      const commandArgument = words[commandOptionIndex + 1];
      if (
        commandOptionIndex < 0 ||
        !commandArgument ||
        words.length !== commandOptionIndex + 2
      ) {
        break;
      }
      current = commandArgument.trim();
...
export function isContextCompactionToolBlock(
  block: ProcessingBlock,
): block is ToolBlock {
...
function isRedundantPatchApplyBlock(block: ProcessingBlock): boolean {
  return (
    block.type === "tool" &&
    block.status === "done" &&
    isPatchApplyToolName(block.toolName)
  );
}
...
function isHiddenToolActivityBlock(block: ProcessingBlock): boolean {
  return (
    ...
    (HIDDEN_ACTIVITY_TOOLS.has(block.toolName.toLowerCase()) ||
      (block.toolName === RECONNECTING_TOOL_NAME &&
        isCompletedToolBlock(block)))
  );
}
...
export function isWebSearchActivityGroup(blocks: ToolBlock[]): boolean {
  return (
    blocks.length > 0 &&
    blocks.every((block) => isWebSearchToolName(block.toolName))
  );
}
...
export function isGuidanceActivityGroup(blocks: ToolBlock[]): boolean {
  return (
    blocks.length > 0 &&
    blocks.every((block) => isGuidanceToolName(block.toolName))
  );
}
