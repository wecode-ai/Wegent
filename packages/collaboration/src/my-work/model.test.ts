...
describe('my-work model', () => {
  it('preserves grouped-view overlap while assigning one primary group', () => {
    ...
    expect(isMyWorkItemInGroup(review, 'action', isExecutionStateActive)).toBe(true)
    expect(isMyWorkItemInGroup(review, 'review', isExecutionStateActive)).toBe(true)
    ...
    expect(isMyWorkItemInGroup(approval, 'approval', isExecutionStateActive)).toBe(true)
    expect(isMyWorkItemInGroup(approval, 'action', isExecutionStateActive)).toBe(false)
  ...
  it('uses execution state when present and the runtime binding only when absent', () => {
    ...
    expect(
      myWorkGroupOf(item({ status: 'in_progress', has_active_task: true }), isExecutionStateActive)
    ).toBe('running')
  })
  ...
  it('sorts due items before undated items and then by priority', () => {
    ...
    expect([...items].sort(compareMyWorkItems).map(entry => entry.id)).toEqual([
      'dated',
      'urgent',
      'low',
    ])
  ...
  it('groups timelines, projects, and valid calendar entries deterministically', () => {
    ...
    expect(groupMyWorkTimeline(items).map(bucket => bucket.entries.map(entry => entry.id))).toEqual(
      [['B-1'], ['A-1', 'A-2']]
    )
    expect(
      buildMyWorkCalendarEntries(items, isExecutionStateActive).map(entry => entry.id)
    ).toEqual(['B-1'])
  })
})
