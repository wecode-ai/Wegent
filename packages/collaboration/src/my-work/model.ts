...
export type IsExecutionStateActive = (state: string | null | undefined) => boolean
...
export function isLoopItemExecutionActive(
  ...
  return item.status === 'in_progress' || isExecutionStateActive(item.execution_state)
}
...
export function isMyWorkItemInGroup(
  ...
  switch (group) {
    case 'approval':
      return item.execution_state === 'waiting_approval' && item.can_approve === true
    case 'action':
      return (
...
export function listMyWorkProjects(items: readonly MyWorkItem[]): Array<readonly [string, string]> {
  return Array.from(
...
export function buildMyWorkCalendarEntries<T extends MyWorkItem>(
  ...
  return items.flatMap(item => {
    if (!item.due_at || Number.isNaN(new Date(item.due_at).getTime())) return []
    return [
      {
        id: item.id,
        title: item.title,
