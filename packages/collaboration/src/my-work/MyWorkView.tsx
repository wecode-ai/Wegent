...
export interface MyWorkViewProps<T extends MyWorkItem> {
  ...
  renderCalendar: (props: { items: readonly T[]; onSelectItem: (item: T) => void }) => ReactNode
}
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-border bg-background shadow-sm">
      <header className="flex items-center gap-2 border-b border-border px-4 py-3">
        ...
        <h2 className="text-sm font-semibold">{translate(meta.labelKey, meta.fallback)}</h2>
    ...
    >
      ...
      <div className="divide-y divide-border">
        {sorted.map(item => {
          ...
          const [priorityKey, priorityFallback] = MY_WORK_PRIORITY_LABELS[item.priority]
          return (
            <button
              key={item.id}
              type="button"
            ...
            >
              ...
              <span className="flex items-center gap-1.5 text-xs text-text-secondary">
                ...
                />
                {translate(MY_WORK_GROUP_META[group].labelKey, MY_WORK_GROUP_META[group].fallback)}
              </span>
              <span
                className={classNames(
                  'text-xs',
      ...
      <div className="h-full overflow-y-auto px-8 pb-24 pt-7">
        <div className="mx-auto max-w-[960px]">
          <div className="flex items-center gap-3">
            ...
            <label className="relative ml-auto inline-flex h-8 items-center rounded-lg border border-border bg-background px-3 text-xs text-text-secondary hover:bg-muted">
              ...
              <select
                ...
                aria-label={translate('todo.my_work_project_filter', '按项目过滤任务')}
              >
                <option value="all">{translate('todo.all_projects', '全部项目')}</option>
          ...
          <p className="mt-1 text-sm text-text-muted">
            {translate('todo.my_work_subtitle', '自动汇总本机上的全部任务，无需关联项目空间。')}
          </p>
          <div className="mt-6">
            {view === 'group' ? (
                ...
                {MY_WORK_GROUP_ORDER.map(groupKey => (
                  <GroupSection
                    ...
                    items={visibleItems.filter(item =>
                      isMyWorkItemInGroup(item, groupKey, isExecutionStateActive)
                    )}
                    translate={translate}
            ...
            {view === 'calendar' ? renderCalendar({ items: visibleItems, onSelectItem }) : null}
            {view === 'timeline' ? (
              <TimelineView
                items={visibleItems}
                locale={locale}
