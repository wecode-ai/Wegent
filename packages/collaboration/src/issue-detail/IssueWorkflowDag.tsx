...
export interface IssueWorkflowDagProps {
...
const RuntimeStageNodeCard = memo(function RuntimeStageNodeCard({
  ...
  const { stage, tasks, translate: t, selected, onSelect } = data;
  const isLoopControl =
    stage.node_type === "loop_start" || stage.node_type === "loop_end";
  ...
  const collectors =
    stage.node_type === "branch" ? (stage.collectors ?? {}) : {};
  const collectorLabels = Object.entries(collectors).map(
    ([platform, collector]) => {
      const platformLabel = platform === "gitlab" ? "GitLab" : "GitHub";
      const stateLabel =
        collector.status === "error"
          ? collector.error || t("todo.workflow_branch_collector_error")
          : collector.status === "needs_registration" ||
              collector.mode === "webhook"
    ...
    >
      ...
      <footer className="issue-workflow-node-footer">
        ...
        <span className="ml-auto">
          {t("todo.workflow_task_count", { count: tasks.length })}
        </span>
      </footer>
...
const RuntimeLoopNodeCard = memo(function RuntimeLoopNodeCard({
  ...
  const { stage, translate: t, selected, childSelected, onSelect } = data;
...
function workflowEdge(
  source: SharedWorkflowNode,
  target: SharedWorkflowNode,
  completed: boolean,
): Edge {
  const color = completed
    ? "rgb(0 162 64 / 0.55)"
    : "rgb(var(--color-text-muted) / 0.45)";
  return {
...
function loopBodyLayout(
  loop: SharedWorkflowNode,
  body: SharedWorkflowNode[],
  tasks: SharedWorkflowTaskBinding[],
  translate: IssueWorkflowTranslate,
      ...
      return dependency
        ? [
            workflowEdge(
              dependency,
              node,
              isWorkflowNodeCompleted(dependency.status),
            ),
          ]
        : [];
    }),
...
export function IssueWorkflowDag({
  ...
  const graph = useMemo(() => {
    ...
    nodes.forEach((node) => {
      ...
      (node.body_node_ids ?? []).forEach((bodyId) =>
        loopIdByNodeId.set(bodyId, node.id),
      );
    });
    ...
    return {
      edges: [
        ...edges,
        ...Array.from(loopLayouts.values()).flatMap((layout) => layout.edges),
      ],
      nodes: [
        ...laidOutTopLevel,
  ...
  }, [currentStageId, effectiveSelectedStageId, nodes, tasks, translate]);
  ...
  useEffect(() => {
    const deactivateGraphInteraction = (event: PointerEvent) => {
      if (
        ...
        !graphContainerRef.current?.contains(event.target)
      ) {
    ...
    return () =>
      document.removeEventListener("pointerdown", deactivateGraphInteraction);
  }, []);
    ...
    <>
      ...
      >
        <ReactFlow
          ...
          onNodeClick={(_, node) =>
            setStageSelection({ stageId: node.id, currentStageId })
          }
          minZoom={0.35}
          maxZoom={1.5}
