...
import {
  IssueActivityThread,
  type IssueActivityThreadModel,
} from "./IssueActivityThread";
...
export function IssueProjectChatThread({
  ...
  function renderMessage(message: ProjectChatMessage, eventOnly = false) {
    ...
    const singleExecution =
      singleExecutionForMessage?.(target?.activityMessage ?? message) ?? false;
    const dispatchRole =
      message.sender.type === "agent" &&
      message.metadata.dispatch_role === "manager"
        ? "manager"
        : message.sender.type === "agent"
          ? "member"
          : undefined;
