...
const message = (id: string, status = 'streaming'): ProjectChatMessage =>
  ({
    ...
    content: id,
    sender: { type: id === 'root' ? 'user' : 'agent', id: 'sender', name: 'Sender' },
    metadata: {},
    status,
    createdAt: '',
    updatedAt: '',
...
describe.each(['cloud-task-activity-card-', 'collaboration-chat-card-'])(
  ...
  prefix => {
    ...
    function Fixture({ messages }: { messages: ProjectChatMessage[] }) {
      controls = useIssueActivityScroll({ messages, loading: false, cardTestIdPrefix: prefix })
      return (
    ...
    it('reveals only the submitted card and relinquishes follow when the user scrolls away', async () => {
      ...
      scrollTo.mockClear()
      act(() => container.querySelector('[data-testid="list"]')!.dispatchEvent(new Event('scroll')))
      await render()
      act(() => frame?.(0))
      expect(scrollTo).not.toHaveBeenCalled()
    })
    it('stops following after the final response and keeps top-level submit at the top', async () => {
      act(() => controls.followCard('root'))
      await render([message('root'), message('run', 'completed')])
      act(() => frame?.(0))
      scrollTo.mockClear()
      cardBottom = 800
      await render([message('root'), message('run', 'completed'), message('another')])
      act(() => frame?.(0))
      expect(scrollTo).not.toHaveBeenCalled()
      act(() => controls.scrollTaskCommentsToTop())
