...
export interface WorkflowDeliverableDraft {
  requirement: IssueWorkflowDeliverableRequirement;
  text?: string;
  files?: File[];
  url?: string;
  title?: string;
...
function isComplete(value: WorkflowDeliverableDraft | undefined): boolean {
  ...
  if (type === "file" || type === "code_snapshot")
  ...
  const canSubmit =
    ...
    (action !== "approve" ||
      requirements.every((requirement) =>
        isComplete(values[requirement.id]),
      )) &&
              ...
              >
                ...
                {requirement.value_type === "text" ? (
                  <textarea
                    value={value.text ?? ""}
                    onChange={(event) =>
                      updateValue(requirement, { text: event.target.value })
                    }
                  ...
                  <div className="space-y-2">
                    <input
                      value={value.url ?? ""}
                      onChange={(event) =>
                        updateValue(requirement, { url: event.target.value })
                      }
                      placeholder="https://"
                    ...
                    <input
                      value={value.title ?? ""}
                      onChange={(event) =>
                        updateValue(requirement, { title: event.target.value })
                      }
                      placeholder={t(
                        "todo.workflow_deliverable_link_title",
                        "链接标题（可选）",
                      )}
                      className="h-9 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none focus:border-blue-500"
                    />
