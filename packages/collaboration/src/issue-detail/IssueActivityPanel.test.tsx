...
describe("IssueActivityPanel", () => {
  ...
  it("uses only the current Issue chat stream instead of adding REST records to the PC feed", async () => {
    const messages: ProjectChatMessage[] = Array.from(
      { length: 7 },
      (_, index) => ({
  ...
  it("updates the actual activity badge and viewer from one execution subscription", async () => {
    ...
    expect(
      document.querySelector('[data-testid="runtime-execution-detail-status"]')
        ?.textContent,
    ).toContain("执行中");
    ...
    expect(badge.getAttribute("data-status")).toBe("succeeded");
    vi.mocked(runtime.getTranscript).mockRejectedValueOnce(
  ...
  it("keeps one comment composer and clears it when the issue changes", () => {
    ...
    expect(
      container.querySelectorAll(".composer-prosemirror-editor"),
    ).toHaveLength(1);
  });
  ...
  it("preserves execution output instead of interpreting marker-like text", () => {
    expect(activityDisplayBody("REMOTE_AGENT_DISPATCH_COMPLETED", "")).toBe(
      "REMOTE_AGENT_DISPATCH_COMPLETED",
    );
    expect(activityDisplayBody("正常评论", "")).toBe("正常评论");
    expect(activityDisplayBody("", "暂无内容")).toBe("暂无内容");
  });
  ...
  it("does not show an execution from before the current assignment", () => {
    ...
    const current = container.querySelector(
      '[data-testid="collaboration-assignment-assignment-1"]',
    );
