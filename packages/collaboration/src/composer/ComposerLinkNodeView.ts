...
export class ComposerLinkNodeView implements NodeView {
  ...
  private readonly payload = (): ComposerLinkPayload =>
    this.node.attrs as ComposerLinkPayload;
  ...
  private readonly handleMouseDown = (event: MouseEvent): void => {
    ...
    this.view.dispatch(
      this.view.state.tr.setSelection(
        TextSelection.create(this.view.state.doc, nextPosition),
      ),
    );
    this.view.focus();
  };
  ...
  private readonly handleClick = (event: MouseEvent): void => {
    ...
    const range =
      position === undefined
        ? undefined
        : computeSerializedRange(this.view, position, this.node);
