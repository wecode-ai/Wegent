...
import { useConversationTranslation } from "../conversation/ConversationTranslation";
...
export function LinkEditPopover({
  ...
  const { openExternalUrl } = useMarkdownServices();
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const placeAbove =
        spaceAbove >= POPOVER_HEIGHT_ESTIMATE || spaceAbove >= spaceBelow;
      setPosition({
  ...
  const urlErrorLabel = t("workbench.browser_invalid_url");
  ...
  return createPortal(
    <div
      ...
      className={`fixed z-system-popover flex -translate-x-1/2 items-center gap-1 rounded-2xl border border-border/70 bg-background p-1.5 shadow-[0_16px_44px_rgba(0,0,0,0.16)] ${portalTheme.className}`}
      style={{
        ...portalTheme.style,
        left: position.left,
    ...
    >
      ...
      <button
        ...
        aria-label={t("workbench.edit_link_text")}
        title={t("workbench.edit_link_text")}
      >
      ...
      <button
        ...
        onClick={() => setMode("url")}
        className="flex h-8 w-8 items-center justify-center rounded-lg text-text-secondary hover:bg-muted hover:text-text-primary"
        aria-label={t("workbench.edit_link_url")}
        title={t("workbench.edit_link_url")}
      >
      ...
      <button
        ...
        aria-label={t("workbench.remove_link_preview")}
        title={t("workbench.remove_link_preview")}
      >
...
function LinkEditInput({
  ...
  return createPortal(
    <div
      ...
      ref={popoverRef}
      data-testid="link-edit-popover"
      className={`fixed z-system-popover flex -translate-x-1/2 flex-col items-center gap-1 rounded-2xl border border-border/70 bg-background p-1.5 shadow-[0_16px_44px_rgba(0,0,0,0.16)] ${portalTheme.className}`}
      style={{
        ...portalTheme.style,
        left: position.left,
        bottom: position.bottom,
