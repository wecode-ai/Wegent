...
export interface QuickPhraseMenuProps {
  translate: CollaborationTranslate
  globalPhrases: QuickPhrase[]
  attachmentUrl?: (path: string) => string | undefined
  onRemoveStash?: (phrase: QuickPhrase) => Promise<void>
...
function StashThumbnail({
  path,
  attachmentUrl,
}: {
  path: string
  attachmentUrl?: QuickPhraseMenuProps['attachmentUrl']
}) {
  if (isImagePath(path) && attachmentUrl) {
    return (
      <img
...
function StashCard({
  ...
  return (
    ...
    >
      <button
        type="button"
        data-testid={`quick-phrase-stash-select-${phrase.id}`}
        ...
        {paths.length > 0 ? (
          <span className="relative flex h-11 min-w-12 items-center justify-center">
            {thumbnails.map((path, index) => (
              ...
              >
                <StashThumbnail path={path} attachmentUrl={attachmentUrl} />
              </span>
            ))}
            {paths.length > 3 && (
      ...
      <button
        ...
        className="absolute right-1 top-1 flex h-5 w-5 max-md:h-11 max-md:w-11 items-center justify-center rounded-full bg-background/90 text-text-muted shadow-sm hover:bg-muted hover:text-text-primary"
      >
...
export function QuickPhraseMenu({
  ...
  const runStashAction = async (action: (() => Promise<void>) | undefined) => {
        ...
        <button
          ...
          className={
            compact
              ...
              : 'flex h-8 w-8 max-md:h-11 max-md:w-11 items-center justify-center rounded-lg text-text-primary hover:bg-muted disabled:opacity-40'
          }
      ...
      {open &&
        ...
        createPortal(
          <div
            ...
            className={`${portalTheme.className} fixed z-system-popover w-[min(26rem,calc(100vw-2rem))] overflow-y-auto rounded-xl border border-border bg-background p-1.5 text-text-primary shadow-lg`}
          >
            {actionError && (
            ...
            <div className="flex h-9 max-md:h-11 items-center gap-2 rounded-lg bg-muted/60 px-2">
              ...
              <input
                ...
                className="min-w-0 flex-1 max-md:min-h-11 bg-transparent text-sm outline-none placeholder:text-text-muted"
              />
            </div>
            {stashed.length > 0 && (
              <div className="relative mt-1 px-1 pt-2">
                {previewedStash && (
                  <StashPreview phrase={previewedStash} attachmentUrl={attachmentUrl} />
                )}
                <div className="mb-1.5 flex items-center justify-between px-1">
                  ...
                  <button
                    ...
                    disabled={actionPending || !onClearStash}
                    onClick={() => void runStashAction(onClearStash)}
                ...
                >
                  {stashed.map((phrase, index) => (
                    <StashCard
                      ...
                      deleting={actionPending || !onRemoveStash}
                      onDelete={() =>
                        void runStashAction(onRemoveStash ? () => onRemoveStash(phrase) : undefined)
                      }
                    />
                  ))}
                </div>
            ...
            <div role="listbox" className="max-h-64 overflow-y-auto">
              {filteredProjectPhrases.map((phrase, index) => (
                <button
                  ...
                  className={`flex w-full items-start gap-2 rounded-lg px-2 py-2 text-left max-md:min-h-11 ${selectedIndex === stashed.length + index ? 'bg-muted' : 'hover:bg-muted'}`}
                >
              ...
              {globalRegular.map((phrase, index) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => choose(phrase)}
                    className={`flex w-full items-start gap-2 rounded-lg px-2 py-2 text-left max-md:min-h-11 ${selectedIndex === optionIndex ? 'bg-muted' : 'hover:bg-muted'}`}
                  >
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-sm font-normal">{phrase.title}</span>
                      <span className="block truncate text-xs text-text-muted">
            ...
            <button
              ...
              className="mt-1 flex h-8 max-md:min-h-11 w-full items-center gap-2 border-t border-border px-2 pt-1 text-sm text-text-secondary hover:text-text-primary"
            >
