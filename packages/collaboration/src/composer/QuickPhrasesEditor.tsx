...
import type { QuickPhrase, QuickPhraseMode } from '@wegent/chat-core/composer-quick-phrases'

export type QuickPhraseChangeAction = 'create' | 'update' | 'delete' | 'move'

export interface QuickPhrasesEditorProps {
  translate: CollaborationTranslate
  phrases: QuickPhrase[]
  onChange: (phrases: QuickPhrase[], action: QuickPhraseChangeAction) => void | Promise<void>
  disabled?: boolean
  testIdPrefix?: string
}
...
export function QuickPhrasesEditor({
  ...
  const change = async (next: QuickPhrase[], action: QuickPhraseChangeAction) => {
    if (disabled) return false
    setSaving(true)
    setValidationError('')
    try {
      await onChange(next, action)
      return true
    } catch (cause) {
        ...
        {phrases.map((phrase, index) => (
          ...
          >
            ...
            <button
              ...
              className="h-8 w-8 max-md:h-11 max-md:w-11 rounded-lg p-2 hover:bg-background disabled:opacity-30"
            ...
            <button
              ...
              disabled={disabled}
              className="h-8 w-8 max-md:h-11 max-md:w-11 rounded-lg p-2 text-destructive hover:bg-destructive/10 disabled:opacity-30"
          ...
          <div className="w-full max-w-[520px] rounded-[20px] border border-border bg-background p-5 shadow-lg">
            ...
            <label className="mt-4 block text-sm">
              ...
              <input
                ...
                className="mt-1 h-10 max-md:min-h-11 w-full rounded-lg border border-border bg-background px-3 outline-none focus:border-blue-500"
              />
            </label>
            ...
            <fieldset className="mt-3">
              ...
              <div className="mt-2 flex gap-4">
                {(['normal', 'plan', 'goal'] as QuickPhraseMode[]).map(mode => (
                  <label
                    key={mode}
                    className="flex items-center gap-1.5 text-sm max-md:min-h-11 max-md:min-w-11"
                  >
                    <input
                      disabled={disabled}
                      type="radio"
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                ...
                className="h-8 max-md:min-h-11 max-md:min-w-11 rounded-lg px-3 text-sm hover:bg-muted"
              >
                {t('common.cancel', '取消')}
              </button>
              <button
                ...
                className="h-8 max-md:min-h-11 max-md:min-w-11 rounded-lg bg-text-primary px-3 text-sm font-medium text-background hover:opacity-90"
              >
                {t('common.save', '保存')}
              </button>
            </div>
