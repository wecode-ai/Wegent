...
} from "./composerMarkdownSchema";
import {
  findComposerMarkdownLinks,
  parseComposerMarkdown,
} from "./composerMarkdownParser";
import {
  ...
  parseDOM: [
    {
      ...
      getAttrs(element) {
        ...
        return {
          ...
          reference:
            element.getAttribute("data-composer-skill-reference") ?? "",
        };
      },
    },
  ],
...
export function createComposerDocument(value: string): ProseMirrorNode {
  return parseComposerMarkdown(
    value,
    composerSchema,
    createPlainComposerDocument,
  );
}

function createPlainComposerDocument(
  value: string,
  autolinks = false,
): ProseMirrorNode {
  ...
  for (const token of tokens) {
    ...
    if (sanitizedValue[offset] === " ") {
      content.push(
        composerSchema.text(" ", [
          composerSchema.marks.composer_mention_separator.create(),
        ]),
      );
      offset += 1;
    }
  ...
  text.split("\n").forEach((line, index) => {
    ...
    for (const link of autolinks ? findComposerMarkdownLinks(line) : []) {
      ...
      if (link.start > offset)
        content.push(composerSchema.text(line.slice(offset, link.start)));
      content.push(
    ...
    if (offset < line.length)
      content.push(composerSchema.text(line.slice(offset)));
  });
  return content;
}
        ...
        !items.some(
          (right, otherIndex) =>
            ...
            (left.start > right.start ||
              (left.start === right.start && index > otherIndex)),
        ),
    )
