...
import { buildMyWorkCalendarEntries, MyWorkView, type MyWorkItem } from '../my-work'
  ...
  return (
    ...
    >
      {entries.map(entry => (
        ...
        >
          <time className="w-28 shrink-0 text-xs text-text-muted" dateTime={entry.start}>
          ...
          <span className="shrink-0 text-xs text-text-muted">{entry.item.project_name}</span>
        </button>
      ))}
    </div>
  )
...
export function MyWorkAdapter({ items, locale, onBack, onSelectItem }: MyWorkAdapterProps) {
