...
- `knowledge_runtime` 已经作为薄 remote adapter 落地，负责 `content_ref` 拉取、协议校验、鉴权和调用 `knowledge_engine`；默认 rollout 目标仍以 Backend local mode 为主。
...
- `knowledge_engine` 已承接底层索引与检索执行；Backend local data-plane 与 `knowledge_runtime` remote 都围绕该执行库收敛。
