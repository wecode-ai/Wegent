...
复用 `resource_members` 和 `share_links`，新增 `CloudProject` 资源类型。项目角色按动作授权，不使用单一的 `can_edit` 推断所有能力。

| 角色       | 读取/评论 | 编辑/执行 | 领取未分配任务 | 移交自己的任务/提交待验收 | 分配他人/完成/重新打开 | 管理成员 | 归档/移交所有权 |
| ---------- | --------- | --------- | -------------- | ------------------------- | ---------------------- | -------- | --------------- |
| Reporter   | 是        | 否        | 否             | 否                        | 否                     | 否       | 否              |
| Developer  | 是        | 是        | 是             | 是                        | 否                     | 否       | 否              |
| Maintainer | 是        | 是        | 是             | 是                        | 是                     | 是       | 否              |
| Owner      | 是        | 是        | 是             | 是                        | 是                     | 是       | 是              |

Issue 响应通过 `permissions` 返回 `edit_content`、`comment`、`claim`、`handoff`、`assign`、`execute`、`submit_review`、`complete` 和 `reopen`。客户端只消费这些服务端能力；更新接口根据负责人、状态和内容字段的实际变化拆分并校验动作，空更新不额外要求写权限。

空间和项目所有权只能移交给已批准的人类成员。移交操作锁定资源后再次确认当前所有者，并在同一事务中把新成员提升为 `Owner`、原所有者降为 `Maintainer`。
