...
云端和远程设备通过 Socket.IO ACK 返回 runtime RPC 结果。executor 对超过 512 KiB 的 ACK 使用 `gzip+base64+json` envelope 压缩，Backend 在进入 runtime work service 前解压；本地 App IPC 始终返回原始 JSON。压缩后的 envelope 也必须保留在 Socket.IO 1 MB 限制以内，无法满足时返回小型 `runtime_rpc_response_too_large` 错误，不能发送超限数据导致设备连接和心跳一起断开。
