...
Wework 发送的富文本引用（例如技能或插件标签）可能在 Codex provider transcript 中被规范化为纯文本。executor 合并 provider transcript 与本地缓存用户消息时，必须优先按稳定的 `clientMessageId` 匹配，并恢复缓存中的原始用户可见 `content`，同时保留 provider 消息上的运行时字段。只有缺少稳定客户端消息 id 时，才使用规范化文本签名匹配。这样任务刷新后仍能把技能引用渲染为标签，而不会退化成裸 `$plugin:skill` 文本。
