...
技能的 MCP server 只在技能被本次请求激活时才挂载到运行时：

| 技能来源 | 是否挂载 MCP |
| --- | --- |
| Ghost / Bot 显式声明的技能 | 是 |
| 用户显式选择或预加载的技能 | 是 |
| backend 为功能主动注入的技能（交互表单、订阅管理） | 是 |
| 用户默认技能绑定里“仅可用”的技能 | 否 |

“仅可用”的技能仍会下载到执行端并出现在 `skill_names` 中，模型可以按需加载；只是它们的 MCP 工具不会进入本次会话。因为每个 MCP server 的工具描述都会进入模型 prompt，用户绑定的技能一多，工具描述就会迅速吃掉整个上下文窗口，最终让模型直接返回 `Prompt is too long`。

运行时由 backend 打标：`mcp_deferred` 表示技能可用但不挂载 MCP；需要保留 MCP 的注入技能（如订阅管理）在引用里显式声明 `mcp_active`。
