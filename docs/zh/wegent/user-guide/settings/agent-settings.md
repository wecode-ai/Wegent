...
Wegent Web 与 Wework 的协作项目使用同一套智能体新建表单。表单中的配置分为三个彼此独立的维度：
...
**Wegent 托管或共享智能体是一种来源，不是执行器类型。** 共享智能体保留自身定义的执行器和能力配置；项目只保存对该智能体的引用。当前项目中创建的智能体则由表单直接保存执行器、模型、系统提示词和能力要求。
