...
在 Issue 评论里输入 `@` 可以选取项目成员并插入 `@成员名`。只有项目成员能被 @；@ 非成员会被拒绝。被 @ 的成员会收到站内通知，同时尝试推送钉钉；两条消息都可以点回该 Issue。@ 智能体不受影响，仍按原有逻辑触发执行。

通知标题写明发起人和 Issue，正文是评论摘要并附上看板名。推送钉钉时标题加粗显示，正文与「查看详情」链接保持一致，站内和 IM 收到的消息读起来是同一套内容。

## 任务执行通知

Issue 自己的机器人任务在进入执行、需要人工处理、结束时会各发一条通知。需要人工处理包括：等待人工审批、等待选择运行设备、以及 AI 需要你补充输入。收件人是任务负责人；任务没有负责人时通知创建者。点击通知会打开对应 Issue。
