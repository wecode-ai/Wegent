...
协作看板点击 Issue 后，右侧详情按时间顺序展示创建、状态变更和评论。执行任务属于发起它的评论：评论卡片展示执行状态；点击状态标记在右侧打开关联任务对话，保留 Issue 详情。协作界面不再打开独立的执行浮层；任务 Tab 的看板视图仍可使用原有任务入口。

评论卡片默认只显示正文预览，长内容可展开。作者、执行状态、时间和“回复”入口不随正文折叠。点击“回复”才展开该卡片内的输入框；执行中的评论不提供回复。运行结束后的状态以保存的执行结果为准，不把已完成任务显示为“状态待核实”。

AI 管理的工作流由管理者根据 Issue 生成每个子任务的执行提示词，并交给执行者。执行者回报结果后，管理者读取这些回报，再决定将 Issue 移到“待确认”或“已完成”；子任务完成本身不会自动触发这两个状态迁移。迁移记录与人工状态变更一同显示在动态中。自动化摘要只展示当前阶段和各子任务进度，避免重复列出整条阶段链。
