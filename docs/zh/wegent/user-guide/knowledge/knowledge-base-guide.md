...
在新建或编辑知识库的「高级设置」中开启「自动更新钉钉文档」（默认关闭）。系统每天扫描一次，检查该库已有及以后导入的钉钉文档更新时间；已成功且可用的副本，若来源时间与已保存基线相同，或来源未给出更新时间而本地已有基线，则跳过；其余副本重新导入（含尚无基线者），且只有钉钉给出更新时间时才会建立基线。首次检查在下一轮扫描时发生，不保证固定时刻完成。
