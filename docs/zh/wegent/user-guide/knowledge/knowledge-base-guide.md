...
在新建或编辑知识库的「高级设置」中开启「自动更新钉钉文档」（默认关闭）。系统每天扫描一次，检查该库已有及以后导入的钉钉文档更新时间；已成功且可用的副本，若来源时间与已保存基线相同则跳过，否则重新导入。旧副本没有基线时会更新一次建立基线。首次检查在下一轮扫描时发生，不保证固定时刻完成。
...
第一版接受钉钉 MCP `updateTime` 的延迟：正文可能已变化而元信息时间尚未变化，导致本轮跳过。上线后需关注不同格式的时间延迟、长期未变化的节点和实际内容新鲜度；此前在线文档实测出现过该现象，不保证下一周期必然恢复。需要立即更新时可手动再次导入，后台同步测试接口仍遵循相同的时间判变规则。

部署需要启用现有的定时任务配置 `SCHEDULED_TASKS_ENABLED`，并运行 Celery Beat 和消费默认队列的 Worker。升级后应重启这些进程以加载新增任务。无需数据库迁移。
