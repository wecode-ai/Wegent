...
模板默认为钉钉内置的 markdown 卡片（正文 + 按钮），不需要自建卡片。这里特意不用 AI 卡片模板：AI 卡片自带“点赞/点踩”反馈区，而且只有落到终态（`flowStatus=3`）才会出现在会话里，通知不需要这两点。

使用自定义模板时，模板需要标题字段 `title`、正文字段 `markdown`、可选的 `tips`，以及写在 `sys_full_json_obj` 里的按钮字段 `msgButtons`。标题走内置模板自带的头部槽位——这是该模板唯一比正文更大的字号（markdown 的 `#` 标题不比它大，`---` / `***` 也不会渲染成分割线，所以标题与正文之间不画线）。

卡片内容与 Markdown 通知同源：标题是动作本身（如“✅ 你的任务已完成”“🔔 hajimi 在评论中提到了你”），事项标题放在正文，避免长标题把正文挤成两行；正文里每条事实的标签加粗（`**任务编号**：WORK-582`），评论内容、任务结果、失败原因或取消原因作为引用块另起一段。卡片底部提供“在 Wework 打开”和“在浏览器打开”两个按钮。可引用回复的本地运行任务通知仍为文本消息，以便直接引用继续任务。
