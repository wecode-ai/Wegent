...
卡片正文使用钉钉的 Markdown 渲染，能力与网页端不同：钉钉不支持 GFM 表格，表格源码会以竖线原文逐行展示。后端在写入卡片前（流式更新与最终回复同一路径）会把完整的表格块转换为等宽对齐的纯文本表格，放在代码块中展示；流式输出中分隔行尚未到齐的表格保持原文，待完整后再转换；原文代码块内的表格不重复转换。标题、加粗、列表、引用、链接等其余语法原样透传。

卡片的配色与布局由钉钉客户端和卡片模板决定。卡片宽度由模板根节点的尺寸配置控制：在钉钉卡片搭建平台打开模板，选中根卡片节点，将「卡片宽度模式」（模板协议字段 `cardSizeWidthMode`）设为「自适应」（`adaptive`）后重新发布，卡片即可按聊天区域等比铺满，不再固定宽度留白。钉钉官方的 AI 回复模板（如「智能回复」）即使用该配置。创建卡片实例的接口没有宽度参数，服务端无法按消息调整；未配置 `chat_card` 的通道使用钉钉内置 AI 卡片模板，其宽度不可修改，需要自适应宽度时改用自定义模板。
