...
3. 在实际钉钉客户端检查 @提及、被分配和各执行状态，以及两个操作入口。Wework 入口使用 `wework://` 链接，需在目标客户端验证唤起行为。
...
| `secondaryUrl` | Wework 桌面深链 |

`tone` 由通知类型与状态决定：提及、分配和开始执行为 `blue`；等待审批或设备为 `orange`；完成为 `green`；失败为 `red`；取消为 `gray`。卡片标题描述动作，任务标题与编号、上下文和可选详情分开显示。可引用回复的本地运行任务通知仍为文本消息，以便直接引用继续任务。
