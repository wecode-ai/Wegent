...
同一个钉钉通道可以把站内通知改用 AI 卡片推送：勾选“任务通知以 AI 卡片发送”后，@提及、被分配、执行完成/失败/取消这类通知会以卡片形式发出；不勾选或卡片发送失败时仍发送原来的 Markdown 通知。应用内通知和站内记录不受影响。

模板默认为钉钉内置的 AI 卡片模板，不需要自建卡片。使用自定义模板时，模板需包含标题、正文和按钮字段（`msgTitle`、`msgContent`、`staticMsgContent`、`msgButtons`），并把正文字段写进 `sys_full_json_obj.order`，否则钉钉不会渲染正文。

主动推送的卡片只有落到终态（`flowStatus=3`）才会出现在会话里，因此通知卡片一律是“已完成”形态，不会自己从“执行中”变更为结果。若需要卡片原地推进，须改用流式接口并额外开通权限。

卡片内容与 Markdown 通知同源：标题是动作本身（如“✅ 你的任务已完成”“🔔 hajimi 在评论中提到了你”），事项标题放在正文，避免长标题把标题栏挤成两行；正文里每条事实的标签加粗（`**任务编号**：WORK-582`），评论内容、任务结果、失败原因或取消原因作为引用块另起一段。卡片底部提供“在 Wework 打开”和“在浏览器打开”两个按钮。可引用回复的本地运行任务通知仍为文本消息，以便直接引用继续任务。

配置存放于通道的 `config.notification_card`，例如：
...
通过 API 关闭时发送 `{"notification_card": null}`；省略该字段代表保持现有配置。
