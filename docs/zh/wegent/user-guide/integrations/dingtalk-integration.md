...
| **会话设置卡片模板 ID** | 模型、设备、智能体和任务的可点击选择卡片（可选） | `xxxxxxxx.schema`      |
...
配置 **会话设置卡片模板 ID** 后，用户可直接发送“设置”打开操作台，也可发送“切模型”、“切设备”、“切智能体”或在私聊中发送“切任务”直达对应选项。原有 `/status`、`/models`、`/devices`、`/agents`、`/switch` 命令继续保留；带参数的命令仍按文本方式执行。卡片创建失败时也会自动使用原文本命令响应。

操作台会展示当前模型、当前设备、私聊绑定任务、当前 Task 智能体和下一新任务智能体：
...
- 卡片只携带 15 分钟有效的随机选项令牌，不包含模型、设备、智能体或任务的内部 ID；重复点击只会执行一次。
