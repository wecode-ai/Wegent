...
当 Wework 云看板任务发生关键变化时，Wegent 会向相关人员最近使用的 IM
私聊发送通知。关键变化包括任务创建、归档、负责人、状态、优先级、截止时间、
新评论以及 AI 执行完成或失败。标题、描述、标签和纯看板排序变化不会触发通知。

相关人员包括操作人、任务创建人、变更前后的人员负责人、任务协作人，以及机器人
负责人的创建者。每位用户只会收到一条通知；如果用户没有可用的 IM 私聊会话，
系统会跳过该用户，并且不会影响任务操作。

GitHub 和 GitLab 任务支持从 Wework 发起的变化通知。钉钉 AI 表格目前只能可靠
识别项目创建人和当前操作人，因此仅通知这两类用户。直接在外部平台完成的修改
暂不触发通知。
