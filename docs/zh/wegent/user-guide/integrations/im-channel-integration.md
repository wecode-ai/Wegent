...
通道的 **默认模型** 是可选的管理级覆盖：每收到一条消息，系统按以下优先级决定使用的模型：

1. 用户在对话中的**显式模型选择**
2. 通道配置的**默认模型**
3. 机器人自身绑定的模型（modelRef / 绑定模型）

当没有用户显式选择时，通道默认模型会以强覆盖方式应用到任务上；此时如果机器人随后升级了自己的绑定模型，也不会影响渠道默认模型已覆盖的任务。

为避免旧配置把会话永久钉在某个已下线或不合适的模型上，系统提供以下兜底：

- 渠道默认模型若已下线或无法解析，任务会自动降级到机器人自身的绑定模型，而不是继续报错。
- 会话续聊时会重新解析当前模型：渠道默认模型被清空或修改后，该会话的下一条消息会自动跟随最新配置，不再沿用旧的覆盖标记。

因此，希望 IM 会话使用机器人自身模型时，请在管理后台将该通道的 **默认模型** 清空（选择“使用智能体/运行时默认模型”）或改为机器人绑定的模型。新会话立即生效，已有会话在下次发消息时自动跟随。
