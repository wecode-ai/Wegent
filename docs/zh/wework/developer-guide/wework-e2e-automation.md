...
主桌面流程还覆盖从 Finder 粘贴普通文件和文件夹：输入框必须显示文件与文件夹路径标签，不得创建附件徽标；发送给 Codex 的请求必须包含对应绝对路径，且不得内联文件内容。该场景不创建大文件，本地聚焦排查可运行 `node wework/e2e/desktop/task-flow.e2e.mjs --pasted-workspace-paths-only`。
...
隔离真实 Tauri 验证可通过 `ai:verify paste-paths` 向输入框派发文件路径粘贴事件。`--value` 接收 JSON 数组，每项包含 `uri`、`name`，文件夹项还需设置 `isDirectory: true`：
