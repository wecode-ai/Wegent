...
Tauri 对话统一使用一套消息行虚拟列表，不再根据消息数量切换实现。滚动位置以“视口底部到列表底部的距离”表示；共享 `ResizeObserver` 测量已挂载消息的真实高度。当视口不在底部且下方消息高度变化时，列表以首个已测量的可见消息为锚点补偿滚动位置。渲染范围在可见区前后各保留 2 条消息。已挂载消息及其 Markdown 正文保持完整渲染，不使用 `IntersectionObserver` 卸载消息内容；`IntersectionObserver` 只用于跟随底部状态和附件预览等独立功能。

每个对话只缓存“消息 ID → 已测量高度”的有界记录，并与距底部滚动快照一起恢复。修改这套逻辑时，应覆盖短对话、长对话、流式增高、历史位置恢复、切换后重开、导航强制挂载和归档缓存淘汰。
