...
每组覆盖随机断开 10% 客户端、3 秒后重连，ACK 延迟/丢失、10 MiB 突发输出、五终端并行、主题切换和隐藏后激活。另测 Backend 重启、Redis 失效、旧 consumer 控制、错误握手及已有会话切版本被拒绝；确认会话身份和输出标记，不能只凭界面恢复判断无损。结束后清理测试会话和隔离资源。
...
- 同地域输入回显 P95 ≤ 50ms，扣除网络 RTT 后链路 P95 ≤ 20ms；连续 `yes` 后 Ctrl-C 停止 P95 ≤ 200ms。
