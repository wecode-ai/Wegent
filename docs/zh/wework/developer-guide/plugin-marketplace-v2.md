...
Wework 的“发布到市场”不要求用户手工选择 ZIP。本机插件由 Tauri 根据 Marketplace 和插件键定位、打包；云端插件由当前 Task 的 Executor 对工作区源码打包。两条路径都校验 `.codex-plugin/plugin.json`、符号链接、越界路径和 50 MB 压缩包上限，并复用现有投稿、对象存储、扫描、ACL 与审核接口；单 Skill Plugin 以 `listing_type=skill` 投稿。
