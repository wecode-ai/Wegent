...
`feature/plugin-account-auth` 已实现 Backend、原生 Executor、后台账号连接服务、
Python SDK 0.7.0 和邮箱插件适配。本地真实后端、Electron 与云端执行器已验证
密码复用，以及合成 OAuth 服务的授权、刷新和撤销闭环。代码尚未提交、发布或部署；
DWS 原生适配器、可恢复交接、公共 CLI 委派和五平台安装包已有实现与本地验证；
公共仓库独立构建 CI、完整制品发布及初始化入口已接通并通过只读校验，实际发布、
真实服务方 OAuth 验收和 Windows 实机验证尚未完成。
