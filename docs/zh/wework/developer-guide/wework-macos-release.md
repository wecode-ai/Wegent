...
发布脚本依次上传版本化 DMG/ZIP、各类不可变组件包、Electron YAML 和组件滚动
清单，最后从公开下载地址逐项验证。相同版本但清单不完整时会补齐；远端存在不完整
的更高版本时直接失败。DMG 是压缩产物，约 350 MB 的 DMG 解包后应用目录约
700 MB 属于同一套完整离线运行环境，不代表重复打包。
