...
- 内网 Nevis 云设备；
- 自托管 Remote Docker 设备。

客户端只能获得 Backend 生成的短时 WebSocket 地址，不能获得 Nevis 地址、sandbox ID、`NEVIS_MANAGER_ID`、`NEVIS_SIGNATURE`、Runtime token 或长期用户 JWT。
...
4. Redis 只保存会话授权元数据和一次性 ticket 摘要，不保存 Nevis signature。
...
`NEVIS_SIGNATURE` 只在 WebSocket 建连时从 Backend 配置即时读取并放入上游 `X-Signature`，不进入 REST 响应、Redis 会话记录、浏览器日志或监控标签。

## 云设备能力

UI 只在设备在线且返回以下能力时显示桌面入口：
...
Nevis 云设备的能力由 Backend 根据配置、sandbox ID 和在线状态投影。Remote Docker 设备由 Executor 完成真实 RFB 握手探测后上报，不能只根据环境变量宣称可用。

当前只声明基础 `text` 剪贴板。只有真实 Nevis/TigerVNC 双向 UTF-8 扩展协议验证通过后，才能改为 `extended-text`。
...
- H.264 必须显式打开；当前默认关闭，避免在未验证 Nevis encoder 和 Chromium WebCodecs 组合前改变编码路径。
...
Managed device image 安装 TigerVNC、XFCE、xclip，并使用：
...
- `DEVICE_VNC_RFB_ADDR` 在 managed image 中固定为 `127.0.0.1:5901`。
- TigerVNC 使用 `-localhost yes`，容器不发布 5901。
- 外部只访问 token-gated Device Session Gateway。
- Executor 必须完成 RFB 版本、安全类型和 ServerInit 握手后才上报 capability。
...
3. Redis 会话中不存在 Nevis signature。
...
9. Remote Docker 的 RFB 只能通过 Session Gateway 访问。
10. 剪贴板 lease 在多个设备和标签页之间不串用。
