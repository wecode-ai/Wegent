...
```mermaid
flowchart LR
  A[Wework] -->|POST /api/devices/device-id/vnc| B[Backend VNC Session Service]
...
- 其他使用此 API 的 Web 客户端需配置精确的 origin。
...
UI 只为明确报告 `desktop.available: true` 的云设备提供桌面入口，设备离线时禁用。未注册 provider 的部署不会报告该能力，因此 Wework 隐藏入口。正常情况下 Backend 返回：
