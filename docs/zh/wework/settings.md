...
- **运行工具**：检测并配置可从桌面工作台直接启动的 OpenCode 和 Claude Code。
...
桌面端可以把已安装的 OpenCode 或 Claude Code 作为本地运行工具直接启动。在
**设置 → 连接 → 运行工具** 中可以：
...
在本地项目或开发工作树中输入任务后，打开输入区的运行工具选择器即可启动工具。OpenCode 使用
`--prompt` 传递完整提示词，Claude Code 使用位置参数传递提示词。进程在当前工作区中通过本地
PTY 运行，终端显示在工作台中央，并支持键盘输入和窗口尺寸同步。运行中的会话也会列在左侧任务
