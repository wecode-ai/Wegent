...
从顶部标签栏的 **+ → 智能应用** 可直接进入 **应用 → 智能应用**；也可以先打开 **应用**，再从 **站点 / 小程序 / 智能应用** 中切换。智能应用下分为 **市场**、**已安装** 和 **我的创建**：市场用于发现和安装官方应用及定向分享给自己的应用；已安装用于绑定模型、运行和卸载本机应用；我的创建用于创建、导入和发布应用。

在 **我的创建** 点击 **创建智能应用**，会打开新对话并引用随 Wework 安装的“智能应用开发助手”插件；该插件负责检查 DSH 开发环境、检索并组合 DSH 插件、在 Wework 内置浏览器中验证本地 profile，以及校验和打包 ZIP。已有安装包时，点击 **导入应用** 选择 ZIP，Wework 校验后直接导入本机并显示在创建列表中；校验失败时不会写入本地应用目录。列表中的 **发布** 用于补充市场信息并选择分享的成员或部门。市场下载的应用仍会通过 Wework 原生预览界面确认版本依赖和绑定模型。

从市场安装时需要选择一个 Wework 模型；从“我的创建”导入的应用可以稍后在 **已安装** 中选择模型。模型只绑定到当前智能应用，其他智能应用可以分别选择不同模型。应用会显示版本、绑定模型和 **已安装 / 运行中 / 启动失败** 状态；应用停止时可以直接修改绑定模型。点击 **打开** 后，Wework 启动隔离的 Harness 实例，并播放应用进入顶部标签栏的动效，再把智能应用作为独立工作区标签页打开；点击 **停止** 会关闭对应标签页并回收实例。打开 **常驻** 后，Wework 每次启动主窗口时都会自动启动该应用并打开它的标签页。
