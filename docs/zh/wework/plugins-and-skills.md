...
在 **我的创建** 点击 **创建智能工作台**，填写名称和保存位置后，Wework 会基于 DSH Web 预设生成一份合法的工作台目录，将它直接关联到本机列表，再打开引用“智能工作台开发助手”的新对话。工作台不是一次性生成物：停止运行后，可从更多操作中随时选择 **添加 DSH 插件** 或 **开发工作台**，开发助手会读取同一目录中的 manifest、依赖、源码与 `cordis.patch.yml` 并增量修改。**在 Finder 中打开** 可直接编辑目录；启动和刷新列表时 Wework 会重新校验目录内容。
