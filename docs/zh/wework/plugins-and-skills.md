...
### 安装 DeepSeek Harness 能力

在 **插件 → 管理插件 → Harness 能力** 分区中点击 **导入安装包**，或把 ZIP 安装包拖到页面。该分区沿用插件中心的侧边栏和顶部返回入口，可以随时切回 **插件** 分区或返回插件市场。Wework 会在安装弹窗中展示能力名称、版本、说明，以及所需的 DeepSeek Harness 和 Node.js 版本；校验失败时会直接显示安装包问题，不会写入本地能力目录。

安装前需要选择一个 Wework 模型。这个模型只绑定到当前能力，其他 Harness 能力可以分别选择不同模型。安装完成后，能力会以卡片形式显示版本、绑定模型和 **已安装 / 运行中 / 启动失败** 状态。点击 **打开** 后，Wework 启动隔离的 Harness 实例，并把它作为独立工作区标签页打开；点击 **停止** 会关闭对应标签页并回收实例。
