...
sequenceDiagram
    ...
    U->>R: 加载执行 transcript
    alt 任务仍在运行
        P-->>R: 回合结束时先持久化已完成消息，再清理活动回合
        R-->>U: 立即返回已完成回合缓存 + 当前实时回合并继续推送事件
