...
16. 重启前是否正在执行只能由独立的持久执行证据判断，不能从 Runtime Task 元数据、Worktree 路径、非归档状态、Provider 历史线程或缺失状态字段推导。`worktrees.json` 中未完成的 `preparing` 记录可以证明 Worktree 准备被中断；持久队列只能证明尚未运行。没有执行证据时，重启对账不得写任务失败、完成时间或活动时间。
