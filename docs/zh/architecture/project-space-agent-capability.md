...
flowchart LR
    ...
    GATEWAY -->|钉钉项目读取| AITABLE_PROVIDER[AI Table Provider<br/>唯一记录 + 主文档]
    AITABLE_PROVIDER --> BUNDLED_DWS[Wework 内置 DWS<br/>隔离登录态]
    GATEWAY -. Provider 读取失败时<br/>返回内置 DWS 绝对地址 .-> SESSION
...
sequenceDiagram
    ...
    P->>G: 调用统一工具契约
    alt 钉钉 AI 表格 Issue
        G->>L: 按 ContextGrant 的 item_id 读取唯一记录
        L->>L: 使用 Wework 内置 DWS 和隔离登录态读取记录与主文档
        alt Provider 读取成功
            L-->>G: 返回全部字段与 primary_document
        else Provider 读取失败
            L-->>G: 返回错误
            G-->>H: 返回 bundled_dws_fallback 的绝对地址、绑定标识和进程级环境
            H->>H: 仅按 fallback 调用内置 DWS
        end
    else 本地项目
...
| 钉钉 Issue → 原生记录与主文档读取     | Executor Project-space Gateway、AI Table Provider、Wework 内置 DWS |
