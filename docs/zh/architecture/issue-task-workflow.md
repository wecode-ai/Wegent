...
flowchart LR
    ...
    MODE -->|AI 调度| AI[AI 管家读取 Issue、提示词与阶段定义]
    ...
    PLAN_REVIEW -->|驳回| AI
    APPROVAL -->|否| MATERIALIZE
    MATERIALIZE --> CHILD[(父 Issue 下的具体 LoopItem)]
    CHILD --> BINDING
    FREE --> BINDING
    STAGE --> BINDING
...
- AI 编排卡片只投影当前 automation run 对应的最新一条管家动态；后续继续对话或重规划产生的新动态必须替换旧动态成为运行详情入口。确认、暂停、恢复和重规划按钮只有在当前客户端具备对应 API 方法时才可操作，不得展示可点击但必然失败的动作。
...
- automation run 只能提交其 `created_by_user_id` 所属用户的方案；归属不匹配必须在写入方案审计关联前失败并整体回滚。
- 方案读取与界面投影不得修复数据、提交事务或发布网络消息。旧记录修复必须通过显式写路径执行，并由调用方决定事务边界。
