...
flowchart LR
    ...
    RUNTIME[Wework 管理的单文件 DSH Runtime 归档] --> SIGN[Node JIT 权限签名与 Isolate 启动校验]
    SIGN --> EXTRACT[(按内容指纹解包)]
    EXTRACT --> INSTANCE
...
| Runtime 归档、Node JIT 签名、内容指纹和构建准备 | `wework/scripts/prepare-deepseek-harness-runtime.mjs`、`wework/scripts/deepseek-harness-node.entitlements.plist`、`wework/src-tauri/tauri.conf.json` |
