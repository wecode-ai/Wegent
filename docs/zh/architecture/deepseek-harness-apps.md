...
flowchart LR
    ...
    OFFICIAL[官方发布脚本] --> CATALOG[(云端智能应用目录)]
    USER[用户发布] --> SUBMISSION[上传与安全扫描]
    SUBMISSION --> ACL[成员 / 部门授权]
    ACL --> CATALOG
    CATALOG --> MARKET
    CATALOG --> DOWNLOAD[短时下载地址]
    DOWNLOAD --> ZIP
    MARKET --> ZIP
    INSTALLED --> ZIP
...
sequenceDiagram
    ...
    opt 发布与定向分享
        U->>M: 选择本地 ZIP 并发布
        M->>API: 上传包、市场资料和成员 / 部门
        API->>API: 校验、扫描并创建不可变版本
        API-->>M: 发布成功
    end
    ...
    alt 创建智能应用
        ...
        U->>M: 从市场下载或导入本地安装包
        M->>API: 请求有权限版本的短时下载地址
        API-->>M: URL + SHA-256 + 大小
    end
    M->>UI: 打开已安装智能应用
    U->>UI: 选择 ZIP 和模型
...
| 云端目录、发布版本、扫描、ACL 与下载授权                           | `backend/app/api/endpoints/smart_apps.py`、`backend/app/services/smart_app_marketplace_service.py`                                                         |
| 云端目录关系模型、版本化扩展 JSON 与数据库约束                    | `backend/app/models/smart_app_marketplace.py`、`backend/alembic/versions/20260820_f82c5d1a9e37_add_smart_app_marketplace.py`                               |
| 官方智能应用确定性发布                                             | `backend/scripts/publish_official_smart_app.py`                                                                                                             |
...
云端分发不变量：云端目录只负责发布与下载授权；官方脚本发布的应用对全部已登录用户可见，用户发布必须绑定至少一个成员或部门且仅发布者和授权对象可见；发布版本不可变，同版本不得覆盖；应用级和版本级扩展信息使用最大 64 KiB、带 `schemaVersion` 且按组织命名空间隔离的 JSON 对象，应用新版本未提供扩展值时必须保留已有未知字段，版本扩展随不可变版本冻结；所有者、权限、状态、版本、哈希以及需要索引、排序、唯一约束或鉴权的字段必须保持关系列，扩展 JSON 不得保存凭据或替代安全边界；版本和上传记录通过 `smart_app_id` 与应用建立逻辑关联，不创建数据库外键，写入、读取和清理流程必须由应用层校验目标应用存在并按顺序处理关联记录；下载后的安装、模型、常驻、运行数据和离线启动完全归设备本地所有，撤销云端授权只能阻止后续发现、下载和更新，禁止远程停止或卸载本地副本。
