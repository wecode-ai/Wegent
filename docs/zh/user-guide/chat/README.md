...
在输入框的“更多”菜单中，可以通过 **外站内容** 添加外部网页 URL。系统会调用内置 spider 服务抓取网页中的可分析内容，并作为一个外站内容上下文加入当前对话。

当前支持的外站内容包括：

- **标题和正文**：抓取结果中的标题、描述和正文会作为文本上下文注入模型。
- **视频**：抓取到的视频会作为外站内容的内部视频素材。若当前模型支持视频理解，发送消息时会把视频内容注入模型；否则仅注入视频 metadata。
- **多图**：抓取到的多张图片会作为外站内容的内部图片素材，发送消息时以多张图片输入传给支持图片理解的模型。
- **首屏评论**：抓取到的首屏评论会作为外站内容的内部文本素材，发送消息时作为普通文本上下文注入模型。

外站内容会以一个 badge 显示在输入框和消息上下文中。发送消息、重新生成、修改原消息、pipeline 下一步和导出时，系统会保留这个外站内容上下文；删除输入框中的外站内容 badge 后，该内容不会进入本次对话。
