...
微博私信的 Task 模式使用 Wework 本地运行时任务，不创建中心数据库 Task。您可以先在 Wework 本地任务中点击“继续到私聊”，选择微博私聊后继续发送消息；也可以在微博私信中发送 `/new`，选择 `2. 新建 Task` 后按提示选择项目，或回复 `0` 创建不关联项目的本地运行时任务。不关联项目时，系统会使用当前选择的本地设备；如果没有选择设备且只有一个在线本地设备，会自动使用该设备。直接使用 `/task` 或 `/switch` 继续任务时，如果尚未绑定本地运行时任务，系统会提示回到 Wework 选择。
