...
私聊机器人会自动加载 IM 会话控制能力。用户可以直接用自然语言表达会话控制意图，而不需要记住命令。例如：

| 用户说法 | 行为 |
|----------|------|
| “现在连的是哪个任务？” | 查看当前私聊绑定的 Chat、Task 或 Wework 本地任务 |
| “不要接着刚才那个了” | 清除当前私聊目标，后续消息从新的上下文开始 |
| “重新开始” | 开始新的私聊会话 |
| “确认” / “取消” | 确认或取消上一步需要确认的会话控制操作 |

当清除操作会断开 Wework 本地任务时，机器人会先要求用户确认，避免误断开正在继续的本地任务。
...
创建或编辑 IM 通道时，需要选择机器人用途：

| 用途 | 说明 |
|------|------|
| **Wegent Chat** | 面向普通 Wegent 智能体对话，适合日常问答、Chat 任务和普通多轮对话 |
| **Wework Local** | 面向 Wework 本地任务续接，适合从 Wework 中选择本地运行时任务后在 IM 私聊中继续 |

一个 IM 平台可以配置两个机器人，分别承担这两类用途。Wework 中“继续到私聊”的会话列表只显示 **Wework Local** 用途的私聊会话，避免用户误选普通聊天机器人。
