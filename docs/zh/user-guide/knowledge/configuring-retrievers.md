...
创建知识库时，**高级设置** 中的 **RAG 配置** 提供三种模式：

| 模式 | 行为 | 适用场景 |
|------|------|----------|
| 自动配置 | 系统自动选择可用的检索器和 Embedding 模型，并保留您调整过的检索参数 | 推荐默认选项 |
| 手动选择 | 必须明确选择检索器和 Embedding 模型，缺少任一项时不能创建 | 需要固定存储后端或模型时 |
| 不启用 | 创建无 RAG 知识库，不写入检索配置 | 没有向量库、小型知识库或测试场景 |

无 RAG 模式的创建步骤：

1. **创建知识库**：在创建对话框中打开高级设置，将 **RAG 配置** 设为 **不启用**
