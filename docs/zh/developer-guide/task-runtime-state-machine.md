...
- 异常路径：关键事件缺失、取消 ack 丢失、断线重连或本地状态和服务端状态不一致。状态机进入非稳态后，内部 `RuntimeStabilityProbe` 使用统一 grace window 延迟触发 `checkHealth(reason)`，通过 `runtime-check` 拉取服务端 task status 和 active stream checkpoint，再回到稳定状态。
...
这个延迟只触发状态机健康检查，不直接在 UI 或 socket 层判定成功失败。页面可见、WebSocket 重连、queued message 卡住、手动刷新和网络恢复等外部触发仍然直接调用健康检查，不再维护额外的延迟分支。
