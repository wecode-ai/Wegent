...
- task detail snapshot 只同步生命周期字段；即使响应里包含 `subtasks`，也不能从 REST detail 恢复或展示消息。
...
- 如果状态机正在 `waiting_socket` 等待 socket recovery，terminal task detail 只能更新 runtime 生命周期，不能把状态机切到 `ready`；消息仍必须等待 socket `task:join` 同步。
