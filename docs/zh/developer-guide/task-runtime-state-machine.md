...
- 取消请求可能早于 Codex app-server 创建 turn 到达。executor 必须在发送 `turn/start` 前消费该信号，避免已取消的任务继续启动。
- 如果 `turn/interrupt` 返回“没有活跃 turn”，说明该 turn 已在取消与中断之间结束；executor 必须将其作为幂等的取消结果收敛为 `cancelled`，不能向用户报告生成失败。
