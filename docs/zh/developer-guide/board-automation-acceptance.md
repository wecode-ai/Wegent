...
## 2026-09-10 AI 按需读取上下文

启动指令保留本次任务要求、工单和阶段标识、执行配置、工作空间继承地址，以及已有交付物目录。动态、历史结论、交付物正文均留在原存储，入队、绑定与下发时不读取、不冻结、不复制到 `execution_payload`。同一阶段输入重复展开为来源、提示词、附加上下文的旧路径已删除；不扩数据库字段、不截断业务内容。

```mermaid
sequenceDiagram
  participant C as 调度员
  participant Q as 后端执行队列
  participant R as 目标机器上的 AI
  participant B as 看板工具
  C->>Q: 本次指令、工单/阶段 ID、执行配置
  Q->>Q: 查询本工单已提交交付物，提取目录元数据
  Q->>R: 精简启动请求、工作空间地址、交付物目录
  R->>B: get_board_item / list_board_item_comments
  B-->>R: 当前工单、交办、分页动态与楼层上下文
  R->>B: 按目录 ID 调用 read_delivery，按需读取要求或文件
  B-->>R: 请求的业务内容
  R->>B: 写回工作结果和交付物
```

`list_board_item_comments` 按工单权限读取动态，支持从最新页向前分页，并复用楼层上下文补齐逻辑；不读取其他工单动态、不触发运行状态修复或自动分派。`get_workflow_stage_context` 在 AI 调用时读取当前上下文，不再写入绑定快照。阶段启动前仍解析 workspace_policy=inherit 所需的前驱任务地址，并保留已有执行目标校验。

目录包含 delivery_id、requirement_id、名称、类型、所属阶段、提交时间和 superseded_by_delivery_id；不包含正文、完整 fulfillment、文件内容或下载地址。同阶段同要求以最新提交标记替代关系，一份交付物中的不同要求分别判断；无明确阶段或要求的交付物不推断替代关系。目录覆盖本工单此前已提交的成果，包括回退后当前阶段的历史成果，适应 AI 跳过或回退节点；不按图连线丢弃可用成果。草稿、已删除及其他工单的交付物不进入目录。来源任务绑定提供阶段归属，任务解绑不抹去已提交成果的来源；无绑定的成果可由阶段交付物链接确定归属。AI 通过 list_deliveries 查询启动后的新增成果，正文继续用 read_delivery 按需读取，不新增表或迁移。

回归源码覆盖：目录元数据与部分替代、跨阶段要求隔离、跨工单/草稿/删除过滤、来源绑定恢复、交付物正文缺失时仍能启动、超长工单及交付物正文不进入启动包、启动不调用正文读取器、原任务绑定不保存快照、动态分页与权限隔离、工作空间继承、CI event-center 通过真实 MCP 读取工单/动态/交付物。按用户要求未运行测试或应用验证；提交后同步 Test-Wegent，由用户部署验收。

## 2026-09-09 工作结果与调度状态独立

同一个任务重跑时，当前结果以最新一次运行状态为准。AI 执行阶段在任务重跑成功后显示完成，并清除旧的失败或取消提示；不再额外转成待审批、待验收或待交付物。历史交办结果仍保留，不能据此否定新一次任务结果。多个绑定按现有绑定顺序取最新任务，不用旧任务的运行状态覆盖它。

工作结果更新不修改交办、工单状态或自动调度开关，不释放后续节点。用户手动停止后，即使重跑成功，调度仍暂停；只有点击“恢复推进”才恢复调度。正在分派新交办时不让旧任务结果提前完成新交办；交给人处理的阶段仍由人工明确继续。已有任务成功但阶段仍失败的快照会在显示时校正，恢复推进时也把最新工作结果写入供调度员读取。

```mermaid
flowchart TD
  S[用户停止任务] --> P[自动调度暂停]
  S --> H[保留历史取消记录]
  R[用户重跑同一个任务] --> T[更新最新任务状态]
  T -->|运行中| N[阶段执行中，清除旧错误]
  T -->|成功| D[阶段完成，清除旧错误]
  T -->|失败| F[阶段失败]
  N --> P
  D --> P
  F --> P
  P -->|用户点击恢复推进| C[调度员读取最新结果，决定下一步]
```

回归覆盖：取消后同任务重跑成功、成功后再运行失败、旧错误清除、最新绑定优先、人工控制保留、暂停期间不新增调度任务、从界面显式恢复。已更新前后端用例和 CI event-center 场景源码。按用户要求未运行测试或 Electron 验证，由 Test-Wegent 验收。恢复按钮仍在阶段图上方，复用现有接口。

## 2026-09-09 同一件事在原评论下盖楼

本节替代此前的顶部独立人工回复区：AI 提问作为主楼，用户补充和后续 AI 追问按时间留在楼中楼。卡片、即时提醒和收件箱定位到本次交办所在的讨论及回复框，不再另开表单。同一目标即使任务已经结束也继续原讨论；新增可独立执行、独立交付的目标才新建动态和任务。

```mermaid
flowchart TD
  E[卡片或通知] --> Q[定位本次交办的评论楼]
  Q --> R[在楼内填写回复]
  R --> S[回复：保存用户评论，保持人工推进权]
  R --> C[回复并继续推进：保存用户评论并提交交办结果]
  S --> C
  C --> AI[AI 处理同一工单]
  AI --> F[后续追问或结果仍在原楼]
  F --> R
...
问题、回答和交办结果共用既有评论表的 `reply_to_message_id` / `thread_root_message_id`。负责人回复以其本人身份保存，不再伪装成 AI 评论，也不额外新建顶层动态。先回复后继续不重复发同一条评论。评论与交办状态在同一事务提交，提交后才推送；事务回滚不显示虚假楼层。分页加载同时补齐主楼及当前待回复问题，避免楼层较多时入口消失。

验收重点：从通知或卡片进入原楼；补充多次仍保持人工控制；再次追问继续同一楼；已发送文字不重新填回输入框；未发送文字关闭后恢复；旧通知不能向新交办提交；其他评论及任务会话可独立回复。已同步更新回归用例与 CI event-center 场景，按用户要求未运行测试或应用验证，交由 Test-Wegent 验收。

## 2026-09-09 人工回复入口与交办通知

工单详情和动态弹窗顶部共用人工回复区，明确展示负责人、本次待处理事项和输入框。看板卡片、交办动态下的“去回复”和通知均可到达该区域。普通评论可以直接回复，不要求存在 AI 任务；回复实际任务仍继续原会话。

```mermaid
sequenceDiagram
  participant AI as AI / 顺序流程
  participant API as 工单服务
  participant DB as 既有工单、评论、通知表
  participant UI as 卡片 / 收件箱 / 即时与系统通知
  participant H as 负责人
  AI->>API: 交给人处理
  API->>DB: 同一事务保存交办、未读状态与通知
  DB-->>UI: 提交后投递，链接带交办编号
  H->>UI: 点击去回复
  UI->>API: 加载工单并核对交办编号
  API-->>UI: 当前交办 / 已处理或失效
  H->>API: 保存回复
  API->>DB: 保存草稿和普通评论，保持等待人工
  H->>API: 继续推进
  API->>DB: 校验负责人及交办编号，提交结果
  API->>AI: 恢复工单推进
```

复用已有工作流 JSON 存储回复草稿，不新增表。只有当前负责人可保存和提交；输入、保存、普通评论、独立任务完成均不释放人工推进权。显式暂停时可保存回复，恢复后才可继续推进。旧通知不可提交到新交办。输入草稿按用户、工单和交办隔离，保存后可跨设备读取。

待用户验收：

| 场景 | 操作与预期 |
| --- | --- |
| 交给人工 | 负责人收到收件箱未读通知和即时提醒；系统允许通知时显示系统通知；卡片显示待谁回复 |
| 三个入口 | 点击卡片、交办动态、通知的“去回复”，均显示顶部人工回复区 |
| 保存与关闭 | 输入并保存，评论留痕；关闭重开恢复内容，工单仍等待人工，无新调度任务 |
| 明确提交 | 点击“继续推进”后才提交结果并恢复 AI |
| 暂停与权限 | 暂停时继续推进禁用；非负责人只读；后端拒绝越权及旧交办提交 |
| 旧通知与失败 | 旧通知提示已处理或失效；请求失败保留文字，可重试；重复保存不重复评论 |
| 普通动态 | 无关联任务的评论可以回复，不再报“原任务不可用”，也不启动新任务 |

已补充后端与界面回归用例，并扩展 CI 已覆盖的 event-center 桌面场景。本次遵照用户要求不执行测试、构建或 Electron 验证，交由用户在 Test-Wegent 验收；以下历史报告不代表本次改动已验证。
...
## 2026-09-09 任务对话停止后禁止重试

现场日志显示 WebSocket `runtime.tasks.cancel` 在 21:40:11 返回成功，21:40:14 又创建执行 529，21:40:15 创建调度执行 530。桌面 WebSocket 转发绕过了 HTTP 接口中的人工停止记录，中断被当作普通失败而重试。

```mermaid
flowchart LR
  A[HTTP / WebSocket 用户停止] --> B[按所有者与设备身份查找执行]
  B --> C[保存取消请求和工单暂停]
  C --> D[提交事务]
  D --> E[发送 Runtime 停止指令]
  E --> F[终态回调仅收尾，不重试或再分派]
