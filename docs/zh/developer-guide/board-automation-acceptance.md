...
启动指令只保留本次任务要求、工单和阶段标识、执行配置与工作空间继承地址。动态、历史结论、交付物正文均留在原存储，入队、绑定与下发时不读取、不冻结、不复制到 `execution_payload`。同一阶段输入重复展开为来源、提示词、附加上下文的旧路径已删除；不扩数据库字段、不截断业务内容。
...
回归源码覆盖：超长工单正文不进入启动包、启动不调用正文读取器、原任务绑定不保存快照、动态分页与权限隔离、工作空间继承、CI event-center 通过真实 MCP 读取工单/动态/交付物。按用户要求未运行测试或应用验证；提交后同步 Test-Wegent，由用户部署验收。
