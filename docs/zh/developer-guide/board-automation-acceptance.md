...
## 2026-09-09 阶段状态与显式恢复推进

AI 推进时，阶段状态以工单调度记录为准。任务状态只用于显示执行详情，不能据此推导人工审批或释放下一阶段；暂停期间同样禁止任务结果推进阶段。顺序流程按 `execution_mode` 区分人工与 AI，只有人工阶段接受审批。恢复入口与阶段图放在同一区域，复用现有恢复接口。

```mermaid
flowchart TD
  T[任务运行状态或交付物更新] --> P{AI 推进或已暂停?}
  P -->|是| K[保留调度记录的阶段状态]
  P -->|否| M{阶段执行方式}
  M -->|人工成功| H[待人工审批]
  M -->|AI 成功| D[按交付物判断完成]
  S[用户停止] --> W[已暂停：展示恢复推进]
  W -->|用户点击恢复推进| R[现有 resume 接口]
  R --> C[调度员继续处理]
  W -->|新增任务或评论| W
```

回归覆盖：无 automation_rule_id 的 AI 节点不会出现人工审批；显式人工节点保持审批能力；暂停和 AI 工单收到迟到任务结果仍保留阶段状态；恢复入口可见且唯一，失败后保持暂停；CI event-center 场景从界面点击恢复。按用户要求仅更新用例源码，未运行测试或 Electron 验证，由 Test-Wegent 验收。
