...
历史 transcript 可能使用 camelCase `subtaskId` 或 snake_case `subtask_id` 标识 assistant 消息所属的 subtask。Wework 在恢复消息时必须统一识别这两种字段；工具调用块和文件变更块都依赖该标识，不能因传输层命名格式不同而丢弃历史执行记录。
