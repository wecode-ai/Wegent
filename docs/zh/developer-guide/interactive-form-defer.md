...
| `PUBLISH_PRESIGNED_DOWNLOAD_EXPIRE_SECONDS` | `3122064000` | 下载 URL 的有效期，默认 99 年 |

上传 URL 应保持短有效期，降低未使用授权泄露后的风险。下载 URL 需要覆盖发布产物被后续流程读取的时间窗口，因此与上传 URL 使用独立配置；当下载有效期超过 MinIO 客户端默认预签名上限时，Backend 会使用长效签名路径生成 URL。
