...
`wework/e2e/desktop/task-flow.e2e.mjs` 覆盖本机工作区中的真实任务生命周期，并允许产品扩展注入可选桌面场景：
...
8. 如果设置了 `WEWORK_E2E_DESKTOP_SCENARIO_MODULE`，动态加载对应产品场景；公共 runner 不直接包含 VNC fixture 或断言。

`pnpm --filter wework e2e:desktop:wecode` 使用 `wework/wecode/e2e/desktop/task-flow.e2e.mjs` 包装入口，并开启 `WEWORK_E2E_DESKTOP_SCENARIO_ONLY=true`。Wecode 场景通过带 Bearer 认证的真实 HTTP 请求读取云设备 VNC 配置，从设置页和工作区桌面入口打开内置 VNC 页，校验地址栏不含 WebSocket 地址或 token，并完成带 token 的 WebSocket upgrade、RFB 3.8 握手及连接标记断言。RFB 模拟服务、VNC HTTP/WebSocket fixture、诊断状态和具体控制动作均位于 `wework/wecode/`。
...
公共控制器在内置动作未处理命令时，通过 `wework/src/extensions/desktop-control-contract.ts` 委派给产品扩展。Wecode 的 `wework/wecode/extensions/desktop-control.ts` 提供 `evalEmbeddedBrowserJson`、`closeEmbeddedBrowser` 和 `prepareEmbeddedBrowserRelabelRegression`；公共自动化层不识别 VNC。Wecode 桌面场景用通用求值动作读取连接标记和标题，确认 noVNC 只有在真实 RFB 握手完成后才进入已连接状态，并从标题确认 sandbox ID；地址栏是否含凭据由主 WebView 中的独立等待断言校验。
