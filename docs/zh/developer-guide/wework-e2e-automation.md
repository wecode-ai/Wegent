...
`wework/e2e/desktop/task-flow.e2e.mjs` 覆盖本机工作区中的真实任务生命周期和云设备桌面链路：
...
8. 通过带 Bearer 认证的真实 HTTP 请求读取云设备 VNC 配置，从桌面入口打开内置 VNC 页，校验地址栏不含 WebSocket 地址或 token，并完成带 token 的 WebSocket upgrade、RFB 3.8 握手及连接标记断言。
...
控制器的 `getEmbeddedBrowserVncState` 动作会通过原生内置浏览器 eval 读取 VNC 页的连接标记和标题。桌面端链路用它确认 noVNC 只有在真实 RFB 握手完成后才进入已连接状态，并从标题确认 sandbox ID；地址栏是否含凭据由主 WebView 中的独立等待断言校验。
