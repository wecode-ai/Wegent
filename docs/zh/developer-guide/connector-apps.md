...
Connector Apps 让 Wework 在不修改 Codex 源码、也不依赖 ChatGPT 登录的前提下连接组织内部系统。系统把“App”拆成三个独立职责：Wegent Admin 管理定义和认证策略，Wegent Backend 保存加密凭据并代理上游 MCP，Executor 向 Codex 暴露本地 MCP。Wework 不提供 Connector 设置页面，只在连接 Wegent 云端后同步可用能力。

## 运行链路

1. 管理员在 Wegent 的“系统管理 → 应用连接”中配置远程 MCP 地址、认证方式、可见角色和工具白名单。
...
6. `wegent_apps` 子进程从 Executor 私有目录读取自动轮换的短期 JWT，并调用 Wegent Connector Runtime。Backend 解密凭据，并代表用户连接上游 Streamable HTTP 或 SSE MCP。
...
这个设计不使用 Codex 的原生 `codex_apps` Server。原生 Server 的 ChatGPT 认证和远端 App 目录属于 Codex 自身能力；Wegent 使用标准 MCP 配置进入 Codex 流程，因此无需修改或登录 Codex。
