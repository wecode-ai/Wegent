...
- `POST /api/runtime-work/llm-responses/responses`：代理端点。
...
2. 如果模型带有真实 provider 凭证，后端：
   - 生成一个加密的 Fernet 代理 token，token 中包含用户 ID、模型 namespace/name、签发与过期时间，以及唯一的 `jti`。
   - 根据 token `jti` 与主签名密钥派生每个 token 独立的 HMAC 签名密钥。
   - 返回 `base_url` 为后端代理端点，`api_key` 为代理 token，`signing_key` 为 per-token 签名密钥，`codex_responses_compat_proxy: true`。
3. Wework/executor 的 Codex compat proxy 把请求发到该 `base_url`，并携带：
   - `Authorization: Bearer {proxy_token}`
   - `X-Wegent-Request-Nonce: {uuid}`
   - `X-Wegent-Body-Signature: {hex(HMAC-SHA256(signing_key, nonce + body))}`
4. 后端解密 token，校验 nonce 未被使用过，校验请求体 HMAC 签名，解析 Model CRD，取出真实 `base_url`、`api_key`、`model_id`。
...
请求体 HMAC 签名使用主签名密钥。密钥获取优先级：

1. 环境变量 `LLM_PROXY_SIGNING_KEY`。
2. `SystemConfig` 表中 `llm_proxy_signing_key` 配置项；不存在时自动生成并持久化。
3. 无数据库的测试/开发环境使用基于现有设置派生的确定性密钥。

每个 token 的签名密钥通过 `HMAC-SHA256(master_signing_key, token_jti)` 派生，并且只返回给本地 executor。

生产环境无需手动配置 `LLM_PROXY_TOKEN_KEY` 或 `LLM_PROXY_SIGNING_KEY`，后端会在首次使用时自动生成并持久化密钥。

## 安全收益

- Wework 桌面端和本地 executor 永远不会拿到真实 `api_key`。
- 代理 token 与用户和模型 CRD 绑定，并有过期时间。
- token 不再放在 URL 中，因此不会出现在 access log、浏览器历史或代理日志里。
- 每次代理请求必须携带唯一 nonce 和有效的请求体签名；重放捕获到的请求会因为 nonce 已被使用而失败，nonce 会在 Redis 中保存到 token 过期为止。
...
- 后端新增加密的 Codex responses 代理网关，并增加请求签名与重放防护。
