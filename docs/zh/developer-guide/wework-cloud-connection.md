...
打包或本地开发时通过 `VITE_WEGENT_BACKEND_URL` 设置唯一的 Backend 根地址。Wework 从该地址派生 `/api` 和 `/socket.io`，并用它预填“连接云端”；用户仍可修改，本机已有的连接地址优先。配置该变量后，桌面端左下角账户区在未连接时显示“Wegent 账户 / 未登录”，点击后仍打开完整账户菜单；用户从菜单顶部的“登录 Wegent”进入云端授权。连接成功后，账户区显示云端用户名和邮箱；账户菜单中的“退出登录”只会断开云端连接。
...
用户可以输入 Backend 根地址，也可以直接输入 `/api` 地址。前端会把地址归一化为同域的 HTTP API 和 Socket.IO 连接：API 使用 `<Backend>/api`，Socket.IO 使用 `<Backend>/socket.io`。连接时先请求 `/health`，再调用 `/auth/wework/sessions` 创建短生命周期授权会话。Backend 返回完整 `authorize_url`，本地 Wework 在内置授权窗打开该云端授权页，并携带 `poll_token` 轮询会话结果。升级前保存的独立 Socket 域名会在启动时迁移为 Backend 域名。
