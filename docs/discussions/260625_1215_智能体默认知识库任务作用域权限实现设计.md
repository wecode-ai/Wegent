...
| 智能体默认知识库 | `Ghost.spec.defaultKnowledgeBaseRefs` | Bot/Ghost 编辑者或管理员 | 每次执行时从当前智能体配置解析，普通智能体按智能体 owner 权限生效 | 仅搜索，不允许文档级探索过滤 |
...
本方案把“智能体默认知识库”从任务快照中移出，改为执行时按原智能体配置和授权边界动态解析。目标权限语义是：任务使用哪个智能体，就在运行时按那个智能体 owner 的默认知识库能力边界解析；用户没有智能体权限时不能通过任务复制绕过；公共智能体只允许组织知识库；默认知识库只提供搜索，不扩大到文档探索能力。
