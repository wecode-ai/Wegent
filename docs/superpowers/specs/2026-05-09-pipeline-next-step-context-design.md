...
- 用户可以编辑将发送给下一阶段的消息内容
- 用户可以选择要带入下一阶段的上下文
- 有 `final_prompt` 时默认使用 `final_prompt`
- 没有 `final_prompt` 时默认使用最后一条已完成 AI 回复全文
...
默认不勾选完整历史消息。更早历史可以作为“更多历史”候选项展示，用户需要时手动勾选。

### 有 final_prompt 和无 final_prompt 的差异

有 `final_prompt` 时：

- 默认消息内容为 `final_prompt`
- 默认勾选 `final_prompt` 对应输出项
- 完整 AI 回复可选，但不默认勾选

没有 `final_prompt` 时：

- 默认消息内容为最后一条已完成 AI 回复全文
- 默认勾选该 AI 回复全文
...
默认发送消息本身会出现在 textarea 中。勾选文本上下文时，需要避免重复拼接同一段内容。推荐规则是：

- textarea 是最终将发送的主消息
- 勾选文本上下文只用于把额外文本追加到主消息后方
- 默认 AI 输出内容已经写入 textarea 时，不再额外追加
...
- 有 `final_prompt` 时，textarea 默认使用 `final_prompt`
- 没有 `final_prompt` 时，textarea 默认使用最后一条已完成 AI 回复全文
- 没有可用 AI 回复时，确认按钮禁用
- 默认勾选当前阶段相关用户消息、AI 输出项和结构化上下文
...
- 上下文选择区分文本上下文和结构化上下文，避免重复发送同一段默认 AI 输出
