...
Sites API，允许当前用户搜索和查看站点、打开内部或外部 URL、把尚未发布的站点
发布到外网，以及删除站点登记及其公网入口。页面右上角提供“创建”入口，点击后
进入一个全新的标准对话，并自动预选 Sites 能力。
...
- 调用 Sites API 删除站点登记和公网入口，但不删除本地目录。
...
- 删除操作只移除 Sites API 中的站点登记，并关闭对应公网入口。
- 不删除 `path` 指向的本地目录、站点源文件或任务对话。
- 成功响应为 `204 No Content`；Wework 直接从当前列表移除该行并递减总数。
- 删除失败时保留站点行和确认框，在确认框内展示接口错误并允许重试或取消。
- 发布中和删除中的同一行不能并发触发另一项变更操作。
...
- 每行末尾提供“…”操作菜单；菜单中的“删除站点”使用破坏性语义色，并支持键盘
  聚焦、Escape 关闭和点击菜单外关闭。
- 点击“删除站点”先显示应用内确认框。确认文案明确说明将删除站点登记和公网入口，
  但不会删除本地目录；用户确认后才调用删除接口。
...
- `src/api/sites.ts`：外部接口 DTO、查询参数编码、列表、发布和删除方法。
- `src/components/sites/SitesWorkspace.tsx`：列表查询、搜索、分页、删除确认和页面状态
  协调。
- `src/components/sites/SiteListItem.tsx`：单行展示、URL 打开、发布状态和行操作菜单。
...
- `scripts/ai-verify.mjs`：启动隔离 Wework 时把已创建的测试 Codex Home 同时作为
  `CODEX_HOME` 和 `WEGENT_CODEX_HOME` 传入应用，使本地执行器使用同一隔离目录及
  其中的认证链接。
...
- 删除必须经过确认；确认后调用编码过的 `siteid`，204 时只移除目标行。
- 删除失败保留目标行和确认框，展示错误并支持重试；取消确认不调用接口。
- “创建”生成新的空白对话 scope，并只在该 scope 预选 `sites:sites-building`，
  同时覆盖统一技能和本地插件技能两种来源。
- Skill 不可用时不导航，并显示恢复入口。
- `/sites` 路由保持 Workbench 挂载，侧边栏显示正确选中态。
- 中英文命名空间均注册且关键文案存在。
- AI Verify 启动环境同时设置同一个隔离 `CODEX_HOME` 与 `WEGENT_CODEX_HOME`，
  不访问或改写个人运行目录。

### 桌面验证

- 运行 Wework 全量 Vitest、类型检查、ESLint 和 Prettier 检查。
- 通过真实 `http://127.0.0.1:8765` API 验证列表、搜索、发布和删除，不在 E2E 中
  Mock 站点请求。
- 使用 `pnpm --filter wework ai:verify --scenario ...` 验证实际 Tauri 桌面 UI：侧边栏
  入口、站点页面、发布状态、删除确认与删除结果，以及“创建”后的 Sites 已选中并
  能正常发起对话。
...
5. 每行“…”菜单可以删除站点登记和公网入口；必须先确认，且本地目录保持不变。
6. 页面没有分享、访问范围和版本相关入口。
7. 点击“创建”进入全新标准对话，Sites 已选中但没有自动发送消息；在隔离测试运行
   中发送消息不会因 Codex Home 认证路径错误而返回 401。
8. Wegent Backend 没有新增站点识别或自动登记逻辑。
