...
1. 从 Wework 已加载的技能列表中按稳定名称 `sites-building` 解析 Skill；使用返回的
   `namespace` 和 `is_public`，不硬编码命名空间。
2. 建立一个全新的空白对话 scope，保留原任务和未发送草稿。
3. 仅为这个新 scope 设置解析出的 `SkillRef`。
4. 导航到标准对话页 `/`，聚焦输入框。
5. 输入框的已选能力区域显示用户可读名称 `Sites`；不自动填充或发送提示词。
6. 用户发送后，沿用现有 `additional_skills` 请求字段运行 Sites Skill。
...
- “创建”生成新的空白对话 scope，并只在该 scope 预选 `sites-building`。
