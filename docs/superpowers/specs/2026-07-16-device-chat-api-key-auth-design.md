...
2. 否则读取 `Authorization`。Bearer scheme 大小写不敏感；没有 scheme 时沿用
   灵活鉴权依赖既有的原始凭证语义。
3. 解析后的值为 `wg-` 前缀时按个人 API Key 校验，其他值按 JWT 校验。
...
- `backend/app/core/security.py`
  - 集中解析大小写不敏感的 Bearer 或原始凭证。
  - 为灵活鉴权依赖声明 OAuth2 和 API Key OpenAPI security scheme。
...
- 未提供凭证继续返回 `401`。
- 完整 API Key 不写入日志或响应；既有鉴权工具只可能记录截断后的 key 前缀。
...
6. 大小写不同的 Bearer JWT 保持兼容。
7. 灵活鉴权依赖接受的原始 JWT/API Key 会原样传入服务层。
8. OpenAPI 同时声明 OAuth2 和 API Key security scheme。
