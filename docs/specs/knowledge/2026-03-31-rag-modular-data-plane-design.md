...
满足以上条件后，再将 `rag data plane module` 迁移为独立 `rag_service`，风险最低。
...
- 后续独立 `rag_service` 时，无需重新设计核心 contract
...
2. 再形成模块化数据面
3. 先落地 `summary_vector_index`
4. 再落地 `tableRAG`
5. 再迁移 restricted 安全压缩
6. 最后视 contract 稳定度决定是否独立 `rag_service`
