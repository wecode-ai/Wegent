...
待定的是**它当初是有意下线,还是重构时漏掉的**。这决定接下来做什么:

- 如果是有意下线 —— 把 `ShareLinkDialog` 和它在 `permission/components/index.ts` 的导出
  一起删掉,别留着第二份看起来可用其实够不到的实现。
- 如果是漏掉 —— 要决定它挂回哪里。现役的 Desktop / Mobile 两个页面组件各自渲染弹层,
  没有统一的弹层容器了,所以不是把旧代码接回去那么简单。

判断这条不需要读代码,需要的是产品侧确认分享链接这个能力还要不要。
