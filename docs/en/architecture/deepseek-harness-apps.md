...
flowchart LR
    ...
    RUNTIME[Single-file Wework-managed DSH runtime archive] --> SIGN[Node JIT entitlement signing and Isolate startup validation]
    SIGN --> EXTRACT[(Content-addressed extraction)]
    EXTRACT --> INSTANCE
...
| Runtime archive, Node JIT signing, content fingerprint, and build preparation | `wework/scripts/prepare-deepseek-harness-runtime.mjs`, `wework/scripts/deepseek-harness-node.entitlements.plist`, `wework/src-tauri/tauri.conf.json` |
