...
3. Worktree support uses independent `runtime.worktrees.capabilities`. An optional live projection uses `runtime_features.worktrees` and never reuses the Skills, Plugins, and MCP `capabilities` payload. Cloud and Remote must also return `persistentStorageVerified=true`. Only a deployment that has verified a stable volume, stable absolute mount paths, and the single-writer constraint may inject that value. If it is missing or false, Wework disables Worktrees with `worktree_persistent_storage_unverified`.
4. `runtime.worktrees.preflight` creates no task Worktree; `runtime.tasks.create` repeats the critical checks immediately before creation.
...
10. Deletion requires stop acknowledgement from every linked Runtime. Archive, snapshot, and removal happen in order, and every failure preserves diagnosable data.
11. Restart reconciliation may recover a manageable Worktree, but never auto-resumes an interrupted agent.
...
14. Offline, missing route, timeout, disconnect, unsupported, non-Git, path-conflict, and non-durable-storage failures have stable distinct errors. Mutating RPCs are never automatically retried.
15. Terminal, IDE, file tree, Git, archive, restore, and settings always use the owning device and the task's final Worktree path.
