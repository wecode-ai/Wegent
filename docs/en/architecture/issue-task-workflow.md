...
- The stage DAG has no structural start or end node and renders no end marker: a stage without predecessors is the entry (a root becomes ready directly), and the flow simply ends at the last stage. An Issue enters review only when every required stage completes; legacy start/end nodes are stripped on load and during projection.
