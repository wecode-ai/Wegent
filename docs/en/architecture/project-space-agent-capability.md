...
- The project-space MCP is enabled by default for every Agent session. Every generic task first receives mandatory My Tasks base membership; Composer project-space selection only adds project context, while no selection uses the default My Tasks context. Project or Issue conversations receive default `space_id/item_id` values and out-of-scope protection through ContextGrant.
