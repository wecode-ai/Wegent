...
- The stage DAG has no structural start/end nodes and renders no end marker: a stage without predecessors is the entry, and the flow simply ends at the last stage. After `complete` finishes a wait node, every required stage completing carries the Issue into review; legacy start/end nodes are stripped on load and during projection.
