...
Invariants: all entries share assignment and activation; activation occurs only after commit; `loop_item_executions` is the sole board-execution truth; Wegent binding uses exact execution/task/subtask/team IDs; MySQL `loop_items.assignee_team_id=0` means unassigned only, and services plus APIs must normalize it to `null` instead of treating `0` as a Team ID; this sentinel rule does not apply to `loop_item_executions.team_id`; cancellation writes intent first and only a Runtime ACK writes terminal state; messages and UI never overwrite execution state.
