...
flowchart LR
    ...
    PROCESS --> TURN_CACHE[completed-turn cache]
    TURN_CACHE --> TRANSCRIPT[Runtime transcript]
...
sequenceDiagram
    ...
    alt task is still running
        P-->>R: persist completed messages before clearing the active turn
        R-->>U: return completed-turn cache + current live turn and keep streaming events
