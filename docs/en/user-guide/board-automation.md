...
When AI asks a person a question, cards and notifications locate that thread's reply composer. Reply only posts a comment; the assignee explicitly chooses Reply and continue to submit the assignment result and return control to AI. A posted reply can be submitted without typing it again. Later AI questions stay in the same thread and the coordinator continues its original conversation: a custom Runtime reuses the original task ID, while Wegent reuses the original Task and opens another Subtask turn inside it. Each continuation has independent coordination and execution records, so it cannot rewrite the preceding turn's result. If that coordinator conversation no longer exists, continuation fails explicitly instead of silently creating a task. Replies to ordinary activities with a runtime task continue their existing task conversation.
