...
Historical transcripts may identify an assistant message's subtask with either camelCase `subtaskId` or snake_case `subtask_id`. Wework must normalize both forms when restoring messages because tool-call blocks and file-change blocks depend on that identity and must not disappear because of transport naming conventions.
