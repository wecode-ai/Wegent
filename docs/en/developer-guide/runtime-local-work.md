...
When the archived list is empty, Settings still offers **Scan leftover files**. This request does not depend on deleted archived records: the executor scans only first-level directories named `runtime-<digits>` under `~/.wegent-executor/workspace/worktrees` and `~/.wecode/wegent-executor/workspace/worktrees`, excluding directories belonging to active or otherwise tracked LocalTasks. The scan result still requires user confirmation before deletion, so it never removes a Project root, an arbitrary custom directory, or an active task worktree.
