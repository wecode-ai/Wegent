...
Set `VITE_WEGENT_BACKEND_URL` at build time or during local development as the single Backend root URL. Wework derives `/api` and `/socket.io` from it and prefills it in Connect cloud; the field remains editable, and an existing local connection takes precedence. When configured, the desktop account area shows "Wegent account / Not signed in" while disconnected and continues to open the full account menu. Users start cloud authorization through "Sign in to Wegent" at the top of that menu. After connection, the account area shows the cloud username and email; Log out in the account menu only disconnects the cloud connection.
...
Users may enter either the Backend root URL or an `/api` URL. The frontend normalizes that input into same-origin HTTP API and Socket.IO connections: the API uses `<Backend>/api`, and Socket.IO uses `<Backend>/socket.io`. Connecting first checks `/health`, then calls `/auth/wework/sessions` to create a short-lived authorization session. Backend returns a complete `authorize_url`; local Wework opens that cloud authorization page in the embedded authorization browser and polls the session result with the client-only `poll_token`. A separately stored Socket origin from an older version is migrated to the Backend origin on startup.
