...
| `$ai_generation` | Each LLM-backed assistant response, measured from assistant start to settled. | `$ai_generation_id`, `$ai_trace_id` (the run's opaque trace id; the PostHog-required property that groups generations into a trace), `$ai_parent_id` (same per-run id, kept for tree nesting), `$ai_model` (the model name from the app's model catalog), `$ai_provider` (bounded known-provider enum), `$ai_input_tokens`, `$ai_output_tokens`, `$ai_total_tokens`, `$ai_latency` (seconds), `result`. |
...
`$ai_model` reports the model name from the app's model catalog (for example `ali-deepseek-v3.1(国内)` or `deepseek-flash-responses(公网)`) — the entry the user sees and selects. A model name is configuration metadata: it never contains prompts, responses, or file contents. Before transmission the value is only flattened to a single line and bounded (control characters removed, whitespace collapsed, truncated to 128 characters); there is no model-id allowlist, so a new model in the catalog needs no enumerator update and the value falls back to `other` only when no model was resolved for the run. `$ai_provider` is a bounded enum of known providers derived with the following priority: first, the model id prefix identifies the vendor (e.g. `moonshot-kimi-*` and `kimi-*` map to `moonshot`, `deepseek-*` to `deepseek`, `minimax-*` to `minimax`, and `gpt-*` to `openai`); if that misses, official Codex catalog models are reported as `openai` because they are all routed through Wework's openai-responses proxy; only as a last resort does it fall back to the provider string on the model configuration. Note that the provider string is free text: both the cloud and self-configured model channels can write the API transport (e.g. Kimi served over an anthropic-messages endpoint reports `anthropic`) instead of the actual vendor, so when the model id is unrecognized `$ai_provider` may be dirty data. `$ai_cost` is currently not sent: the property stays in the event allowlist until the backend exposes per-call cost. PostHog therefore derives cost from `$ai_model`, `$ai_provider`, and the token counts, so a catalog model name that is not a standard model id does not match PostHog's pricing table and its cost shows as empty; use token counts or a backend-reported cost for billing-grade numbers.
