...
Wework must not periodically read transcripts for running tasks to detect a missed terminal event. The normal lifecycle consumes only the stream. The task pane normally consumes a terminal event and refreshes its task snapshot. If that task is still running after the event loop completes, the resident coordinator performs one fallback `runtime.tasks.list` reconciliation, so persisted completion is still recovered when the pane subscription is absent or does not match. Explicit stale-projection signals such as `executor.event_lagged` and runtime transport replacement also trigger reconciliation. Concurrent terminal or anomaly signals share one in-flight request, and signals received while it is in flight coalesce into at most one serial trailing reconciliation. Reconciliation uses the executor's persisted `running`, `status`, `completedAt`, `threadStatus`, and `turnStatus` fields directly; it must not derive task completion from transcripts, turn items, or rollout JSONL.
