...
Rich references sent by Wework, such as skill or plugin tokens, may be normalized to plain text in the Codex provider transcript. Wework's original product-side user message is stored in the LocalTask `runtimeHandle.messages`. When the executor merges the provider transcript, it must match by stable `clientMessageId` first, keep provider `content` as the only body, and extract only the skill-reference ranges and targets that the transcript cannot express. Those non-overlapping additions are returned as `presentationReferences`. Normalized text signatures are only a fallback when no stable client message ID is available. This composition applies to running, successful, and failed tasks; Wework combines the body and presentation references at the rendering boundary so reopening a completed task keeps skill references rendered as tokens instead of degrading to raw `$plugin:skill` text.
