...
Rich references sent by Wework, such as skill or plugin tokens, may be normalized to plain text in the Codex provider transcript. When the executor merges that transcript with cached local user messages, it must match by stable `clientMessageId` first and restore the cached user-visible `content` while retaining runtime fields from the provider message. Normalized text signatures are only a fallback when no stable client message ID is available. This keeps skill references rendered as tokens after a transcript refresh instead of degrading to raw `$plugin:skill` text.
