...
- A bottom-origin scroller (`flex-col-reverse` with a negative `scrollTop`) preserves its distance from the start of the content while the content height changes. A message that sits entirely above the viewport therefore moves every row underneath it — including the one the reader is looking at — together with the scroll position when it is re-measured (estimated height corrected to the measured height). The reader has to be put back, otherwise each measurement correction drags them down by one size delta and lands them on the very bottom once that delta exceeds their distance from the bottom. Messages that reach into or past the viewport top only change the height underneath the reader, which the scroller already keeps visually stable, so no scroll position may be written for them.
- There is exactly one decision point: while the reader owns the viewport, the scroll area's layout-change handler reads where the anchor row ended up, compares that with its recorded offset, and writes the measured difference back to `scrollTop`. That difference already covers both the drag a re-measure applied and whatever the reader scrolled in the meantime, so no correction may be predicted from a reported size delta — a predicted write lands before the layout actually settles, over- or undershoots once the measured height differs from the prediction, and shows up as a visible bounce. The handler runs inside the ResizeObserver callback (after layout, before paint), so the move is never visible. When the anchor row has been unmounted, fall back to the reader's stable distance from the bottom, sampled only from user scroll reports whose content height did not change, so a drag can never be mistaken for the reader's intent. The correction must re-sample the anchor baseline afterwards — otherwise the next comparison treats the reader's own scrolling as a drag and pushes them back.
