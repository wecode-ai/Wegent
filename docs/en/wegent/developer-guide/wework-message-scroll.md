...
The goal-continuation regression reads the real executor log to verify that goal submission awaits exactly one turn automatically started by `thread/goal/set`, issues no extra `turn/start`, and renders no error card during streaming or after completion.
