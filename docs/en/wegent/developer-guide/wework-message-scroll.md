...
- A bottom-origin scroller (`flex-col-reverse` with a negative `scrollTop`) reacts to a content height change in two opposite ways. A change **above** the viewport is applied to the reader's distance from the bottom as the height delta, which carries the text they are reading with it. A change **below** the viewport with nothing underneath to absorb it — a streaming response growing, for example — keeps `scrollTop` and pushes the whole history up by the delta instead, which is what the desktop e2e's “the user-selected text jittered while chunks streamed” assertion pins down. The first case has to be given back as `-ΔH`, the second as `+ΔH`; applying one compensation to both pushes the reader into the history on every frame of a stream.
- There is exactly one decision point: while the reader owns the viewport, the scroll area's layout-change handler measures three things — the change in the distance from the bottom, the change in the content height, and how far the text the reader is looking at actually moved — and writes `dragAboveViewport = ΔdistanceFromBottom - ΔcontentHeight - ΔanchorOffset` back to `scrollTop`. Comparing all three over the same window separates "how far this layout moved the reader's text" from "how far the reader scrolled", and it never cancels the reader's own scrolling: for a pure user scroll the three terms cancel out exactly. No correction may be predicted from a reported size delta — a predicted write lands before the layout actually settles, over- or undershoots once the measured height differs from the prediction, and shows up as a visible bounce. Nor may a layout change be corrected by pinning the anchor element to its old screen position: that eats whatever the reader scrolled in the same frame, and while a response streams the layout changes every frame, so the reader finds every scroll pushed back. The handler runs inside the ResizeObserver callback (after layout, before paint), so the move is never visible. When the sampled text is no longer on screen the change cannot be attributed, so nothing is guessed and the sample is simply retaken. The correction must re-sample afterwards, otherwise the window spans across it.
...
- The user viewport anchor is the reading position the user chose, so it may only be refreshed on real user intent (wheel, pointer, touch, key) and never on a scroll event that follows a content-height change in the same frame; otherwise a layout-driven drag is re-baselined into the anchor and `restoreUserViewportAnchor` can never detect that drag. An empty capture must never overwrite a usable anchor either.
