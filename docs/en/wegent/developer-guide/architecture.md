...
WARMPOOL_TEMPLATE_NAME: ""           # Shared Sandbox/standard Executor template
EXECUTOR_WARMPOOL_ENABLED: true       # Allow standard Executors to use the shared warm pool
EXECUTOR_NON_GIT_WARMPOOL_ENABLED: false # Allow non-Git Executors to use the shared warm pool
EXECUTOR_GIT_WARMPOOL_ENABLED: false  # Allow safe HTTPS Git tasks to use the shared warm pool
WEGENT_GIT_CLONE_TIMEOUT_SECONDS: 600 # Overall Git clone timeout, clamped to 1-3600 seconds
WEGENT_GIT_HTTP_LOW_SPEED_LIMIT: 1024 # Minimum Git HTTP transfer rate in bytes/second
WEGENT_GIT_HTTP_LOW_SPEED_TIME_SECONDS: 60 # Time below the rate before Git aborts
```

#### Kubernetes Executor warm pool

Standard `online` tasks in Kubernetes share the same `SandboxTemplate` and `SandboxWarmPool` selected by `WARMPOOL_TEMPLATE_NAME` with interactive Sandboxes; no dedicated Executor pool is created. Non-Git tasks use the shared pool only when both `EXECUTOR_WARMPOOL_ENABLED=true` and `EXECUTOR_NON_GIT_WARMPOOL_ENABLED=true`; the new switch defaults to disabled, so an unset deployment continues creating direct Pods. Git tasks do not depend on the non-Git switch, but still require the standard Executor master switch plus `EXECUTOR_GIT_WARMPOOL_ENABLED`, and must use HTTPS with an encrypted request token whose account domain exactly matches the repository; the transport must be `encrypted_request_token`. SSH, HTTP, `git_worktree`, missing-token, legacy user-Secret, custom Executor image, and `base_image` tasks continue through direct Pod creation. A configured user Git account alone does not make the current task a Git task.

A warm Pod starts with static runtime configuration only. It does not contain a task ID, authentication token, Git token, skill identity, or task heartbeat ID. After Executor Manager claims the Pod, it applies only non-sensitive task labels. The encrypted Git token arrives in the first `/v1/responses` request; Executor decrypts it into task-private AskPass files and exposes task-scoped environment variables to Git, `gh`, and `glab`. The token is never placed in the clone URL, process argv, Pod labels, annotations, or Redis bindings. `WARMPOOL_ENABLED` remains the shared-pool master switch, and the Git switch defaults to disabled; deployments must explicitly set `EXECUTOR_GIT_WARMPOOL_ENABLED=true` to enable it. Before rollout, upgrade the shared template to an image that contains dynamic task heartbeat and task-scoped Git authentication support and matches `EXECUTOR_IMAGE`, then verify Sandbox, non-Git Executor, and HTTPS Git Executor flows.

Before a Git task claims capacity, Executor Manager validates the `SandboxTemplate` image, GitHub domain proxy configuration, and the AES Secret mounted at `/etc/wegent-executor-secret`. It repeats the checks against the claimed Pod so an old standby Pod cannot slip through after an in-place template update. If any capability is missing or cannot be inspected, the Claim is discarded and the task creates a direct Pod instead; Git credentials are not dispatched to the incompatible warm Pod.

Executor disables interactive credential prompts for `git clone` and enforces both an overall timeout and Git HTTP low-speed limits. When the overall timeout expires, it terminates the entire Git process group, removes the incomplete workspace created by that clone, and returns a terminal task error. Existing workspaces must also pass a local `HEAD` validation so a `.git` directory left by an interrupted clone is not treated as a usable repository. The flow does not retry automatically.
