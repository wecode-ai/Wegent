...
Closing a Wework window does not make the WebView conversation-view cache a message persistence source. When a task is reopened, the executor rebuilds history from the Codex thread transcript. During an active turn, the provider transcript may temporarily omit the current user item, so the executor uses the actual in-memory execution state to merge missing user messages from the local runtime handle back into the transcript by creation time. The merge deduplicates messages by signature and preserves the original order of historical user messages, completed assistant messages, and current streaming output, preventing a reopened window from showing only the content still being generated.
