...
An accessible marketplace Smart app can provide a `wework://smart-app/<app ID>` sharing link. Opening the link from a browser or chat client invokes an installed Wework desktop app; if Wework is not signed in, it waits for sign-in to finish. Wework then verifies access for the current account, downloads and installs a missing app, updates an older local installation, and starts that Smart app directly. The deep link works both while Wework is running and from a cold start, regardless of whether the Experimental features entry is visible. Private, unshared apps do not expose a sharing link, and knowing an ID does not bypass server-side access control.
