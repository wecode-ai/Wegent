...
Open the new-tab menu, choose **Browser**, and enter an `http`, `https`, or `file://` address. The current page remains loaded when you switch to file or review tabs. `file://` addresses are useful for previewing local HTML, PDF, image, and text files. When a local file cannot be previewed, Wework cancels the local-file download and shows a notice with an action to reveal the original file in Finder.
