...
- Configure an exact origin for any Web client that uses this API.
...
The desktop entry is available only for cloud devices with an advertised `desktop.available: true` capability. It is disabled while the device is offline. Deployments without a registered provider do not advertise this capability, so Wework hides the entry. Normally Backend reports:
...
5. Wework consumes the short-lived session URL contract without exposing upstream credentials.
