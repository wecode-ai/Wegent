...
The `VncSessionState`, VNC command registrations, and loopback bridge startup call in `wework/src-tauri/src/lib.rs` remain required static native-host wiring. Backend VNC configuration APIs, the WebSocket proxy, and this documentation also remain outside Wecode. Apart from those required integration points, public Vite configuration, React components, E2E control code, and tests must not add VNC, noVNC, or dedicated IPC implementations.
