...
- Electron Smart apps reuse the same renderer-owned `<webview>` hosting path and use a stable `smart-app:<installationId>` logical label. The main process manages only the Harness runtime process and no longer creates a visible `WebContentsView`. A component remount must atomically replace the old guest, and delayed closes must carry the expected native label so a stale component cannot close the replacement.
