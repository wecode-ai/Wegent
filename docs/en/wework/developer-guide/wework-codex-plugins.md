...
Codex app-server requests from `item/commandExecution/requestApproval`, `item/fileChange/requestApproval`, and `item/permissions/requestApproval` are mapped to Wework `request_user_input` cards. The card displays only the decisions advertised by the protocol and returns stable protocol values; the Executor converts the selected value to Codex `accept`, `acceptForSession`, `decline`, or `cancel` responses. Approved permission requests use turn or session scope according to the user's choice, while a denial grants no additional permissions.
