...
2. Title binding to navigation: `on_navigation` for any scheme clears the webview entry's `last_history_id`; recordable schemes (http/https/file) additionally mark `pending_history_url` and clear the stale title. A `on_document_title_changed` firing mid-navigation (titles usually arrive before Finished) does not backfill history — the new title is carried by the visit recorded at Finished. Only after a successful recording is the returned id written to `last_history_id` (generation-skipped or failed recordings never set it), and post-load title changes backfill by that exact id — same-URL tabs, failed recordings, and custom-protocol pages can never write a new page's title onto the previous page's entry.
