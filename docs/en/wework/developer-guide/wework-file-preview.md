...
Mermaid can generate HTML labels containing `<br>` inside an SVG `foreignObject`. The renderer must parse Mermaid output with HTML semantics before importing the SVG; reparsing it as strict XML first would reject non-self-closing HTML tags produced by browser serialization. Before import, remove `script`, `iframe`, `object`, and `embed` elements plus event-handler attributes from both the SVG root and its descendants. PlantUML output remains parsed as strict SVG/XML.
