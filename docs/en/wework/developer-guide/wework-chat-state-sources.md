...
| Whether the assistant is streaming    | `paneSession.status.isAssistantStreaming`                                                                              | Streaming message presentation and debug state                                                                     | Derived from the latest `assistant + streaming` message; layouts must not scan messages directly                                                                                                                                            |
...
| Current runtime running snapshot      | `getRuntimePaneTaskExecution(state.runtimeWork, address)`                                                              | `status.taskExecution`, queue advancement, `currentRuntimeTaskRunning`, composer pause button                      | `turnRunning` only means the current turn is executing; visible `running` is composed from `turnRunning` and the task's active Goal status                                                                                                  |
...
| -------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Running        | `task.running === true`, or `task.goalStatus === 'active'`                        | Show the sidebar spinner, pause in the current pane composer, and "Thinking" in the message area             |
...
An active Goal may briefly report `running: false` between two automatic continuation turns. While `goalStatus` remains `active`, the task is still running in product semantics: keep the sidebar spinner, composer pause button, and message-area "Thinking" visible, and do not create an unread dot or completion notification. A stale streaming message from the previous turn must not be revived; the between-turn "Thinking" indicator is derived separately from the Goal's continuing running state.
...
Unread tracking stores only the running-to-settled edge and must not infer terminal state from free-form `status` text. The current task and every running task must be excluded from visible unread state. Opening a task clears its unread state.
