...
CONFIG_FILE="$SCRIPT_DIR/.wegentrc"

# Load configuration from .wegentrc file
load_config() {
    if [ -f "$CONFIG_FILE" ]; then
        echo -e "${BLUE}Loading configuration from .wegentrc...${NC}"
...
# Save configuration to .wegentrc file
save_config() {
    local config_content="# Wegent Configuration File
# Generated by start.sh --init
# You can modify these values or regenerate with: ./start.sh --init
...
"
    ...
    echo -e "${GREEN}✓ Configuration saved to .wegentrc${NC}"
}
...
init_config() {
    ...
    if [ "$mode" = "standalone" ] && [ -f "$CONFIG_FILE" ]; then
        echo -e "${YELLOW}⚠️  Configuration file .wegentrc already exists.${NC}"
    ...
    # Frontend Port
    echo -e "${CYAN}1. Frontend Port${NC}"
    echo -e "   The port where the web UI will be accessible."
    read -p "   Frontend Port [$DEFAULT_FRONTEND_PORT]: " input_port
    FRONTEND_PORT=${input_port:-$DEFAULT_FRONTEND_PORT}
    echo ""

    # Executor Image
    echo -e "${CYAN}2. Executor Docker Image${NC}"
    ...
    # API URL
    echo -e "${CYAN}3. Backend API URL${NC}"
    echo -e "   The URL for frontend to communicate with backend."
    echo -e "   Use 'http://localhost:8000' for local development."
    read -p "   API URL [$DEFAULT_API_URL]: " input_api_url
    API_URL=${input_api_url:-$DEFAULT_API_URL}
    echo ""

    # Socket URL
    echo -e "${CYAN}4. Socket URL${NC}"
    ...
    local default_socket="http://$local_ip:8000"
    read -p "   Socket URL [$default_socket]: " input_socket_url
    SOCKET_URL=${input_socket_url:-$default_socket}
    echo ""
    ...
    echo -e "  Socket URL:       ${CYAN}$SOCKET_URL${NC}"
    echo -e "${BLUE}════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    if [ "$mode" = "standalone" ]; then
        ...
        echo -e "${YELLOW}To modify configuration, edit .wegentrc or run './start.sh --init' again.${NC}"
    fi
}
...
Configuration File:
  The script supports a .wegentrc configuration file in the project root.
  Variables defined in .wegentrc will be loaded automatically on startup.
  Use '--init' to create or update the configuration file interactively.

  Supported variables in .wegentrc:
    FRONTEND_PORT     - Frontend port number
    EXECUTOR_IMAGE    - Docker image for executor
    API_URL           - Backend API URL
    SOCKET_URL        - WebSocket URL for real-time communication
...
# Load configuration from .wegentrc file (if exists)
# This sets variables from the config file
load_config

# Apply command line overrides (CLI arguments take precedence over config file)
[ -n "$CLI_FRONTEND_PORT" ] && FRONTEND_PORT="$CLI_FRONTEND_PORT"
[ -n "$CLI_EXECUTOR_IMAGE" ] && EXECUTOR_IMAGE="$CLI_EXECUTOR_IMAGE"
[ -n "$CLI_API_URL" ] && API_URL="$CLI_API_URL"
[ -n "$CLI_SOCKET_URL" ] && SOCKET_URL="$CLI_SOCKET_URL"
...
start_services() {
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        echo -e "${YELLOW}Let's create a configuration file (.wegentrc) first.${NC}"
        echo ""
