...
fn local_execution_runtime_payload(execution: &LocalExecution) -> Value {
    ...
    let task_instructions = stored
        .get("system_prompt")
        .and_then(Value::as_str)
        .unwrap_or_default()
        .trim();
    let system_prompt = [execution.agent_system_prompt.trim(), task_instructions]
        .into_iter()
        .filter(|value| !value.is_empty())
        .collect::<Vec<_>>()
        .join("\n\n");
    ...
    let mut payload = json!({
        ...
        "message": message,
        "projectInstructions": system_prompt,
        "title": runtime_title,
...
mod tests {
    ...
    fn local_issue_automation_runs_constrained_agents_in_order_and_completes_issue() {
        ...
        let first_activity = comments
            .iter()
            .find(|comment| comment.metadata["execution_id"] == json!(first.id))
            .expect("the first workflow execution must have an activity");
        assert_eq!(first_activity.metadata["workflow_task_title"], "Claude");
        let second_activity = comments
            .iter()
            .find(|comment| comment.metadata["execution_id"] == json!(second.id))
            .expect("the second workflow execution must have an activity");
        assert_eq!(second_activity.metadata["workflow_task_title"], "Codex");
    }
