...
pub(crate) fn cleanup_task_files_preview(link: &RuntimeTaskLink) -> Value {
    cleanup_task_files_response(link, false, true)
}

pub(crate) fn cleanup_task_files_response(
...
pub(crate) fn cleanup_summary_response(results: Vec<Value>, deleted: bool) -> Value {
...
pub(crate) fn cleanup_targets_for_task(link: &RuntimeTaskLink) -> Vec<CleanupTarget> {
...
pub(crate) fn push_cleanup_target(
...
pub(crate) fn worktree_cleanup_target(path: &str) -> Option<CleanupTarget> {
...
pub(crate) fn standalone_chat_cleanup_target(
    local_task_id: &str,
    path: &str,
...
pub(crate) fn workspace_attachment_cleanup_target(
    link: &RuntimeTaskLink,
    relative: &str,
) -> Option<CleanupTarget> {
...
pub(crate) fn local_attachment_cleanup_target(path: &str) -> Option<CleanupTarget> {
    let normalized = normalize_workspace_path(path);
...
pub(crate) fn local_attachment_paths(value: &Value) -> Vec<String> {
    let mut paths = Vec::new();
    collect_local_attachment_paths(value, &mut paths);
    paths
}

pub(crate) fn collect_local_attachment_paths(value: &Value, paths: &mut Vec<String>) {
    match value {
        Value::Array(items) => {
            for item in items {
...
pub(crate) fn remove_cleanup_target(target: &CleanupTarget) -> Result<(), String> {
...
pub(crate) fn remove_git_worktree_best_effort(path: &Path) {
...
pub(crate) fn path_size(path: &Path) -> Option<u64> {
...
pub(crate) fn is_managed_worktree_path(path: &str) -> bool {
    path.contains("/.wecode/wegent-executor/workspace/worktrees/")
        || path.contains("/.wegent-executor/workspace/worktrees/")
}

pub(crate) fn is_local_attachment_draft_path(path: &str) -> bool {
...
pub(crate) fn task_action_success(link: &RuntimeTaskLink) -> Value {
    json!({
        "success": true,
        "accepted": true,
        "taskId": link.local_task_id,
...
pub(crate) fn task_action_failure(link: &RuntimeTaskLink, error: String) -> Value {
    json!({
        "success": false,
        "accepted": false,
        "taskId": link.local_task_id,
...
pub(crate) fn codex_error_is_missing_rollout(error: &str, thread_id: &str) -> bool {
    let error = error.to_ascii_lowercase();
    error.contains("no rollout found for thread id")
        && error.contains(&thread_id.to_ascii_lowercase())
}

pub(crate) fn task_goal_missing_session(link: &RuntimeTaskLink) -> Value {
    json!({
        "success": false,
        "accepted": false,
        "taskId": link.local_task_id,
