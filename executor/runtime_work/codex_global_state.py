...
class CodexGlobalProjectIndex:
    ...
    def project_for_thread(
        ...
        if hinted_root:
            hinted_project = self.projects_by_key.get(_normalize_path_or_raw(hinted_root))
...
def load_codex_global_state(codex_home: Path) -> Optional[CodexGlobalState]:
    ...
    thread_workspace_root_hints = {
        ...
        for thread_id, root in _string_map(payload.get("thread-workspace-root-hints")).items()
    }
    return CodexGlobalState(
        ...
        workspace_root_labels={
            _normalize_path_or_raw(root): label for root, label in workspace_root_labels.items()
        },
