...
#[tokio::test]
async fn app_ipc_prioritizes_lag_recovery_over_queued_runtime_deltas() {
    use tokio::{
        io::{AsyncBufReadExt, AsyncWriteExt, BufReader},
    ...
    let server = AppIpcServer::new().with_device_id("device-1");
    let event_source = server.clone();
    let (client, executor) = tokio::io::duplex(256);
    let (executor_reader, executor_writer) = tokio::io::split(executor);
    let task = tokio::spawn(async move { server.serve_io(executor_reader, executor_writer).await });
    let (reader, mut writer) = tokio::io::split(client);
    let mut reader = BufReader::new(reader);
    ...
    for sequence in 0..512 {
        event_source
            .emit_event(
                ...
                json!({ "sequence": sequence, "delta": delta }),
            )
            .unwrap();
    }
    ...
    event_source
        .emit_event(
            "runtime.plan.updated",
            json!({ "sequence": 512, "delta": delta }),
        )
        .unwrap();
    sleep(Duration::from_millis(50)).await;
    for sequence in 513..2561 {
        event_source
            .emit_event(
                "runtime.plan.updated",
                json!({ "sequence": sequence, "delta": delta }),
    ...
    let lag_event = timeout(Duration::from_secs(2), async {
        for _ in 0..10 {
            line.clear();
            reader.read_line(&mut line).await.unwrap();
            let message: Value = serde_json::from_str(&line).unwrap();
            if message["event"] == "executor.event_lagged" {
                return Some(message);
            }
        }
        None
    ...
    .await
    .expect("lag recovery event should not wait behind queued deltas")
    .expect("lag recovery event should be prioritized");

    assert!(lag_event["payload"]["skipped"].as_u64().unwrap() > 0);
