...
#[tokio::test]
async fn static_custom_headers_are_materialized_for_native_marketplaces() {
    let temp = TempRoot::new("static-custom-header-marketplace");
    let claude = temp.path().join(".claude/plugins");
    let codex = temp.path().join(".codex/plugins");
    let source = r#"{"business":{"url":"https://business.invalid/mcp","headers":{"X-Author-Header":"author"}},"private":{"url":"https://private.invalid/mcp","headers":{"Authorization":"Bearer ${{task_token}}"}},"public":{"url":"https://public.invalid/mcp"}}"#;
    let package = zip_bytes(&[
        (
            "fixture/.codex-plugin/plugin.json",
            r#"{"name":"fixture","version":"1.0.0","mcpServers":"./.mcp.json"}"#,
    ...
    let result = handler
        .apply_sync(json!({"mode":"replace", "plugins":[{
            "installed_plugin_id":20,"name":"fixture","marketplace":"wegent",
            "version":"1.0.0","download_path":"/download","checksum":checksum,
            "component_config":{"mcp:business":{"headers":{
                "X-Author-Header":"override","X-Custom":"static"
            }},"mcp:private":{"headers":{"Authorization":"Bearer fixed"}}}
        }], "skills":[],"mcps":[]}))
        .await
        .unwrap();
    ...
    for (home, runtime, name) in [
        ...
        let native = read_json(root.join(manifest["mcpServers"].as_str().unwrap()));
        assert_eq!(
            native["mcpServers"]["business"]["headers"]["X-Author-Header"],
            "override"
        );
        assert_eq!(
            native["mcpServers"]["business"]["headers"]["X-Custom"],
            "static"
        );
        let snapshot = read_json(root.join(format!(".wegent-task-mcp-source-{runtime}.json")));
        assert_eq!(
            native["mcpServers"]["private"]["headers"]["Authorization"], "Bearer fixed",
            "{runtime} native config must retain the static override: native={native}, snapshot={snapshot}"
        );
        if runtime == "claude" {
            let default = read_json(root.join(".mcp.json"));
            assert_eq!(
                default["mcpServers"]["business"]["headers"]["X-Custom"],
                "static"
            );
            assert_eq!(
                default["mcpServers"]["private"]["headers"]["Authorization"], "Bearer fixed",
                "{runtime} default config must retain the static override"
            );
        }
    }
}
...
#[tokio::test]
async fn invalid_custom_headers_fail_native_marketplace_sync() {
    let temp = TempRoot::new("invalid-custom-header-marketplace");
    let package = zip_bytes(&[(
        "fixture/.codex-plugin/plugin.json",
        r#"{"name":"fixture","version":"1.0.0","mcpServers":{"business":{"url":"https://business.invalid/mcp"}}}"#,
    )]);
    let checksum = sha256_hex(&package);
    let store = GlobalCapabilityStore::new(
        temp.path().join("manifest.json"),
    ...
    let result = handler
        .apply_sync(json!({"mode":"replace", "plugins":[{
            "installed_plugin_id":20,"name":"fixture","marketplace":"wegent",
            "version":"1.0.0","download_path":"/download","checksum":checksum,
            "component_config":{"mcp:business":{"headers":{"X-Custom":"bad\0value"}}}
        }], "skills":[],"mcps":[]}))
        .await
        .unwrap();
    ...
    assert!(result["plugins"][0]["error"]
        .as_str()
        .unwrap()
        .contains("Invalid plugin MCP custom header"));
}
