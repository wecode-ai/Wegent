...
class TestAgent:
    ...
    @patch("executor.agents.base.WorkspaceSetup")
    def test_download_code_success(self, mock_workspace_setup_class, agent):
        """Test successful code download"""
        mock_workspace_setup = MagicMock()
        mock_workspace_setup.setup_workspace.return_value = MagicMock(
            success=True,
            workspace_path="/workspace/tasks/task-123",
            project_path="/workspace/tasks/task-123/repo",
            feature_name=None,
            error_message=None,
            is_feature_workspace=False,
        )
        mock_workspace_setup_class.return_value = mock_workspace_setup

        with patch.object(agent, "setup_git_config"):
            agent.download_code()

        mock_workspace_setup.setup_workspace.assert_called_once()
        assert agent.project_path is not None

    @patch("executor.agents.base.WorkspaceSetup")
    def test_download_code_already_exists(self, mock_workspace_setup_class, agent):
