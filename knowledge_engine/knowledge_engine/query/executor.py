...
class QueryExecutor:
    ...
    def _prepare_retrieval(
        ...
        user_id: int | None,
    ) -> tuple[dict[str, Any], RetrievalScope | None, Any, dict[str, Any]]:
        """Normalize and validate one query in the configured sync runner."""
        ...
        return retrieval_setting, resolved_scope, resolved_queries, kwargs
    ...
    async def _merge_hierarchical_records(
        ...
        *,
        ...
        parent_node_ids = await self._sync_runner(collect_parent_node_ids, records)
        if not parent_node_ids:
            return result

        parent_records = await self._sync_runner(
        ...
        return await self._sync_runner(
            self._merge_parent_records,
            result,
            records,
            parent_records,
        )
