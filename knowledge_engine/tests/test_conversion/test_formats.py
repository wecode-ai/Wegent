...
def test_registry_preserves_existing_text_code_accept_extensions():
    """Existing KB text/code/config upload extensions remain accepted."""
    ...
    assert expected <= actual
    for ext in expected:
        assert get_knowledge_pipeline(ext) == KnowledgeFormatPipeline.DIRECT
...
def test_text_validation_rejects_binary_tsv_file():
    try:
        validate_knowledge_file(b"\x00\x01\x02", "tsv")
    except ValueError as exc:
        assert "binary" in str(exc)
    else:
        raise AssertionError("Expected binary TSV file to be rejected")
