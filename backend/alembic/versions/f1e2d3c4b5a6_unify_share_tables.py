...
def downgrade() -> None:
    ...
    try:
        op.drop_constraint(
            "fk_resource_members_user_id", "resource_members", type_="foreignkey"
        )
    except Exception:
    ...
    op.create_index(
        "ix_shared_teams_original_user_id", "shared_teams", ["original_user_id"]
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "user_id", "original_task_id", name="uniq_user_original_task"
        ),
    )
    op.create_index("idx_shared_tasks_user_id", "shared_tasks", ["user_id"])
    op.create_index(
        "idx_shared_tasks_original_user_id", "shared_tasks", ["original_user_id"]
    )
    op.create_index(
        "idx_shared_tasks_original_task_id", "shared_tasks", ["original_task_id"]
    )
    op.create_index(
        "idx_shared_tasks_copied_task_id", "shared_tasks", ["copied_task_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("ACTIVE", "REMOVED", name="task_member_status"),
            nullable=False,
