...
def upgrade() -> None:
    ...
    # user_id is NOT NULL DEFAULT 0; 0 means unset/invalid, >0 means valid user.
    dialect = conn.dialect.name
    if dialect == "mysql":
        # Backfill valid users
        op.execute(
            """
        ...
        # Handle invalid rows (user_id = 0) with unique placeholder to avoid constraint conflicts
        op.execute(
            """
            UPDATE resource_members
            SET entity_id = CONCAT('__UNKNOWN__', id),
                entity_type = 'unknown'
            WHERE user_id = 0 AND entity_id = ''
            """
        )
    elif dialect == "sqlite":
        op.execute(
