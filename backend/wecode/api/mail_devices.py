...
"""WeCode device endpoints for mail configuration."""
...
from wecode.schemas.mail import (
    MailConfigRequest,
    MailConfigResponse,
)
...
router = APIRouter()
MAIL_EXTENSION_NAME = "mail"
MAIL_EXTENSION_SCRIPT_PATH = "scripts/mail-executor-ext.sh"
...
async def create_mail_config(
    ...
    """Generate a mail config file on a connected device."""
    ...
    try:
        ...
        logger.info(
            "[Mail Config] Sending device:run_extension to socket_id=%s, "
        ...
        logger.info(
            "[Mail Config] Received response from device: type=%s, value=%s",
            type(response).__name__,
            response if isinstance(response, dict) else repr(response)[:200],
        )
    except Exception as exc:
        logger.exception(
            "[Mail Config] Failed to dispatch command: user_id=%s, device_id=%s",
            user_id,
            device_id,
        )
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=f"Failed to configure mail: {exc}",
        ) from exc

    if response is None:
        logger.warning(
            "[Mail Config] No response from device (timeout or not connected): "
    ...
    if not isinstance(response, dict):
        logger.warning(
            "[Mail Config] Invalid response type from device: type=%s, value=%s",
            type(response).__name__,
            repr(response)[:200],
        )
        raise HTTPException(
    ...
    message = str(response.get("message") or "Failed to configure mail")
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=message,
    ...
    return MailConfigResponse(**response)
