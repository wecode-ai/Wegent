...
def apply_patch() -> None:
    ...
    _orig_get_branch_diff = GitLabProvider.get_branch_diff
    ...
    async def patched_get_branch_diff(self, user, repo_name: str, source_branch: str, target_branch: str, git_domain: str):
        restored = await _resolve_tokens_for_user(user)
        try:
            return await _orig_get_branch_diff(self, user, repo_name, source_branch, target_branch, git_domain)
        finally:
            _restore_tokens(restored)
    ...
    GitLabProvider.get_branch_diff = patched_get_branch_diff  # type: ignore[attr-defined]
