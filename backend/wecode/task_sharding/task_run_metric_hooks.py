"""Transaction hooks for task run metrics stored in internal shard tables."""
...
_TRACKED_OBJECTS_KEY = "wecode_sharded_task_run_metric_tracked_objects"
_COMMITTED_EVENTS_KEY = "wecode_sharded_task_run_metric_committed_events"
...
class ShardedTaskRunMetricHooks:
    """Capture internal sharded subtask changes and publish after commit."""
    ...
    def register(self) -> None:
        """Register shard listeners once for the configured session factory."""
        if self._registered:
            return
        ...
        event.listen(
            self._session_factory,
            "after_rollback",
            self._after_rollback_listener,
        )
        self._registered = True

    def unregister(self) -> None:
        """Remove shard listeners during patch shutdown."""
    ...
    def _before_flush(
        ...
        for subtask in session.new:
            if not _is_sharded_subtask(subtask) or not _is_assistant(subtask.role):
                continue
        ...
        for subtask in session.dirty:
            if not _is_sharded_subtask(subtask):
                continue
    ...
    def _after_flush_postexec(self, session: Session, flush_context: Any) -> None:
        tracked: dict[int, _TrackedSubtask] = session.info.pop(
            _TRACKED_OBJECTS_KEY,
            {},
        )
    ...
    def _after_commit(self, session: Session) -> None:
        ...
        try:
            ...
            logger.exception(
                "Failed to publish committed sharded task run metrics; "
...
def queue_bulk_sharded_subtask_status_metrics(
    session: Session,
    subtasks: Iterable[Any],
    *,
    status: SubtaskStatus,
) -> None:
    """Queue shard metrics for a bulk update that bypasses ORM flush hooks."""
    committed: dict[int, TaskRunMetricEvent] = session.info.setdefault(
        _COMMITTED_EVENTS_KEY,
        {},
    )
...
def _is_sharded_subtask(value: Any) -> bool:
    return getattr(type(value), "__source_model__", None) is Subtask
...
sharded_task_run_metric_hooks = ShardedTaskRunMetricHooks(
    SessionLocal,
    task_run_metrics_store,
)
