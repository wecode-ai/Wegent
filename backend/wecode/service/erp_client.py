...
class ErpClient:
        ...
        try:
            ...
            with httpx.Client(timeout=self.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"ERP token request failed: {response.status_code}: {response.text}"
                    )
                    return None
    ...
    def batch_check_membership(
        ...
        for i in range(0, len(department_ids), 50):
            ...
            try:
                ...
                logger.error(f"ERP batch_check_membership request failed: {e}")
    ...
    def search_departments(self, keyword: str) -> list[DepartmentInfo]:
        ...
        try:
            ...
            if response.status_code == 200:
                resp_data = response.json().get("data", {})
                departments = resp_data.get("departments", [])
                return [DepartmentInfo.model_validate(d) for d in departments]
            else:
                logger.error(
                    f"ERP search_departments error: "
                    f"{response.status_code}: {response.text}"
                )
                return []
        except httpx.RequestError as e:
            logger.error(f"ERP search_departments request failed: {e}")
            return []
    ...
    def search_employee(self, keyword: str) -> Optional[EmployeeInfo]:
        ...
        try:
            ...
            if response.status_code == 200:
                resp_data = response.json().get("data", {})
                employees = resp_data.get("employees", [])
                if employees:
                    return EmployeeInfo.model_validate(employees[0])
            else:
                logger.error(
                    f"ERP search_employee error: "
                    f"{response.status_code}: {response.text}"
                )
        except httpx.RequestError as e:
            logger.error(f"ERP search_employee request failed: {e}")
