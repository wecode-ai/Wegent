...
class GetUserGitInfo:
    ...
    def _validate_git_token(self, git_info_item: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if validation_result.get("valid"):
                ...
                self.logger.warning(
                    f"Git token validation failed: {git_info_item['git_domain']}, token: {git_info_item['git_domain']}"
                )
                return False
    ...
    def get_and_validate_git_info(
        ...
        self.logger.info(
            f"Completed fetching and validating git tokens: count={len(git_info)}"
        )
        return git_info
            ...
            with httpx.Client() as client:
                ...
                for key in self.target_keys:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Failed to process real token (key: {key}): {str(e)}"
                        )
                        continue
