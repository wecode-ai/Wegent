...
class SaveGitToken:
    ...
    def __init__(self):
        ...
        self.save_token_api_url = (
            "http://paas.intra.weibo.com/2/appnest/api/code-server-new/secret/apply"
        )
    ...
    async def save_gitlab_tokens(
        self, username: str, email: str, git_info: List[Dict[str, Any]]
    ) -> bool:
        """
        ...
        try:
            ...
            for item in git_info:
                if (
                    item.get("type") == "gitlab"
                    and item.get("git_token")
                    and item.get("git_token") != "***"
                ):
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    self.save_token_api_url,
                    params={"user": username, "email": quote(email)},
                    headers={
                ...
                if result.get("code") == 0:
                    self.logger.info(
                        f"Successfully saved gitlab tokens for user: {username}"
                    )
                    return True
                else:
    ...
    def replace_tokens_with_placeholders(
    ...
    def save_gitlab_tokens_blocking(
        self, username: str, email: str, git_info: List[Dict[str, Any]]
    ) -> None:
        """
        ...
        try:
            with httpx.Client(timeout=10) as client:
                response = client.post(
                    self.save_token_api_url,
                    params={"user": username, "email": quote(email or "")},
                    headers={
                        "Authorization": self.auth_header,
                        "Content-Type": "application/json",
                    },
