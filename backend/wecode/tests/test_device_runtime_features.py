...
import wecode.schemas.vnc  # noqa: F401  register desktop capability validation
...
def test_heartbeat_runtime_features_use_independent_contract():
    ...
    desktop = payload.runtime_features.model_extra["desktop"]
    assert desktop["protocol"] == "rfb"
    assert desktop["transport"] == "websocket"
    assert desktop["clipboard"] == "extended-text"
...
def test_runtime_features_drop_only_malformed_desktop_capability():
    ...
    assert info.runtime_features is not None
    assert "desktop" not in info.runtime_features.model_extra
