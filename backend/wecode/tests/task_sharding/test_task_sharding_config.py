...
def test_task_sharding_defaults_match_internal_runtime() -> None:
    ...
    assert settings.WECODE_TASK_SEQ_INITIAL_SEQUENCE == 150_000


def test_task_sequence_initial_sequence_can_be_loaded_from_env(monkeypatch) -> None:
    class EnvOnlySettings(TaskShardingSettings):
        @classmethod
        def settings_customise_sources(
            cls,
            settings_cls: type[BaseSettings],
    ...
    monkeypatch.setenv("WECODE_TASK_SEQ_INITIAL_SEQUENCE", "250000")

    settings = EnvOnlySettings()

    assert settings.WECODE_TASK_SEQ_INITIAL_SEQUENCE == 250_000
...
def test_task_sequence_initial_sequence_rejects_negative_value() -> None:
    with pytest.raises(ValidationError):
        build_settings(WECODE_TASK_SEQ_INITIAL_SEQUENCE=-1)
