...
class SkillToolRegistry:
    ...
    def ensure_provider_loaded(
        ...
        Args:
            ...
            is_public: Whether this is a public skill (user_id=0). Only public skills
                       can load code for security reasons.

        Returns:
            True if provider is available (already registered or newly loaded),
            False if provider could not be loaded or security check failed
        """
        ...
        # Load provider from ZIP (this is done outside the lock as it's CPU-intensive)
        if not zip_content:
            return False
        ...
        # Thread-safe check-and-register using lock
        with self._providers_lock:
            if provider.provider_name in self._providers:
                # Already registered (possibly by another thread)
                return True
