...
# Redis key prefix for device selection. Legacy callers remain user-scoped while
# DingTalk supplies a conversation scope.
...
class DeviceSelectionManager:
    ...
    def _generate_key(user_id: int, scope: str | None = None) -> str:
        ...
        return f"{CHANNEL_USER_DEVICE_PREFIX}{user_id}{suffix}"
    ...
    async def get_selection(
        ...
        """
        key = DeviceSelectionManager._generate_key(user_id, scope)
    ...
    async def set_selection(
        ...
        *,
        ...
        logger.info(
            "[DeviceSelection] Set selection for user %d scope=%s: type=%s, device_id=%s",
            user_id,
            scope or "legacy",
    ...
    async def clear_selection(user_id: int, *, scope: str | None = None) -> bool:
        ...
        logger.info(
            "[DeviceSelection] Cleared selection for user %d scope=%s",
            user_id,
            scope or "legacy",
        )
        return result
    ...
    async def set_local_device(
        ...
        *,
        ...
        return await DeviceSelectionManager.set_selection(
            user_id, selection, scope=scope
        )

    @staticmethod
    async def set_cloud_executor(user_id: int, *, scope: str | None = None) -> bool:
        """
    ...
    @staticmethod
    async def set_chat_mode(user_id: int, *, scope: str | None = None) -> bool:
        """
