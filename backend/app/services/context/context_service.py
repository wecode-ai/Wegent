...
"""
Context service for unified management of subtask contexts.

Provides a unified interface for handling attachments, knowledge base references,
and other extensible context types associated with subtasks.
"""
...
class ContextService:
    ...
    Handles file upload, document parsing, and context lifecycle.
    ...
    def upload_attachment(
        ...
        context = SubtaskContext(
            ...
            image_base64="",  # Will be updated after parsing
            extracted_text="",  # Will be updated after parsing
        ...
        context.type_data["storage_key"] = storage_key
        ...
        try:
            ...
            context.text_length = parse_result.text_length if parse_result.text_length else 0
            context.image_base64 = parse_result.image_base64 if parse_result.image_base64 else ""
    ...
    def get_attachment_binary_data(
        ...
        if context.context_type != ContextType.ATTACHMENT.value:
            raise ValueError("Context is not an attachment")
        ...
        if not context.storage_key:
            logger.warning(f"Context {context.id} has no storage_key for external storage")
            return None

        storage_backend = get_storage_backend(db)
        return storage_backend.get(context.storage_key)
    ...
    def get_attachment_url(
        ...
        if not context.storage_key or context.storage_backend == "mysql":
            return None

        storage_backend = get_storage_backend(db)
        return storage_backend.get_url(context.storage_key, expires)

    def is_image_context(self, context: SubtaskContext) -> bool:
        """
        Check if a context is an image attachment.
    ...
    def build_vision_content_block(self, context: SubtaskContext) -> Optional[Dict[str, Any]]:
        """
        ...
        return {
            "type": "image_url",
            "image_url": {"url": f"data:{context.mime_type};base64,{context.image_base64}"},
        }

    def build_document_text_prefix(self, context: SubtaskContext) -> Optional[str]:
        """
    ...
    def build_message_with_attachment(
        ...
        Args:
            ...
            context: Context with extracted text or image data
    ...
    def get_context_optional(
        ...
        """
        Get context by ID, returns None if not found.
        ...
        Returns:
            SubtaskContext or None if not found
        """
        query = db.query(SubtaskContext).filter(SubtaskContext.id == context_id)
    ...
    def link_to_subtask(
        ...
        """
        Link a single context to subtask.
    ...
    def link_many_to_subtask(
        ...
        """
        Link multiple contexts to subtask.
        ...
        count = (
            ...
            .filter(SubtaskContext.id.in_(context_ids))
            .update({"subtask_id": subtask_id}, synchronize_session=False)
        )
        db.commit()

        logger.info(f"Linked {count} contexts to subtask {subtask_id}")
        return count
    ...
    def get_briefs_by_subtask(
        ...
        Returns:
            List of SubtaskContextBrief records
        """
    ...
    def get_knowledge_bases_by_subtask(
        self,
        db: Session,
        subtask_id: int,
    ) -> List[SubtaskContext]:
        ...
        Returns:
            List of knowledge base SubtaskContext records
        """
        return (
            db.query(SubtaskContext)
            .filter(
    ...
    def delete_context(
        ...
        # Only allow deletion of unlinked contexts (subtask_id == 0 means unlinked)
        ...
        if context.context_type == ContextType.ATTACHMENT.value and context.storage_key:
            try:
                ...
                logger.warning(f"Failed to delete context {context_id} from storage: {e}")
