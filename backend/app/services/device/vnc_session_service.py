...
class VncSessionRecord:
    """Authorization metadata persisted for one proxy session."""
    ...
    def from_dict(cls, value: Any) -> "VncSessionRecord":
        ...
        return cls(
            ...
            provider_instance_id=(
                str(value["provider_instance_id"])
                if value.get("provider_instance_id")
                else None
            ),
...
class VncSessionProviderRegistry:
    ...
    def register(self, device_type: DeviceType, provider: VncSessionProvider) -> None:
        if device_type != DeviceType.CLOUD:
            raise ValueError("VNC desktop providers support cloud devices only")
...
class VncSessionService:
    ...
    async def start_session(
        ...
        *,
        ...
        provider = (
            self._registry.get(identity.device_type)
            if identity.device_type == DeviceType.CLOUD
            else None
        )
        if provider is None:
        ...
        _validated_upstream_url(upstream.url)
        ...
        record = VncSessionRecord(
            ...
            provider_instance_id=upstream.provider_instance_id,
            expires_at=expires_at,
        )
        try:
    ...
    async def authorize_connection(
        ...
        *,
        ...
        if (
            ...
            or identity.device_type != DeviceType.CLOUD
        ):
            return None
        ...
        if (
            ...
            or upstream.provider_instance_id != record.provider_instance_id
        ):
            return None
        _validated_upstream_url(upstream.url)
        return upstream
