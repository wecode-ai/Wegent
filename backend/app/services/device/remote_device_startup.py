...
def _build_process_start_command(env: Dict[str, str], install_url: str) -> str:
    ...
    lines = [
        ...
        '  echo "The remote device startup script currently supports Linux only." >&2',
        "  exit 1",
        "fi",
        "",
        ...
        'PID_FILE="${WEGENT_EXECUTOR_PID_FILE:-$EXECUTOR_HOME/executor.pid}"',
        ...
        *env_lines,
        ...
        "EXECUTOR_PID=$!",
        'printf "%s\\n" "$EXECUTOR_PID" >"$PID_FILE"',
        'echo "wegent-executor started with PID $EXECUTOR_PID"',
