...
class MemoryService:
    ...
    - Uses mem0 best practices (run_id for session isolation)
    """
    ...
    async def retrieve_for_chat(
        ...
        Uses run_id for task-level session isolation (mem0 best practice).
        """
        if not self.enabled:
            return ""
        ...
        try:
            ...
            # Use run_id for task-level session isolation (mem0 best practice scenario 3)
            ...
            for mem in memories:
                score = mem.get("score")
                score_attr = f' relevance="{score:.2f}"' if score is not None else ""
...
{memories_xml}
            ...
            logger.debug(
                "[MEMORY] Retrieval failed (non-blocking): %s", e, exc_info=True
            )
            return ""
    ...
    async def _store_exchange_async(
        ...
        try:
            ...
            logger.debug("[MEMORY] Stored for task=%d", task_id)

        except Exception as e:
            logger.debug("[MEMORY] Storage failed (non-blocking): %s", e, exc_info=True)
    ...
    async def _delete_by_task_async(self, task_id: int):
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    logger.info(
                        "[MEMORY] Deleted all for task=%d via bulk delete", task_id
                    )
                    return
            except Exception:
            ...
            logger.info(
                "[MEMORY] Deleted %d memories for task=%d", len(memories), task_id
            )

        except Exception as e:
            logger.debug(
                "[MEMORY] Deletion failed (non-blocking) for task=%d: %s",
                task_id,
                e,
                exc_info=True,
            )
