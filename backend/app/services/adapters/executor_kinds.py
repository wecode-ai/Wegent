...
class ExecutorKindsService(
    ...
    async def dispatch_tasks(
        ...
        *,
        ...
        if task_ids:
            ...
            # Scenario 3: Also check for follow-up subtasks (续聊) - tasks that are COMPLETED
            # but have new PENDING subtasks and existing executor containers
            if len(subtasks) < limit:
                remaining_limit = limit - len(subtasks)
                followup_subtasks = self._get_followup_subtasks_with_executor(
                    db, status, remaining_limit, type
                )
                subtasks.extend(followup_subtasks)
    ...
    def _get_followup_subtasks_with_executor(
        self, db: Session, status: str, limit: int, type: str
    ) -> List[Subtask]:
        """
        ...
        For each found subtask, it copies the executor_name from the most recent
        completed subtask so that executor_manager can reuse the existing container.

        Args:
            db: Database session
            status: Subtask status to filter by (should be "PENDING")
            limit: Maximum number of subtasks to return
            type: Task type to filter by ("online" or "offline")
        ...
        for task in tasks:
            ...
            db.add(pending_subtask)

            logger.info(
                f"[followup] Found follow-up subtask {pending_subtask.id} for task {task_id}, "
                f"copying executor_name={completed_subtask_with_executor.executor_name}"
            )

            subtasks.append(pending_subtask)
