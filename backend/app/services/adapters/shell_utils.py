...
# Pattern for legacy shell names: {bot_name}-shell
LEGACY_SHELL_NAME_PATTERN = re.compile(r"^(.+)-shell$")


def is_legacy_bot_shell(shell_name: str) -> bool:
    """
    Check if a shell name follows the legacy {bot_name}-shell pattern.

    Legacy shells were created per-bot in the old system where each bot
    had its own dedicated shell record.

    Args:
        shell_name: Name of the shell to check

    Returns:
        True if this is a legacy bot-specific shell name
    """
    return bool(LEGACY_SHELL_NAME_PATTERN.match(shell_name))
...
def get_shell_info_by_name(
    ...
    Returns:
        Dict with:
            ...
            - is_legacy: Whether this is a legacy bot-specific shell
    ...
    if user_shell and isinstance(user_shell.json, dict):
        ...
        # Note: AliasChoices in ShellSpec handles 'runtime' -> 'shellType' conversion
        result = {
            "shell_type": shell_crd.spec.shellType,
            "support_model": shell_crd.spec.supportModel or [],
            "execution_type": "local_engine",
            "base_image": getattr(shell_crd.spec, "baseImage", None),
            "is_custom": True,
            "is_legacy": is_legacy_bot_shell(shell_name),
        }
...
def resolve_shell_name_for_display(
    shell_name: str, shell: Optional[Union[Kind, PublicShell]]
) -> Tuple[str, str]:
    """
    Resolve the display shell_name and shell_type from a shell reference.

    For legacy bot-specific shells ({bot_name}-shell), this returns the actual
    shell_type (e.g., 'ClaudeCode') as both shell_name and shell_type for
    better display in the frontend.

    For new shells (custom shells or public shells), shell_name is returned as-is.

    Args:
        shell_name: Original shell name from bot's shellRef
        shell: The shell object (Kind or PublicShell), or None if not found

    Returns:
        Tuple of (display_shell_name, shell_type)
        - display_shell_name: The name to show in frontend (e.g., 'ClaudeCode')
        - shell_type: The actual agent type (e.g., 'ClaudeCode', 'Agno', 'Dify')
    """
    if shell is None:
        # Shell not found, return original name
        return shell_name, ""

    shell_crd = Shell.model_validate(shell.json)
    shell_type = shell_crd.spec.shellType

    # For legacy shells, use shell_type as display name
    if is_legacy_bot_shell(shell_name):
        return shell_type, shell_type

    # For new shells, return original name
    return shell_name, shell_type
