...
class TaskKindsService(BaseService[Kind, TaskCreate, TaskUpdate]):
    ...
    def get_user_group_chats(
        ...
        Only returns tasks where user is a member (not owner) of group chats.
        ...
        # Get task member counts in batch for member_count field
        task_ids_for_members = [t.id for t in tasks]
        member_counts = {}
        if task_ids_for_members:
            member_count_results = (
        ...
        for task in tasks:
            ...
            # For group chat members, we need to look up the team via task owner
            team_result = db.execute(
                text(
                    """
                    SELECT id FROM kinds
                ...
                {"owner_id": task.user_id, "name": team_name, "namespace": team_namespace},
            ).fetchone()
            team_id = team_result[0] if team_result else None
            ...
            result.append(
                {
                    ...
                    "team_id": team_id,
                    "git_repo": git_repo,
                    "is_group_chat": True,  # All results are group chats
                    "member_count": member_counts.get(task.id, 0),
                }
            )
