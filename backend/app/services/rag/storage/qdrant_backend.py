...
class QdrantBackend(BaseStorageBackend):
    ...
    async def list_knowledge_bases(self, **kwargs) -> List[Dict[str, Any]]:
        """
        List all knowledge bases in Qdrant.

        This method aggregates unique knowledge_id values from all collections
        ...
        try:
            ...
            if mode == "fixed":
                ...
                collections = [fixed_name] if fixed_name else []
            elif mode == "per_user":
                user_id = kwargs.get("user_id")
                if not user_id:
                    return []
                collections = [f"{prefix}_user_{user_id}"]
            else:
                # For per_dataset and rolling modes, get all collections with prefix
                all_collections = qdrant_client.get_collections().collections
                collections = [
                    coll.name for coll in all_collections if coll.name.startswith(f"{prefix}_")
                ]

            # Aggregate knowledge_id values from all collections
            knowledge_bases = {}
            for collection_name in collections:
                try:
                    # Scroll through all points to collect unique knowledge_ids
                    offset = None
                    kb_docs = {}
                    while True:
                        ...
                        for point in results:
                            payload = point.payload or {}
                            kb_id = payload.get("knowledge_id")
                            doc_ref = payload.get("doc_ref")
                            if not kb_id:
                                continue
                            ...
                            if doc_ref:
                                kb_docs[kb_id]["docs"].add(doc_ref)

                        if offset is None:
                            break

                    # Add to knowledge_bases dict
                    for kb_id, kb_data in kb_docs.items():
                        if kb_id not in knowledge_bases:
                            knowledge_bases[kb_id] = {
                                ...
                                "name": kb_id,
                                "description": "",
                                "document_count": len(kb_data["docs"]),
                                "created_at": kb_data.get("created_at"),
                            }
                        else:
                            # Merge document counts
                            knowledge_bases[kb_id]["document_count"] += len(kb_data["docs"])
