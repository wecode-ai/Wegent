...
async def index_document_local(
    ...
    *,
    ...
    preparation = await run_knowledge_io(
        _prepare_index_document_with_owned_session, spec
    )
...
def _prepare_index_document_with_owned_session(
    spec: IndexRuntimeSpec,
...
async def purge_knowledge_index_local(
    ...
    *,
    ...
    del db
    return await run_knowledge_io(_purge_knowledge_index_sync, spec)


def _purge_knowledge_index_sync(spec: PurgeKnowledgeRuntimeSpec) -> dict:
    """Build the storage client and purge it in one bounded worker."""

    storage_backend = create_storage_backend_from_runtime_config(spec.retriever_config)
    return storage_backend.delete_knowledge(
        knowledge_id=str(spec.knowledge_base_id),
        user_id=spec.index_owner_user_id,
    )
...
async def drop_knowledge_index_local(
    ...
    *,
    ...
    del db
    return await run_knowledge_io(_drop_knowledge_index_sync, spec)


def _drop_knowledge_index_sync(spec: DropKnowledgeIndexRuntimeSpec) -> dict:
    """Build the storage client and drop it in one bounded worker."""

    storage_backend = create_storage_backend_from_runtime_config(spec.retriever_config)
    return storage_backend.drop_knowledge_index(
        knowledge_id=str(spec.knowledge_base_id),
        user_id=spec.index_owner_user_id,
    )
