...
def resolve_env_placeholder(value: str) -> str:
    ...
    def replace_env(match):
        ...
        if env_value:
            logger.info(f"[model_resolver] Resolved env var ${{{env_var}}} to value (length={len(env_value)})")
        else:
            logger.warning(f"[model_resolver] Env var ${{{env_var}}} not found or empty")
        return env_value
            ...
            try:
                ...
                if isinstance(executor_env, dict):
                    ...
                    if dh:
                        ...
                        logger.info(f"[model_resolver] Loaded DEFAULT_HEADERS from EXECUTOR_ENV: keys={list(default_headers.keys())}")
        ...
        if not default_headers:
            ...
            if dh_str:
                try:
                    ...
                    logger.info(f"[model_resolver] Loaded DEFAULT_HEADERS from env: keys={list(default_headers.keys())}")
                except json.JSONDecodeError as e:
                    logger.warning(f"[model_resolver] Failed to parse DEFAULT_HEADERS env: {e}")
    ...
    try:
        ...
        for k, v in default_headers.items():
            if isinstance(v, str):
                ...
                result_headers[k] = v
        logger.info(f"Built default_headers with placeholders: keys={list(result_headers.keys())}")
    except Exception as e:
...
def _extract_model_config(model_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    logger.info(f"[model_resolver] _extract_model_config: model_spec keys = {list(model_spec.keys())}")
    
    model_config = model_spec.get("modelConfig", {})
    logger.info(f"[model_resolver] _extract_model_config: modelConfig keys = {list(model_config.keys()) if model_config else 'empty'}")
    
    env = model_config.get("env", {})
    logger.info(f"[model_resolver] _extract_model_config: env keys = {list(env.keys()) if env else 'empty'}")
    ...
    if api_key and "${" in api_key:
        logger.info(f"[model_resolver] api_key contains placeholder, resolving from env...")
    ...
    if isinstance(default_headers, str):
        try:
            ...
            logger.warning(f"Failed to parse DEFAULT_HEADERS as JSON: {default_headers}")
            default_headers = {}
    
    logger.info(f"[model_resolver] _extract_model_config: DEFAULT_HEADERS keys = {list(default_headers.keys()) if default_headers else 'empty'}")
    
    # Log extracted values (mask API key)
    masked_key = f"{api_key[:8]}...{api_key[-4:]}" if api_key and len(api_key) > 12 else ("***" if api_key else "EMPTY")
    logger.info(f"[model_resolver] _extract_model_config: api_key={masked_key}, base_url={base_url}, model_id={model_id}, model_type={model_type}")
    
    # Decrypt API key if encrypted (only if it doesn't look like a placeholder)
    if api_key and "${" not in api_key:
        try:
            decrypted_key = decrypt_api_key(api_key)
            masked_decrypted = f"{decrypted_key[:8]}...{decrypted_key[-4:]}" if decrypted_key and len(decrypted_key) > 12 else ("***" if decrypted_key else "EMPTY")
            logger.info(f"[model_resolver] _extract_model_config: decrypted api_key={masked_decrypted}")
