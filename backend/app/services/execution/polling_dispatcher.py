...
"""
Polling dispatcher for long-running async API tasks.

Handles Gemini Deep Research which requires:
...
without going through Chat Shell.
"""
...
async def dispatch_polling(
    ...
    Calls the Gemini Interaction API directly.
    ...
    try:
        ...
        logger.info(
            f"[PollingDispatcher] Creating job: "
        ...
        logger.info(
            f"[PollingDispatcher] Completed: task_id={task_id}, "
...
async def _fetch_thought_summaries(
    ...
    try:
        ...
        logger.warning(f"[PollingDispatcher] Thought fetch error: {e}")
    return thought_summaries
