...
class DeviceNamespace(socketio.AsyncNamespace):
    ...
    async def on_task_progress(self, sid: str, data: dict) -> dict:
        ...
        try:
            ...
            # Handle result with delta calculation for streaming
            ...
            if "result" in data and data["result"] is not None:
                ...
                # Calculate delta content (only new characters since last emit)
                delta_content = full_content[last_offset:]
                new_offset = len(full_content)

                # Update subtask.result, preserving internal tracking fields
                subtask.result = {
                    "value": full_content,
                ...
                # Emit incremental content to frontend if there's new content
                if ws_emitter and delta_content:
    ...
    async def on_task_complete(self, sid: str, data: dict) -> dict:
        ...
        try:
            ...
            # Update task status using the unified update_task method
            # This ensures consistent status update logic across executor and device modes
            ...
            # Note: Device status is not reset here to allow concurrent tasks
            # Device manages its own status via device:status events
