...
class DeviceNamespace(socketio.AsyncNamespace):
    ...
    async def _match_cloud_device_by_ip(
        self, user_id: int, client_ip: str, executor_device_id: str
    ) -> Optional[str]:
        """Match cloud device by IP address and update its device_id.

        When a cloud device executor connects, it uses a self-generated device_id.
        ...
        Args:
            ...
            executor_device_id: Device ID generated by executor
        ...
        try:
            with _db_session() as db:
                ...
                logger.info(
                    f"[Device WS] Cloud device IP matching: "
                ...
                for device in cloud_devices:
                    ...
                    # Get sandbox IP from Nevis API
                    try:
                        nevis_status = await cloud_device_provider.get_nevis_status(
                            sandbox_id
                        )
                        sandbox_ip = nevis_status.get("ip_address")

                        logger.info(
                            f"[Device WS] Comparing IPs: "
                            f"sandbox_id={sandbox_id}, "
                            f"sandbox_ip={sandbox_ip}, client_ip={client_ip}"
                        )

                        if sandbox_ip and sandbox_ip == client_ip:
                            # Found matching cloud device, update its device_id
                            logger.info(
                                f"[Device WS] Cloud device IP match: "
                                f"sandbox_id={sandbox_id}, ip={client_ip}, "
    ...
    async def on_device_register(self, sid: str, data: dict) -> dict:
        ...
        if client_ip:
            cloud_device_id = await self._match_cloud_device_by_ip(
