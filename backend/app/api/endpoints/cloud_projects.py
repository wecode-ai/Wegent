...
@router.patch(
    "/{project_id}/local-bindings/{binding_id}",
    response_model=LocalBindingResponse,
)
def update_local_binding(
    ...
    binding = cloud_project_service.update_local_binding(
        db, project_id, binding_id, current_user.id, values
    )
    return LocalBindingResponse.model_validate(binding)
...
def delete_local_binding(
    ...
    cloud_project_service.delete_local_binding(
        db, project_id, binding_id, current_user.id
    )
