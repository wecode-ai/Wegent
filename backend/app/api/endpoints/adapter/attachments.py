...
This module uses the unified ContextService internally while maintaining
backward-compatible API responses for attachments.
"""
...
from app.services.context.context_service import ContextNotFoundError, context_service
...
async def download_attachment(
    ...
    if binary_data is None:
        logger.error(
            f"Failed to retrieve binary data for context {attachment_id}, "
...
@router.get("/subtask/{subtask_id}", response_model=List[AttachmentDetailResponse])
async def get_attachments_by_subtask(
    subtask_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(security.get_current_user),
):
    """
    Get attachments by subtask ID.

    Returns:
        List of attachment details for the subtask
    """
    contexts = context_service.get_attachments_by_subtask(
        db=db,
    ...
    # Verify ownership for each context
    result = []
    for context in contexts:
        if context.user_id == current_user.id:
            result.append(AttachmentDetailResponse.from_context(context))
