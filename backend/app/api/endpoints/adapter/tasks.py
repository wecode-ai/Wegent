...
async def get_workspace_files(
    ...
    This endpoint returns a tree structure of files generated by the task
    in its executor container. Used for "no repository" mode to display
    AI-generated files.

    Returns:
        - files: List of WorkspaceFile objects (tree structure)
        - total_count: Total number of files
        - truncated: True if file count exceeds limit

    Error codes in response:
        - container_not_found: Executor container not found
        - container_stopped: Executor container is not running
        - timeout: Request timed out
        - connection_error: Network error
    """
    from app.models.task import TaskResource
    from app.services.task_member_service import task_member_service
    from app.services.workspace_files import get_workspace_files_service
    ...
    # Get workspace files from executor container
    service = get_workspace_files_service()
    result, error = await service.list_workspace_files(task_id)
...
async def download_workspace_files(
    ...
    """
    Download all files in the task's workspace as a ZIP archive.

    This endpoint packages all files generated by the task in its executor
    container into a ZIP file for download.

    Returns:
        StreamingResponse with ZIP file (application/zip)

    Raises:
        404: Task not found or no access
        503: Executor container not available
    """
    from app.models.task import TaskResource
    from app.services.task_member_service import task_member_service
    from app.services.workspace_files import get_workspace_files_service
    ...
    # Download workspace files from executor container
    service = get_workspace_files_service()
    zip_bytes, filename, error = await service.download_workspace_zip(task_id)

    if error:
        error_messages = {
            "container_not_found": "Executor container not found",
            "container_stopped": "Executor container is not running",
            "directory_not_found": "Workspace directory not found",
            "download_failed": "Failed to download files",
            "timeout": "Request timed out",
            "connection_error": "Network error",
        }
        raise HTTPException(
            status_code=503,
            detail=error_messages.get(error, f"Error: {error}"),
        )
