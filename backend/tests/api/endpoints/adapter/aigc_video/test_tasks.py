...
from app.api.endpoints.adapter.aigc_video.client import AigcCardStatus
from app.api.endpoints.adapter.aigc_video.polling import (
    AIGC_CARD_POLL_CONTEXTS_KEY,
    prepare_poll_context,
)
from app.api.endpoints.adapter.aigc_video.tasks import (
...
def test_poll_card_status_persists_completed_card(monkeypatch):
    captured = {}
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.tasks.claim_poll",
        lambda **_kwargs: True,
    )
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.tasks.finish_poll",
        lambda **kwargs: captured.update(finished=kwargs),
    )
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.tasks.fetch_card_status",
    ...
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.tasks._persist_card", persist
    )
...
def test_poll_card_status_persists_progress_and_schedules_next(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.tasks._schedule_next", schedule
    )
...
def test_dispatch_persists_context_before_queueing(monkeypatch):
    events = []
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.tasks.create_poll_token",
        lambda: "poll-token",
    )
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.tasks.persist_poll_context",
...
def test_prepare_poll_context_preserves_other_cards(monkeypatch):
    ...
    captured = {}
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.polling.subtask_store.get_by_id",
        lambda _db, **_kwargs: subtask,
    )
    monkeypatch.setattr(
        "app.api.endpoints.adapter.aigc_video.polling.subtask_store.update_result",
