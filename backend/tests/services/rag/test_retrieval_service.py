...
class TestGetOriginalDocumentsFromKnowledgeBase:
    ...
    async def test_query_by_document_ids_only(self):
        """Should support querying by document IDs without knowledge_base_id."""
        from app.services.rag.retrieval_service import RetrievalService
...
class TestDirectInjectionTruncationRejection:
    ...
    @pytest.mark.asyncio
    async def test_truncated_documents_reject_entire_direct_injection(self):
        """If any KB has truncated documents, entire direct_injection should be rejected."""
        from app.services.rag.retrieval_service import RetrievalService
        ...
        # Should fallback to RAG because second KB has truncated documents
        assert result["mode"] == "rag_retrieval"
        service.retrieve_from_knowledge_base_internal.assert_awaited()
