...
"""Tests for DingTalkWorkspaceService — two-phase KB sync."""
...
from app.services.dingtalk_workspace_service import (
...
class TestListWikiSpaces:
    """Tests for DingTalkWorkspaceService._list_wiki_spaces."""
    ...
    async def test_returns_knowledge_bases_from_items_key(self) -> None:
        ...
        with patch.object(
            DingTalkWorkspaceService,
            "_list_wiki_spaces",
            wraps=DingTalkWorkspaceService._list_wiki_spaces,
        ):
            # Patch the lazy imports used inside _list_wiki_spaces
            with patch(
                "mcp.client.streamable_http.streamablehttp_client"
            ) as mock_http, patch("mcp.ClientSession") as mock_cls:
                ...
                with patch(
                    "app.services.dingtalk_workspace_service.streamablehttp_client",
                    mock_http,
                    create=True,
                ), patch(
                    "app.services.dingtalk_workspace_service.ClientSession",
                    mock_cls,
                    create=True,
                ):
...
# ---------------------------------------------------------------------------
# _fetch_all_workspace_nodes
# ---------------------------------------------------------------------------


class TestFetchAllWorkspaceNodes:
    """Tests for DingTalkWorkspaceService._fetch_all_workspace_nodes."""
    ...
    async def test_returns_empty_when_no_kbs(self) -> None:
        ...
        with patch.object(
            ...
            result = await DingTalkWorkspaceService._fetch_all_workspace_nodes(
                workspace_mcp_url="https://ws.mcp.example.com",
    ...
    @pytest.mark.asyncio
    async def test_uses_workspace_mcp_url_as_docs_fallback(self) -> None:
        """Falls back to workspace MCP URL when docs MCP URL is not configured."""
        kb_nodes = [{"workspaceId": "WS1", "name": "KB 1"}]
        captured_urls: list[str] = []

        async def capture_url(docs_mcp_url: str, workspace_id: str, all_nodes: list) -> None:
        ...
        with patch.object(
            ...
            await DingTalkWorkspaceService._fetch_all_workspace_nodes(
                workspace_mcp_url="https://ws.mcp.example.com",
    ...
    async def test_skips_kb_with_no_workspace_id(self) -> None:
        ...
        async def track_call(docs_mcp_url: str, workspace_id: str, all_nodes: list) -> None:
    ...
    async def test_continues_after_kb_error(self) -> None:
        ...
        async def maybe_fail(docs_mcp_url: str, workspace_id: str, all_nodes: list) -> None:
            nonlocal call_count
            call_count += 1
...
# ---------------------------------------------------------------------------
# sync_workspace_nodes (integration-style with mocked MCP)
# ---------------------------------------------------------------------------


class TestSyncWorkspaceNodes:
    """Tests for DingTalkWorkspaceService.sync_workspace_nodes."""

    @pytest.mark.asyncio
    @patch("app.services.dingtalk_workspace_service.UserMCPService")
    @patch("app.services.dingtalk_workspace_service.DingTalkDocService")
    async def test_raises_when_not_configured(
        self, mock_doc_service: MagicMock, mock_mcp_svc: MagicMock
    ) -> None:
        """Raises ValueError when workspace MCP URL is not configured."""
        mock_mcp_svc.get_provider_service_config.return_value = {"enabled": False}
        mock_user = MagicMock()
        mock_db = MagicMock()

        with pytest.raises(ValueError, match="not configured"):
            await DingTalkWorkspaceService.sync_workspace_nodes(mock_user, mock_db)
    ...
    async def test_uses_docs_mcp_url_for_list_nodes(
        ...
        """Passes the docs MCP URL to _fetch_all_workspace_nodes."""
        ...
        async def fake_fetch(
            ...
            captured["workspace_url"] = workspace_mcp_url
        ...
        with patch.object(
            ...
            "app.services.dingtalk_workspace_service.DingTalkDocService"
        ...
        assert captured["workspace_url"] == "https://ws.mcp.example.com"
