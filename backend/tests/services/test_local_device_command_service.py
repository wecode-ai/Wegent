...
def test_local_device_command_registry_default_includes_diagnostic_commands():
    ...
    list_directories_definition = resolve_local_device_command(
        "list_directories", settings.LOCAL_DEVICE_COMMANDS
    )
    ...
    assert list_directories_definition is not None
    assert list_directories_definition.command == "ls -1Ap"
    assert list_directories_definition.post_processor == "directory_list"
...
def test_directory_list_post_processor_keeps_only_directories():
    """directory_list post processor should return only directory names."""
    from app.services.device.command_post_processor import apply_command_post_processor
    ...
    assert processed["stdout"] == ["backend", "docs"]
