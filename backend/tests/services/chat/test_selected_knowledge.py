...
@pytest.mark.parametrize("shell_type", ["Chat", "Codex", "ClaudeCode"])
...
@pytest.mark.parametrize("shell_type", ["Codex", "ClaudeCode"])
def test_activate_provider_native_knowledge_requires_coding_agent_mcp_mount(
    shell_type: str,
) -> None:
    request = ExecutionRequest(
        bot=[{"shell_type": shell_type, "mcp_servers": []}],
...
@pytest.mark.parametrize("shell_type", ["Codex", "ClaudeCode"])
def test_activate_provider_native_knowledge_enables_coding_agent_after_mount(
    shell_type: str,
) -> None:
    request = ExecutionRequest(
        bot=[
