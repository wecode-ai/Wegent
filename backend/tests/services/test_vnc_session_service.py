...
from app.services.device.vnc_session_service import (
...
async def test_vnc_session_service_returns_only_a_one_time_proxy_ticket(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.device.vnc_session_service.resolve_runtime_route_identity",
        lambda *_args, **_kwargs: identity,
    )
    monkeypatch.setattr(
        "app.services.device.vnc_session_service.settings.WEGENT_SOCKET_URL",
    ...
    assert "upstream_url" not in record.to_dict()
    assert "runtime-secret" not in str(record.to_dict())
...
def test_vnc_provider_registry_rejects_remote_devices():
    registry = VncSessionProviderRegistry()

    with pytest.raises(ValueError, match="cloud devices only"):
        registry.register(DeviceType.REMOTE, PreparedProvider())
