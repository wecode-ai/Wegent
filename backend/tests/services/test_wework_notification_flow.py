...
async def test_mentioning_a_member_pushes_the_comment_to_dingtalk(
    ...
    headline = f"mentioner 在「{item.title}」提到了你"
    assert FakeDingTalkSender.sent == [
        {
            ...
            "text": (
                f"**{headline}**\n\n"
                "@recipient 这个改动麻烦你确认下\n\n"
                f"看板：{project.name}\n\n"
                f"[查看详情]({notification.url})"
            ),
        }
    ]
...
async def test_execution_lifecycle_pushes_start_blocked_and_end(
    ...
    assert pushed["queued"]["title"] == f"「{item.title}」已开始执行"
    assert pushed["pending_approval"]["title"] == f"「{item.title}」等待你审批"
    assert pushed["completed"]["title"] == f"「{item.title}」已完成"
