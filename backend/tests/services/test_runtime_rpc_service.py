...
@pytest.mark.asyncio
async def test_runtime_rpc_service_uses_public_backend_for_remote_cloud_models(
    monkeypatch,
):
    from app.services.device import runtime_rpc_service as module
    ...
    monkeypatch.setattr(
        module,
        "_load_remote_runtime_proxy_url",
        lambda _user_id: "socks5://proxy.internal:7890",
    )
    monkeypatch.setattr(
        module.settings,
        ...
        payload={
            "executionRequest": {
                "model_config": {
                    ...
                    "base_url": "http://127.0.0.1:8000/api/runtime-work/llm-responses-proxy",
                    "api_key": "desktop-token",
                    "vision_sidecar": {
                        "request_url": "http://127.0.0.1:8000/api/runtime-work/llm-responses-proxy/responses",
                    },
                }
            },
            "friendlyTitleExecutionRequest": {
                "model_config": {
                    ...
                    "base_url": "http://127.0.0.1:13019/v1/codex-router/title",
                }
            },
        },
    )
    ...
    assert cloud_model["base_url"] == (
        "https://backend.example.com/root/api/runtime-work/llm-responses-proxy"
    )
    assert cloud_model["vision_sidecar"]["request_url"] == (
        "https://backend.example.com/root/api/runtime-work/llm-responses-proxy/responses"
    )
    ...
    assert emitted["friendlyTitleExecutionRequest"]["model_config"]["base_url"] == (
        "http://127.0.0.1:13019/v1/codex-router/title"
    )
    assert emitted["friendlyTitleExecutionRequest"]["model_config"]["proxy"] == {
        "url": "socks5://proxy.internal:7890"
    }
