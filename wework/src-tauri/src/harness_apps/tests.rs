...
#[test]
fn runtime_catalog_requires_http_transport_and_integrity_metadata() {
    let directory = tempdir().unwrap();
    fs::write(
...
#[test]
fn runtime_catalog_accepts_http_downloads() {
    let directory = tempdir().unwrap();
    fs::write(
        directory.path().join(BUNDLED_RUNTIME_CATALOG),
        serde_json::to_vec(&serde_json::json!({
    ...
    assert!(read_runtime_catalog(directory.path()).is_ok());
}

#[test]
fn runtime_catalog_rejects_unsupported_download_protocols() {
    let directory = tempdir().unwrap();
    fs::write(
        directory.path().join(BUNDLED_RUNTIME_CATALOG),
        serde_json::to_vec(&serde_json::json!({
