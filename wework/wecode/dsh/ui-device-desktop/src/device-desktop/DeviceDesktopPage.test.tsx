...
vi.mock('@/features/workbench/useWorkbench', () => ({
  useWorkbench: () => ({
    services: {
      ...
      workspaceSessionApi: {
        startDeviceExtensionSession: startDeviceVncMock,
        revokeDeviceExtensionSession: revokeDeviceVncMock,
      },
    },
...
describe('DeviceDesktopPage', () => {
  ...
  test('starts the session inside the isolated Electron child route', async () => {
    ...
    await waitFor(() => expect(startDeviceVncMock).toHaveBeenCalledWith('device-1', 'vnc'))
  ...
  test('treats a cloud device as VNC-capable without an advertised desktop feature', async () => {
    workbenchState.devices = [
      desktopDevice({ device_type: 'cloud', runtime_features: { schemaVersion: 4 } }),
    ]
    startDeviceVncMock.mockResolvedValue({
      session_id: 'vnc-session-cloud-default',
      device_id: 'device-1',
      type: 'vnc',
      path: '/',
      url: 'wss://cloud.example.com/vnc-proxy/sessions/vnc-session-cloud-default?ticket=single-use',
      transport: 'websocket',
    })
  ...
  test('uses the cloud route clipboard bridge from a merged local device record', async () => {
    ...
    await waitFor(() => expect(startDeviceVncMock).toHaveBeenCalledWith('cloud-device', 'vnc'))
  ...
  test('rejects legacy VNC URLs that expose a bearer token', async () => {
    ...
    expect(revokeDeviceVncMock).toHaveBeenCalledWith('vnc', 'vnc-session-legacy')
  })
  ...
  test('revokes a session that resolves after the page unmounts', async () => {
    ...
    await waitFor(() => expect(revokeDeviceVncMock).toHaveBeenCalledWith('vnc', 'vnc-session-late'))
  })
