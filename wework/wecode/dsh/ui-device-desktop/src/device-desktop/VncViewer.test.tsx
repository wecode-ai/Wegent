...
describe('VncViewer', () => {
  ...
  beforeEach(() => {
    ...
    invokeDesktopHostMock.mockImplementation(async capability => {
      if (capability === 'isolatedClipboard.readText') return 'native clipboard'
      return undefined
    })
  })
  ...
  test('connects noVNC with only the websocket URL and configures the viewport', async () => {
    ...
    expect(invokeDesktopHostMock).toHaveBeenCalledWith('isolatedClipboard.activate', {
  ...
  test('bridges remote and native clipboard text through the active Electron lease', async () => {
    ...
    await waitFor(() =>
      expect(invokeDesktopHostMock).toHaveBeenCalledWith('isolatedClipboard.writeText', {
        leaseId: expect.any(String),
        text: 'remote text',
      })
    )
    ...
    expect(invokeDesktopHostMock).toHaveBeenCalledWith('isolatedClipboard.readText', {
      leaseId: expect.any(String),
    })
  ...
  test('retries the Electron clipboard lease when the window regains focus', async () => {
    ...
    invokeDesktopHostMock.mockImplementation(async capability => {
      if (capability === 'isolatedClipboard.activate') {
  ...
  test('retries the Electron clipboard lease from the toolbar action', async () => {
    ...
    invokeDesktopHostMock.mockImplementation(async capability => {
      ...
      if (capability === 'isolatedClipboard.readText') return 'clipboard after focus'
      return undefined
    })
    render(<VncViewer websocketUrl="ws://127.0.0.1/session/websockify?token=secret" />)
    const rfb = rfbState.instances[0]
