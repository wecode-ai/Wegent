...
async function verifyAutomationLifecycle(control, executorHome, homePath) {
  ...
  assert.deepEqual(
    ...
    'Wework persisted model routing headers that should be generated by the Executor'
  )
  ...
  try {
    ...
    const scheduledModelRequest = await control.awaitScenarioRequestCount(
      'automation',
      4,
      AUTOMATION_SCHEDULE_TIMEOUT_MS
    )
    assertModelAttributionHeaders(scheduledModelRequest.headers, {
      executor: 'codex',
      source: 'wegent-app',
      taskSource: 'unknown',
    })
