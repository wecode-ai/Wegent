...
function InlineDeviceDesktopPage({ deviceId }: { deviceId: string }) {
  ...
  const revokeDeviceVnc = services.workspaceSessionApi?.revokeDeviceVnc
  ...
  const clipboardBridge = useMemo(
    () =>
      device?.device_type === 'cloud'
        ...
        : undefined,
    [device?.device_type, deviceId, services.deviceApi]
  )
  const desktopAvailable = !device || supportsVncDesktop(device, deviceId)
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      void (async () => {
        try {
          ...
          if (!active) {
            await revokeDeviceVnc?.(session.session_id).catch(() => undefined)
          ...
          if (createdSessionId && revokeDeviceVnc) {
            await revokeDeviceVnc(createdSessionId).catch(() => undefined)
            createdSessionId = null
          }
    ...
    return () => {
      active = false
      if (createdSessionId && revokeDeviceVnc) {
        void revokeDeviceVnc(createdSessionId).catch(() => undefined)
        createdSessionId = null
      }
    }
