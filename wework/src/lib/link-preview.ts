...
function getFaviconUrl(parsed: URL): string {
  const domain = normalizeHostname(parsed.hostname) ?? parsed.hostname
...
export function extractFirstLink(text: string): LinkPreview | undefined {
  ...
  try {
    ...
    return { url: parsed.toString(), domain, displayUrl, iconUrl: getFaviconUrl(parsed) }
  } catch {
    return undefined
  }
}
