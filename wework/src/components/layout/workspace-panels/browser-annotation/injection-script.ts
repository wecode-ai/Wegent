...
(() => {
  ...
  const svgIcon = (path, size, stroke = false) => '<svg width="' + size + '" height="' + size + '" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="' + path + '" ' + (stroke ? 'stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round"' : 'fill="currentColor"') + '/></svg>';
  const icons = {
  ...
  const showEditor = (element, annotation) => {
    ...
    const adjust = document.createElement('button'); adjust.type = 'button'; adjust.dataset.weworkAnnotation = 'adjust-toggle'; adjust.title = config.strings.adjust; adjust.setAttribute('aria-label', config.strings.adjust); adjust.innerHTML = svgIcon(icons.adjust, 12); style(adjust, { position: 'absolute', top: '12px', left: '12px', width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '0', borderRadius: '6px', background: 'transparent', color: '#5d5d5d', cursor: 'pointer', transition: 'background .18s ease', zIndex: '2' });
    ...
    const submit = isEdit ? null : document.createElement('button'); if (submit) { submit.type = 'button'; submit.dataset.weworkAnnotation = 'submit'; submit.title = config.strings.add; submit.setAttribute('aria-label', config.strings.add); submit.innerHTML = svgIcon(icons.check, 14); style(submit, { position: 'absolute', right: '10px', bottom: '10px', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '0', borderRadius: '999px', background: '#171717', color: 'white', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,.2)', zIndex: '1', opacity: '0.4' }); }
    ...
    const remove = isEdit ? document.createElement('button') : null; if (remove) { remove.type = 'button'; remove.dataset.weworkAnnotation = 'delete'; remove.title = config.strings.delete; remove.setAttribute('aria-label', config.strings.delete); remove.innerHTML = svgIcon(icons.trash, 14); style(remove, { display: 'flex', width: '20px', height: '20px', alignItems: 'center', justifyContent: 'center', border: '0', borderRadius: '6px', background: 'transparent', color: '#9ca3af', cursor: 'pointer' }); }
  ...
  const detach = () => { if (!state.attached) return; closeEditor(); clearHover(); document.removeEventListener('mousemove', onMouseMove, true); document.removeEventListener('click', onClick, true); document.removeEventListener('scroll', schedulePositionUpdate, true); window.removeEventListener('resize', schedulePositionUpdate); window.visualViewport?.removeEventListener('resize', schedulePositionUpdate); window.visualViewport?.removeEventListener('scroll', schedulePositionUpdate); state.resizeObserver?.disconnect(); state.resizeObserver = null; if (state.animationFrame !== null) window.cancelAnimationFrame(state.animationFrame); state.animationFrame = null; state.layer?.remove(); state.layer = null; state.annotations.forEach(annotation => { annotation.marker = null; }); state.attached = false; };
