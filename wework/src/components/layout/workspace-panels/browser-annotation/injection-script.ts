...
(() => {
  ...
  const htmlFor = element => { const html = element.outerHTML || ''; return html.length > 2000 ? html.slice(0, 1999) + '…' : html; };
  const targetFor = element => ({ tagName: element.tagName.toLowerCase(), html: htmlFor(element), text: textFor(element), isSimpleText: simpleTextTarget(element), role: element.getAttribute('role') || undefined, name: textFor(element, 120), rect: rectFor(element) });
  ...
  const showEditor = (element, annotation, clickPoint) => {
    ...
    editor.addEventListener('focusin', event => event.stopPropagation());
