...
export function findComposerMarkdownLinks(value: string) {
  const links: { url: string; start: number; end: number; textStart: number; textEnd: number }[] =
    []
  const visit = (node: RootContent | PhrasingContent) => {
    if (node.type === 'link' && isHttpUrl(node.url) && node.position) {
      ...
      links.push({
        ...
        textStart: explicit ? (node.children[0]?.position?.start.offset ?? start) : start,
...
export function parseComposerMarkdown(
  ...
  const isBareLink = (node: PhrasingContent) => node.type === 'link' && !/^[<[]/.test(raw(node))
  const inline = (nodes: PhrasingContent[], marks: readonly Mark[] = []): PMNode[] =>
    nodes.flatMap(node => {
      const markName = (
        { strong: 'strong', emphasis: 'em', delete: 'strike' } as Record<string, string>
      )[node.type]
      ...
      if (node.type === 'link') {
        if (isBareLink(node)) {
          ...
          plainDocument(raw(node)).firstChild?.forEach(child =>
            content.push(
              child.mark(
                child.isText
                  ? [...marks, schema.marks.link.create({ href: node.url, autolink: true })]
                  : marks
              )
            )
          )
        ...
        if (chips?.childCount === 1 && chips.firstChild?.isAtom && !chips.firstChild.isText) {
      ...
      plainDocument(text).forEach((paragraph, _offset, index) => {
        ...
        paragraph.forEach(child => result.push(child.mark([...marks, ...child.marks])))
      })
      return result
    })
  const block = (node: RootContent): PMNode => {
    switch (node.type) {
      ...
      case 'listItem':
        return schema.node('list_item', { checked: node.checked ?? null }, node.children.map(block))
      case 'table':
        return schema.node(
          'table',
          null,
            ...
            schema.node(
              ...
              row.children.map((cell, column) =>
                schema.node(
                  ...
                  schema.node('paragraph', { markdown: true }, inline(cell.children))
                )
              )
            )
          )
      ...
      default:
        return schema.node('paragraph', null, raw(node) ? schema.text(raw(node)) : undefined)
    }
  }
