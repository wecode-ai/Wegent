...
const linkKey = new PluginKey<ReturnType<typeof buildTextLinks>>('composer-text-links')
...
export function openComposerTextLink(
  ...
  if (event instanceof KeyboardEvent && !['Enter', ' '].includes(event.key)) return false
  if (
  ...
  const link = linkKey
    .getState(view.state)
    ?.links.find(link => String(link.start) === anchor.dataset.composerTextLink)
  if (!link) return false
  event.preventDefault()
  event.stopPropagation()
  onOpen({ url: link.url, label: link.label, iconUrl: '', provider: '' }, anchor, {
    start: link.start,
    end: link.end,
  })
  return true
