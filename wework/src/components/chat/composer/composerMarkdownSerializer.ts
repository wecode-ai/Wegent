...
function inlineMarks(fragment: Fragment, index: number): readonly Mark[] {
  return fragment
    ...
    .map(mark => {
      ...
      while (to < fragment.childCount && mark.isInSet(fragment.child(to).marks)) to++
      let label = ''
      for (let i = from; i < to; i++) label += fragment.child(i).textContent
      const before = from > 0 ? fragment.child(from - 1).textContent : ''
      const after = to < fragment.childCount ? fragment.child(to).textContent : ''
      return /\S$/.test(before) || /^\S/.test(after) || label !== mark.attrs.href
...
export function serializedOffsetFromPosition(doc: PMNode, position: number): number {
...
class MarkdownWriter {
  ...
  block(node: PMNode, position: number, inCell = false) {
    ...
    switch (node.type.name) {
      case 'paragraph':
      case 'heading':
        if (node.type.name === 'heading') this.emit(`${'#'.repeat(node.attrs.level)} `)
        this.inline(
          node.content,
          position + 1,
          inCell,
      ...
      case 'code_block': {
        const fence = '`'.repeat(
          Math.max(3, ...Array.from(node.textContent.matchAll(/`+/g), match => match[0].length + 1))
        )
      ...
      case 'ordered_list':
        node.forEach((item, offset, index) => {
          ...
          const marker =
            node.type.name === 'ordered_list' ? `${Number(node.attrs.start) + index}. ` : '- '
          this.emit(marker)
          if (item.attrs.checked !== null) this.emit(item.attrs.checked ? '[x] ' : '[ ] ')
          const prefix = this.prefix
  ...
  inline(fragment: Fragment, start: number, inCell: boolean, escape: boolean) {
    ...
    fragment.forEach((node, offset, index) => {
      ...
      for (let index = shared; index < marks.length; index++) this.mark(marks[index], true, node)
      active = marks
      this.inlineNode(node.mark(marks), start + offset, inCell, escape)
    })
    for (let index = active.length - 1; index >= 0; index--) this.mark(active[index], false)
      ...
      case 'code':
        if (open) {
          ...
          this.codeFence = '`'.repeat(
            Math.max(1, ...Array.from(text.matchAll(/`+/g), match => match[0].length + 1))
          )
          this.codePadding = /^`|`$|^ .+ $/.test(text) ? ' ' : ''
        }
        this.emit(open ? this.codeFence + this.codePadding : this.codePadding + this.codeFence)
        break
      case 'link':
  ...
  inlineNode(node: PMNode, position: number, inCell: boolean, escape: boolean) {
    this.at(position)
    if (node.isText) {
      const literal = node.marks.some(mark => mark.type.name === 'code' || mark.attrs.autolink)
