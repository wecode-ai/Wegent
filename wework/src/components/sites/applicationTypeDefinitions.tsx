...
const siteDefinition: ApplicationTypeDefinition = {
  ...
  columns: [
    ...
    { key: 'external_column', fallback: '外网发布' },
  ],
  ...
  isItem: item => item.app_type === 'site',
  renderRow: (item, context) =>
    item.app_type === 'site' ? <SiteApplicationRow site={item} context={context} /> : null,
}
...
const miniProgramDefinition: ApplicationTypeDefinition = {
  ...
  columnGridClassName: 'grid-cols-[minmax(0,1fr)_140px_120px]',
  create: {
  ...
  isItem: item => item.app_type === 'mini_program',
  renderRow: (item, context) =>
    item.app_type === 'mini_program' ? (
...
export const DEFAULT_APPLICATION_TYPE: SiteAppType = 'site'
