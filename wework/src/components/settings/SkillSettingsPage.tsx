...
export function SkillSettingsPage() {
  ...
  const loadCodexConfig = useCallback(async () => {
    ...
    try {
      const config = await localCodexApi.readCodexLocalConfig()
  ...
  const updateRemoteAppsEnabled = async (enabled: boolean) => {
    ...
    setCodexConfigError(null)
    try {
      const nextConfig = await localCodexApi.updateCodexLocalConfig({
        remoteAppsEnabled: enabled,
      })
          ...
          <div className="mt-5 rounded-lg border border-border bg-surface p-3">
            ...
            {(codexConfigLoading || codexConfigSaving) && (
              <div className="mt-3 flex items-center gap-2 text-xs text-text-muted">
                ...
                {codexConfigSaving
                  ? t('workbench.codex_plugin_config_saving')
                  : t('common.loading')}
              </div>
            )}
            {codexConfigError && (
