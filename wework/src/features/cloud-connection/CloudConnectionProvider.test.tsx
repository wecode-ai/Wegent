...
describe('CloudConnectionProvider', () => {
  ...
  function storeRecoverableConnection() {
    saveStoredCloudConnection({
      backendUrl: 'https://cloud.example.com',
      apiBaseUrl: 'https://cloud.example.com/api',
      socketBaseUrl: 'https://cloud.example.com',
  ...
  it('automatically recovers a transient startup credential failure', async () => {
    vi.useFakeTimers()
    try {
      storeRecoverableConnection()
      credentialMocks.refreshAccessToken.mockRejectedValueOnce(new Error('network offline'))
      render(
        <CloudConnectionProvider>
          <CloudSocketProbe />
        </CloudConnectionProvider>
  ...
  it('continues retrying if a scheduled access token refresh fails repeatedly', async () => {
    vi.useFakeTimers()
    try {
      storeRecoverableConnection()
      credentialMocks.refreshAccessToken.mockResolvedValueOnce({
        accessToken: tokenWithExp(Math.floor(Date.now() / 1000) + 360),
      })
      const view = render(
        <CloudConnectionProvider>
          <CloudSocketProbe />
      ...
      await act(() => vi.advanceTimersByTimeAsync(0))
      credentialMocks.refreshAccessToken.mockRejectedValueOnce(new Error('offline'))
      credentialMocks.refreshAccessToken.mockRejectedValueOnce(new Error('still offline'))
      ...
      expect(credentialMocks.refreshAccessToken).toHaveBeenCalledTimes(4)
      view.unmount()
    } finally {
      vi.useRealTimers()
    }
  ...
  it('retries restoring credentials immediately when the network returns', async () => {
    vi.useFakeTimers()
    try {
      storeRecoverableConnection()
      credentialMocks.refreshAccessToken.mockRejectedValueOnce(new Error('offline'))
  ...
  it('does not retry revoked credentials', async () => {
    vi.useFakeTimers()
    try {
      storeRecoverableConnection()
      credentialMocks.refreshAccessToken.mockRejectedValueOnce(
        new DesktopCloudCredentialError('cloud_auth_expired', 'revoked', 401)
      )
      render(
        <CloudConnectionProvider>
          <CloudSocketProbe />
  ...
  it('cancels pending recovery when the user disconnects', async () => {
    vi.useFakeTimers()
    try {
      storeRecoverableConnection()
      credentialMocks.refreshAccessToken.mockRejectedValueOnce(new Error('offline'))
      ...
      await act(() => vi.advanceTimersByTimeAsync(0))
      await act(async () => {
        screen.getByTestId('disconnect-cloud-button').click()
      })
