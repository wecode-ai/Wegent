...
export type AppUpdateErrorKind = 'network' | 'unsupported' | 'channel_migration' | 'generic'
...
function appUpdateErrorCode(kind: AppUpdateErrorKind, stage: AppUpdateErrorStage): string {
  ...
  if (kind === 'channel_migration') return 'APP_UPDATE_CHANNEL_MIGRATION_REQUIRED'
...
function sanitizeAppUpdateErrorDetail(message: string, kind: AppUpdateErrorKind): string | null {
  if (!message || kind === 'network' || kind === 'unsupported' || kind === 'channel_migration') {
    return null
  }
