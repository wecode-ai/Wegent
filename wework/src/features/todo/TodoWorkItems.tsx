    ...
    >
      ...
      <div className="space-y-2.5">
        {quickCreateOpen && (
          <div className="flex items-center rounded-lg border border-primary/40 bg-surface p-2 shadow-sm">
            <input
              ...
              className="h-8 min-w-0 flex-1 bg-transparent px-1 text-[12px] text-text-primary outline-none placeholder:text-text-muted"
            />
            <button
              type="button"
        ...
        <button
          ...
          onClick={() => setQuickCreateOpen(true)}
          className="flex h-[34px] w-full items-center gap-2 rounded-md px-2 text-[11px] text-[#7B848C] hover:bg-[#ECEEEF] hover:text-[#4E565E] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
        >
          <Plus className="h-3.5 w-3.5" />
              ...
              {quickCreateState === state && (
                <div className="border-b border-border bg-surface px-3 py-2">
                  <input
                    ...
                    placeholder={t('todo.quick_create_placeholder', '输入事项标题，回车创建')}
                    className="h-8 w-full rounded-md border border-primary/40 bg-background px-3 text-[11px] outline-none"
                  />
                </div>
              )}
              {stateItems.length === 0 ? (
...
function TodoCard({
  ...
  return (
    ...
    >
      ...
      {children.length > 0 && (
        <span className="flex min-w-0 items-center gap-1.5 text-[10px] text-text-secondary">
      ...
      {item.blocker && (
        <span className="line-clamp-2 rounded-md bg-destructive/10 px-2 py-1 text-[10px] text-destructive">
