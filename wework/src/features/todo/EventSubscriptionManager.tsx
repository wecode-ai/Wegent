import { Check, Copy, MoreHorizontal, Plus, RefreshCw, Trash2, Webhook, X } from 'lucide-react'
...
function emptyDraft(sources: ProjectEventSourceCatalogItem[]): SubscriptionDraft {
  return {
    name: '',
...
export function EventSubscriptionPicker({
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      managerRef.current?.scrollIntoView?.({ block: 'nearest', behavior: 'smooth' })
    })
    return () => window.cancelAnimationFrame(frame)
  }, [managerOpen])

  const selectableSubscriptions = subscriptions.filter(item => item.status === 'active')
  ...
  return (
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-2">
      <label className={automationClass('panel-field')}>
        ...
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-2">
          ...
          {showManager ? (
            ...
            >
              {managerOpen ? <X className="h-4 w-4" /> : <Plus className="h-4 w-4" />}
              {managerOpen
      ...
      {managerOpen ? (
        <div ref={managerRef} className="grid min-w-0 grid-cols-[minmax(0,1fr)]">
...
function EventSubscriptionManager({
  ...
  const [draft, setDraft] = useState<SubscriptionDraft>(() => emptyDraft(sources))
  const [editorOpen, setEditorOpen] = useState(subscriptions.length === 0)
  const [busyId, setBusyId] = useState<string | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [copiedValue, setCopiedValue] = useState<string | null>(null)
  const draftSourceType = sources.some(source => source.sourceType === draft.sourceType)
    ? draft.sourceType
  ...
  async function createSubscription() {
    ...
    try {
      const subscription = await api.create(projectId, {
        name: draft.name.trim() || `${sourceName(draftSourceType)} subscription`,
  ...
  async function updateStatus(subscription: ProjectIncomingHook) {
    ...
    try {
      ...
      const next = subscriptions.map(item => (item.id === updated.id ? updated : item))
  ...
  async function rotateSubscription(subscription: ProjectIncomingHook) {
    ...
    try {
      ...
      onSubscriptionsChange(subscriptions.map(item => (item.id === updated.id ? updated : item)))
    } catch (cause) {
    ...
    >
      <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
        <div className="min-w-0">
          <strong className="text-sm font-medium">{t('todo.event_subscription_title')}</strong>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  selected ? 'bg-background ring-1 ring-border' : 'hover:bg-background/70'
                )}
              >
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    >
                      {selected ? <Check className="h-4 w-4" /> : <Webhook className="h-4 w-4" />}
                    </span>
