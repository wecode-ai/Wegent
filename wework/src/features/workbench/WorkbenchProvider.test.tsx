...
function ProjectChatProbe() {
  ...
  return (
    <div>
      ...
      <button
        type="button"
        onClick={() => projectChat.setSelectedModel(selectedModel)}
      >
        select model
      </button>
      <button
        type="button"
        onClick={() => projectChat.setSelectedSkills([selectedSkill])}
      >
        select skill
      </button>
      <button
        type="button"
        onClick={() => projectChat.addExistingAttachment(attachment)}
      >
        add attachment
      </button>
      <button
  ...
  return (
    <div>
      ...
      <span data-testid="retry-message-states">
        {workbench.messages
          .map(
            (message) => `${message.role}:${message.content}:${message.status}`,
          )
          .join("|")}
      </span>
    </div>
  ...
  return (
    <div>
      <span data-testid="message-contents">
        {workbench.messages
          .map(
            (message) => `${message.role}:${message.status}:${message.content}`,
          )
          .join("|")}
      </span>
      <button type="button" onClick={() => void workbench.openTask(8)}>
...
function ProjectSelectionProbe() {
  ...
  return (
    <div>
      ...
      <button
        type="button"
        onClick={() => workbench.selectStandaloneDevice("local-online")}
      >
    ...
    <div>
      ...
      <div data-testid="queued-states">
        {workbench.queuedMessages
          .map(
            (message) =>
              `${message.content}:${message.status}:${message.error ?? ""}`,
          )
          .join("|")}
      </div>
...
describe("WorkbenchProvider", () => {
      ...
      <WorkbenchProvider
        ...
        services={{
          ...
          skillApi: {
            ...
            getTeamSkills: vi
              .fn()
              .mockResolvedValue({ skills: [], preload_skills: [] }),
          },
          projectApi: {
  ...
  test("uses the opened task model as the runtime compatibility anchor", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("model-compatibility-status"),
      ).toHaveTextContent("gpt-5.5-medium:enabled"),
    );
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("model-compatibility-status"),
      ).toHaveTextContent("gpt-5.5-medium:runtime_family_mismatch"),
    );
        ...
        services={{
          ...
          chatStream: {
            ...
            subscribe: vi.fn((nextHandlers) => {
              handlers = nextHandlers as Record<
                string,
  ...
  test("restores an opened task from the taskId URL parameter after refresh", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("current-task-title")).toHaveTextContent(
        "restored chat",
      ),
    );
  ...
  test("sends project chat options for a new project conversation", async () => {
    const sendMessage = vi
      .fn()
      .mockResolvedValue({ success: true, task_id: 99 });
    const updateCurrentUser = vi.fn().mockResolvedValue({
      id: 1,
      user_name: "alice",
      email: "a@b.c",
    ...
    expect(
      screen.getByTestId("message-attachment-filenames"),
  ...
  test("blocks project task sending when the owning project device is missing", async () => {
    const sendMessage = vi
      .fn()
      .mockResolvedValue({ success: true, task_id: 71 });
  ...
  test("sends standalone chats to the preferred online cloud device", async () => {
    const sendMessage = vi
      .fn()
      .mockResolvedValue({ success: true, task_id: 100 });
    ...
    function StandaloneDeviceProbe() {
      ...
      return (
        <div>
          ...
          <button
            type="button"
            onClick={() => void workbench.sendCurrentInput()}
          >
            send
          </button>
        </div>
    ...
    function FollowUpProbe() {
      ...
      return (
        <div>
          ...
          <button
            type="button"
            onClick={() => workbench.setInput("我叫胡云鹏")}
          >
            set first input
          </button>
  ...
  test("keeps later guidance queued while one guidance send is in progress", async () => {
    ...
    const sendMessage = vi
      .fn()
      .mockResolvedValue({ success: true, task_id: 8 });
  ...
  test("opens task history in message order with normalized backend roles", async () => {
    ...
    expect(
      screen.getByTestId("history-attachment-filenames"),
    ).toHaveTextContent("diagram.png");
  });
  ...
  test("sends an attachment-only project message without echoing fallback text", async () => {
      ...
      return (
        <div>
          ...
          <button
            type="button"
            onClick={() =>
              workbench.projectChat.addExistingAttachment(attachment)
            }
          >
            add attachment
          </button>
    ...
    await waitFor(() =>
      expect(screen.getByTestId("attachment-probe-ready")).toHaveTextContent(
        "ready",
      ),
    );
    await userEvent.click(screen.getByText("add attachment"));
    ...
    expect(
      screen.getByTestId("attachment-message-contents"),
    ).not.toHaveTextContent("请参考附件");
    expect(screen.getByTestId("attachment-message-contents")).toHaveTextContent(
      "user:done:",
    );
    expect(
      screen.getByTestId("attachment-current-task-title"),
    ).toHaveTextContent("新对话");
  });

  test("clears the open task and messages after archiving all chats", async () => {
    const archiveAllChats = vi
      .fn()
      .mockResolvedValue({ message: "ok", count: 1 });
      ...
      <WorkbenchProvider
        ...
        services={{
          teamApi: {
            getDefaultWorkbenchTeam: vi
              .fn()
              .mockResolvedValue({ id: 2, name: "coder" }),
          },
          modelApi: {
            listModels: vi.fn().mockResolvedValue({ data: [] }),
          },
