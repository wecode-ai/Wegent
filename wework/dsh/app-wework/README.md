...
`wework.conversation.summary` 是环境面板下半区的候选贡献列表。工作区目录和执行器
由宿主固定渲染；多个独立插件可以声明带通用 `when` 条件的下半区视图，宿主只渲染
第一个匹配项。扩展点本身只提供通用上下文、命令执行、资源打开和按 ID 获取宿主
服务的能力；Git、输出内容等业务数据由宿主服务提供，不属于扩展点协议。贡献应通过
