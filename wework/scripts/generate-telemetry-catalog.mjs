...
const registryNames = ['smartApp', 'plugin']
...
export function validateRegistry(registry) {
  ...
  for (const event of registry.events ?? []) {
    validateEntry(event, 'event', keys)
    assertSnakeCase(event.eventName, 'event name')
    assertUnique(eventNames, event.eventName, 'duplicate event name')
  }
  ...
  return {
    ...
    events: [
      ...(registry.events ?? []).map(event => ({
        ...
        properties: event.publicProperties,
        title: event.name,
      })),
...
async function renderTypeScript(catalog, registryName) {
  const generatedTypesPath = path.join(
    weworkDirectory,
    `src/telemetry/generated/${registryName}Events.ts`
  )
  const typePrefix = registryName[0].toUpperCase() + registryName.slice(1)
  const constantPrefix = catalog.domain.toUpperCase()
  ...
  return formatGenerated(
    `// Generated by scripts/generate-telemetry-catalog.mjs. Do not edit manually.\n\nexport interface ${typePrefix}GeneratedEventMap {\n${eventMap}\n}\n\nexport type ${typePrefix}GeneratedEventName = keyof ${typePrefix}GeneratedEventMap\n\nexport const ${constantPrefix}_EVENT_PROPERTY_KEYS = ${renderTypeScriptValue(propertyKeys)} as const\n\nexport const ${constantPrefix}_EVENT_VALUE_CONSTRAINTS = ${renderTypeScriptValue(constraints)} as const\n\nexport const ${constantPrefix}_ROUTE_DEFINITIONS = ${renderTypeScriptValue(catalog.routeDefinitions)} as const\n\nexport const ${constantPrefix}_OPERATION_DEFINITIONS = ${renderTypeScriptValue(catalog.operationDefinitions)} as const\n\nexport type ${typePrefix}RouteDefinition = (typeof ${constantPrefix}_ROUTE_DEFINITIONS)[number]\nexport type ${typePrefix}OperationDefinition = (typeof ${constantPrefix}_OPERATION_DEFINITIONS)[number]\nexport type ${typePrefix}OperationKey = ${typePrefix}OperationDefinition['key']\n`,
    generatedTypesPath,
    'typescript'
  )
}
...
async function main() {
  ...
  const catalogs = await Promise.all(
    registryNames.map(async name => {
      const registry = JSON.parse(
        await readFile(
          path.join(weworkDirectory, `src/telemetry/registry/${name}Registry.json`),
          'utf8'
        )
      )
  ...
  const catalog = {
    ...
    domain: 'wework',
    owner: 'wework',
    events: catalogs.flatMap(value => value.events),
  }
  const drifted = await Promise.all([
    ...catalogs.map(async (value, index) =>
      writeGeneratedFile(
        path.join(weworkDirectory, `src/telemetry/generated/${registryNames[index]}Events.ts`),
        await renderTypeScript(value, registryNames[index]),
        check
      )
    ),
    writeGeneratedFile(publicCatalogJsonPath, await renderJson(catalog), check),
    writeGeneratedFile(publicCatalogMarkdownPath, await renderMarkdown(catalog), check),
  ])
