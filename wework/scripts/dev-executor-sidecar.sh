...
is_stdio_server() {
  case "${1:-}" in
    browser-mcp-server|space-mcp-server)
      return 0
      ;;
    *)
      return 1
...
if is_stdio_server "${1:-}"; then
  unset CARGO_TARGET_DIR
  unset WEGENT_CARGO_TARGET_DIR_AUTO
  if [ -z "${WEGENT_CARGO_TARGET_ROOT:-}" ] && [ -n "${HOME:-}" ]; then
    export WEGENT_CARGO_TARGET_ROOT="$HOME/.cache/wegent/cargo-target"
  fi
fi
...
if is_stdio_server "${1:-}" && [ -z "${WEGENT_EXECUTOR_BINARY:-}" ]; then
  EXECUTOR_BINARY="$(cargo_target_binary_path "$EXECUTOR_DIR" debug wegent-executor)"
  if [ ! -x "$EXECUTOR_BINARY" ]; then
    cargo build --manifest-path "$EXECUTOR_DIR/Cargo.toml" --bin wegent-executor
  fi
  exec "$EXECUTOR_BINARY" "$@"
fi
