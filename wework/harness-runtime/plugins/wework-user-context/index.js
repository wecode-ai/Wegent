...
const name = 'wework-user-context'
...
function contextUrl(config) {
  const baseUrl = typeof config?.baseUrl === 'string' ? config.baseUrl.trim() : ''
  if (!baseUrl) throw new Error('Wework user context is unavailable')
...
async function fetchContext(ctx, config, signal) {
  ...
  if (result.statusCode < 200 || result.statusCode >= 300) {
    throw new Error('Wework user context is unavailable')
  }
  if (result.body.kind !== 'text' && result.body.kind !== 'html') {
    throw new Error('Wework user context response is unsupported')
  }
...
function apply(ctx, config) {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description: 'Get the current Wework user for this Smart app session.',
      parameters: {},
      output: {
        schema: {
          type: 'object',
