...
creates one flat `node_modules` tree without dependencies on the build
machine's global pnpm store. It creates and materializes archives through the
Node `tar` library instead of a system `tar` command. Before archiving, it
rejects broken symlinks and links that resolve outside the runtime root.
Changes to the dependency layout or archive semantics must also bump
`archiveFormatVersion` so published assets from the old format cannot be
reused.
