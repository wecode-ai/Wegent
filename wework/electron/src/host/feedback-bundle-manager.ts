...
export class FeedbackBundleManager {
  ...
  async submit(request: FeedbackSubmitRequest): Promise<FeedbackSubmitResult> {
    const endpoint = new URL(request.apiUrl)
    if (!['http:', 'https:'].includes(endpoint.protocol)) {
      throw new Error('Feedback API URL must use HTTP or HTTPS')
    }
    if (!request.accessToken.trim()) {
      throw new Error('Feedback authentication is unavailable')
    }
    const { stagingId, staged } = await this.resolveStaged(request.stagingId)
    const bundle = await readFile(staged.path)
    const form = new FormData()
    form.set('report_id', staged.reportId)
    form.set('title', request.title)
    form.set('description', request.description)
    form.set('context', JSON.stringify(request.context))
    form.set(
      'bundle',
    ...
    const result = (await response.json()) as FeedbackSubmitResult
