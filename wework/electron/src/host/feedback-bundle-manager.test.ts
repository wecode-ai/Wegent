...
describe('FeedbackBundleManager', () => {
  ...
  test('submits the staged archive and removes it after success', async () => {
    const root = await temporaryDirectory('wework-feedback-submit-')
    const manager = createManager(root, join(root, 'missing-logs'))
    const preview = await manager.preview({
      includeRuntimeLogs: false,
      includeTaskInfo: false,
    ...
    ).resolves.toMatchObject({ item_id: 'FEEDBACK-1' })

    expect(fetchMock).toHaveBeenCalledWith(
      new URL('https://feedback.example.com/v1/reports'),
