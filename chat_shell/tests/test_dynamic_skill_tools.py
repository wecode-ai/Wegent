...
class _FakeGeminiSearchTool(BaseTool):
    name: str = "gemini_search"
    description: str = "Fake Gemini search tool"
...
def _tool_names(tools) -> list[str]:
    """Return tool names from a list or LangGraph ToolNode."""
    if hasattr(tools, "tools_by_name"):
        return list(tools.tools_by_name.keys())
...
class TestDynamicToolSelectionIntegration:
    ...
    def test_gemini_build_agent_preserves_dynamic_skill_tools(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "GeminiSearchTool",
            _FakeGeminiSearchTool,
        )
        monkeypatch.setattr(
            graph_builder_module,
            "create_gemini_search_tool",
            lambda llm: _FakeGeminiSearchTool(),
        )
