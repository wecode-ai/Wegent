...
async def prepare_knowledge_base_tools(
    ...
    # Consumer users only get knowledge_base_search (RAG), NOT kb_ls/kb_head
    if not is_consumer:
        ...
        logger.info(
            "[knowledge_factory] Consumer mode: Created 1 KB tool: knowledge_base_search "
    ...
    # Consumer users always get the Consumer prompt (content protection mode)
    if is_consumer:
        kb_instruction = KB_PROMPT_CONSUMER
        logger.info(
            "[knowledge_factory] Using CONSUMER mode prompt "
            "(user has Consumer role, content protection enabled)"
        )
    else:
