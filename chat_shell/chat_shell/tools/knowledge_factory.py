...
async def prepare_knowledge_base_tools(
    ...
    is_restricted_observer: Optional[bool] = None,
    ...
    - When is_restricted_observer=True, kb_ls and kb_head tools are NOT created (RestrictedObserver role
      can only use RAG search, not browse documents).

    Args:
        is_restricted_observer: Whether the user has RestrictedObserver role. Callers MUST explicitly
    ...
    # Callers must explicitly declare restricted observer status when KBs are present
    if is_restricted_observer is None:
        raise ValueError(
            "is_restricted_observer must be explicitly set to True or False "
    ...
    # RestrictedObserver users only get knowledge_base_search (RAG), NOT kb_ls/kb_head
    if not is_restricted_observer:
        ...
        logger.info(
            "[knowledge_factory] RestrictedObserver mode: Created 1 KB tool: knowledge_base_search "
            "(kb_ls and kb_head disabled)"
        )
    ...
    # RestrictedObserver users always get the RestrictedObserver prompt (content protection mode)
    if is_restricted_observer:
        kb_instruction = KB_PROMPT_RESTRICTED_OBSERVER
        logger.info(
            "[knowledge_factory] Using RESTRICTED_OBSERVER mode prompt "
            "(user has RestrictedObserver role, content protection enabled)"
        )
    else:
