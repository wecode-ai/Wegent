...
if [ "${NORMALIZED_APP_BASE_PATH}" = "/" ]; then
  NORMALIZED_APP_BASE_PATH=""
fi
API_BASE_URL="${PUBLIC_API_BASE_URL:-${API_BASE_URL:-${NORMALIZED_APP_BASE_PATH}/api}}"
SOCKET_BASE_URL="${SOCKET_BASE_URL:-wss://wss-wegent.intra.weibo.com}"
SOCKET_PATH="${SOCKET_PATH:-/socket.io}"
LOGIN_MODE="${LOGIN_MODE:-oidc}"
