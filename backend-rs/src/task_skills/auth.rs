...
const TASK_SKILLS_MISSING: &str = "Wegent-Task-Skills-Missing";
const TASK_SKILLS_INVALID_API_KEY: &str = "Wegent-Task-Skills-Invalid-Api-Key";
        ...
        get_current_user(&self.state().auth, &self.state().mysql, &headers.view())
            ...
            .map_err(|error| {
                if error.status() == brz_http_server::StatusCode::INTERNAL_SERVER_ERROR {
                    ...
                    brz_http_server::AuthFailure::missing_credentials(TASK_SKILLS_MISSING)
                } else if error.detail() == "Invalid or expired API key" {
                    brz_http_server::AuthFailure::invalid_credentials(TASK_SKILLS_INVALID_API_KEY)
                } else {
    ...
    fn reject(
        ...
        match failure {
            ...
            brz_http_server::AuthFailure::InvalidCredentials {
                challenge: TASK_SKILLS_INVALID_API_KEY,
