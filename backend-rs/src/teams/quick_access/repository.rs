...
//! MySQL data access for `GET /api/users/quick-access`, mirroring the source
...
/// `QUICK_ACCESS_CONFIG_KEY` (`app.api.endpoints.users`); used by tests and
/// documentation to pin the literal in the SQL above.
#[cfg_attr(not(test), allow(dead_code))]
pub const QUICK_ACCESS_CONFIG_KEY: &str = "quick_access_recommended";

/// One `system_configs` row
/// (`db.query(SystemConfig).filter(SystemConfig.config_key == ...)`).
///
/// `config_value` is a VARCHAR column holding a JSON document; it decodes as
/// the source `config_value` property (`json.loads` with an empty-dict
/// fallback), so it reads as JSON while remaining string-typed in the
/// projection. Fields this endpoint does not read carry
/// `#[allow(dead_code)]`.
...
pub struct TeamKindRow {
    pub kinds_id: i64,
    /// Selected to match the recorded projection; not consumed here (the
    /// quick-access `is_system` flag comes from the caller's set logic).
    #[allow(dead_code)]
...
/// Load the quick-access `system_configs` row
/// (`WHERE system_configs.config_key = 'quick_access_recommended' LIMIT 1`).
pub async fn quick_access_system_config<M>(mysql: &M) -> MysqlResult<Option<SystemConfigRow>>
where
    M: Mysql,
{
    mysql
        .fetch_optional(
            "SELECT system_configs.id AS system_configs_id, \
             ...
             FROM system_configs \
             WHERE system_configs.config_key = 'quick_access_recommended' \
             LIMIT 1",
            (),
        )
        .await
...
mod tests {
    ...
    fn config_key_is_source_constant() {
        assert_eq!(QUICK_ACCESS_CONFIG_KEY, "quick_access_recommended");
    }
}
