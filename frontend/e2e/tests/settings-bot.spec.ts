...
test.describe('Settings - Bot Management', () => {
  ...
  test('should access bot management via manage bots button', async ({ page }) => {
    ...
    await expect(
      page.locator('[role="dialog"] button:has-text("New Bot"), [role="dialog"] button:has-text("新建Bot")')
    ).toBeVisible({ timeout: 5000 })
  })
  ...
  test('should display bot list or empty state in dialog', async ({ page }) => {
    ...
    const hasBots = await page.locator('[role="dialog"] .bg-base').first().isVisible({ timeout: 3000 }).catch(() => false)
    const hasEmptyState = await page.locator('[role="dialog"] text=No bots').isVisible({ timeout: 1000 }).catch(() => false)
  ...
  test('should create new bot', async ({ page, testPrefix }) => {
    ...
    // Submit form
    const submitButton = page.locator('[role="dialog"] button:has-text("Save"), [role="dialog"] button:has-text("保存")').first()
  ...
  test('should show edit and delete buttons for existing bots', async ({ page }) => {
    ...
    if (await botCard.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const editButton = page.locator('[role="dialog"] button[title*="Edit"], [role="dialog"] button:has-text("Edit")').first()
