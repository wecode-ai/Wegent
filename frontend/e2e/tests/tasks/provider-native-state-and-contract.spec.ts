...
test.describe('Provider-native binding state and contracts', () => {
  ...
  test('E2E-A2-025 keeps the legacy scoped knowledge path for a non-A2 Shell', async ({
    page,
    request,
  }) => {
    const agnoBotName = `${TEST_PREFIX}-agno-bot`
    const agnoTeamName = `${TEST_PREFIX}-agno-team`
    const botResponse = await request.post(`${PROVIDER_NATIVE_API_URL}/api/bots`, {
      headers: authHeaders(resources.token),
      data: {
        name: agnoBotName,
        shell_name: 'Agno',
        agent_config: { bind_model: resources.modelName, bind_model_type: 'user' },
        system_prompt: 'Use the mounted knowledge scope.',
        namespace: 'default',
        is_active: true,
      },
    })
    ...
    const teamResponse = await request.post(`${PROVIDER_NATIVE_API_URL}/api/teams`, {
      ...
      data: {
        name: agnoTeamName,
        description: 'Non-A2 shell compatibility E2E team',
    ...
    const agnoTeamId = ((await teamResponse.json()) as { id?: number }).id
    expect(agnoTeamId).toBeTruthy()

    try {
      const prompt = makePrompt('025', '输出新版方案的唯一断言标记。')
      await scenario(request, prompt, [
        {
          toolCalls: [
            {
              toolName: 'kb_head',
              arguments: { document_ids: [resources.fixture.documents.a1.id] },
            },
          ],
        },
        { responseContent: PROVIDER_NATIVE_MARKERS.a1 },
      ...
      expect(text).not.toContain('<selected_knowledge_sources>')
      expect(text).toContain('kb_head')
      const calls = collectTaskToolCalls(task).filter(call => call.name.endsWith('kb_head'))
      expect(calls).toHaveLength(1)
      expect(calls[0].input.document_ids).toEqual([resources.fixture.documents.a1.id])
      expect(JSON.stringify(calls[0].output)).toContain(PROVIDER_NATIVE_MARKERS.a1)
      expect(extractTaskAnswer(task)).toBe(PROVIDER_NATIVE_MARKERS.a1)
    } finally {
      await request
        .delete(`${PROVIDER_NATIVE_API_URL}/api/v1/namespaces/default/teams/${agnoTeamName}`, {
          headers: authHeaders(resources.token),
        })
        .catch(() => null)
      await request
        .delete(`${PROVIDER_NATIVE_API_URL}/api/v1/namespaces/default/bots/${agnoBotName}`, {
          headers: authHeaders(resources.token),
        })
        .catch(() => null)
    }
