 ...
 *
 * Usage:
 *   node e2e/mock-server/model-mock-server.mjs
 *
 ...
 */
function analyzeRequestContent(body) {
 ...
 */
function generateOpenAIStream(content) {
  const chunks = []
 ...
 */
function generateAnthropicStream(content) {
  const chunks = []
 ...
 */
async function streamResponse(res, chunks, delayMs = 50) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === '/health' && req.method === 'GET') {
    ...
    res.end(JSON.stringify({ status: 'ok', server: 'mock-model-server', requestCount: recordedRequests.length }))
    return
  }
  ...
  if (url === '/v1/chat/completions' && req.method === 'POST') {
    try {
      ...
      console.log(`[Mock Server] Analysis: hasImage=${analysis.hasImageContent}, hasText=${analysis.hasTextContent}, textLen=${analysis.extractedTextLength}, imageFormat=${analysis.imageFormat}`)
