...
export class LoginPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.errorMessage = page.locator('.error, [data-error], [role="alert"]:not([data-sonner-toaster])')
  }
  ...
  async login(username: string, password: string): Promise<void> {
    await this.navigate()
    await this.waitForSelector('input[name="user_name"], input[name="username"], input[type="text"]')
    ...
    await this.waitForUrl((url) => !url.includes('/login'), 30000)
  }
