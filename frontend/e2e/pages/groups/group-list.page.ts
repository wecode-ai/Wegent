...
export class GroupsPage extends BasePage {
  ...
  constructor(page: Page) {
    super(page)
    this.createButton = page.locator('button:has-text("Create Group"), button:has-text("New Group")')
  ...
  async fillGroupForm(groupData: {
    ...
    if (groupData.visibility) {
      const visibilitySelector = this.page.locator('[data-testid="visibility-select"], [role="combobox"]:has-text("Visibility")')
  ...
  getGroupItem(name: string): Locator {
    return this.page.locator(`[data-testid="group-item"]:has-text("${name}"), .group-item:has-text("${name}")`)
  }
  ...
  async getAllGroupNames(): Promise<string[]> {
    const items = this.page.locator('[data-testid="group-item"] [data-testid="group-name"], .group-item .group-name')
