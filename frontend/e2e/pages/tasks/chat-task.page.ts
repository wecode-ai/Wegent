...
export class ChatTaskPage extends BasePage {
  ...
  constructor(page: Page) {
    super(page)
    this.chatInput = page.locator('[data-testid="chat-input"], textarea[placeholder*="message" i], textarea[placeholder*="type" i]')
    this.sendButton = page.locator('[data-testid="send-button"], button[type="submit"]:has-text("Send")')
    this.messageList = page.locator('[data-testid="message-list"], .message-list')
    this.teamSelector = page.locator('[data-testid="team-selector"], [role="combobox"]:has-text("Team")')
  ...
  async waitForStreamingComplete(timeout: number = 60000): Promise<void> {
    ...
    await this.page.waitForSelector('[data-streaming="true"]', { state: 'detached', timeout }).catch(() => {})
    // Or wait for send button to be enabled again
    await this.page.waitForSelector('[data-testid="send-button"]:not([disabled])', { timeout }).catch(() => {})
  }
