...
export class BotsPage extends SettingsBasePage {
  ...
  constructor(page: Page) {
    super(page)
    this.createButton = page.locator('button:has-text("Create Bot"), button:has-text("New Bot")')
  ...
  async fillBotForm(botData: {
    ...
    if (botData.ghost) {
      const ghostSelector = this.page.locator('[data-testid="ghost-select"], [role="combobox"]:has-text("Ghost")')
    ...
    if (botData.shell) {
      const shellSelector = this.page.locator('[data-testid="shell-select"], [role="combobox"]:has-text("Shell")')
    ...
    if (botData.model) {
      const modelSelector = this.page.locator('[data-testid="model-select"], [role="combobox"]:has-text("Model")')
  ...
  async getAllBotNames(): Promise<string[]> {
    ...
    for (let i = 0; i < count; i++) {
      const nameElement = cards.nth(i).locator('[data-testid="bot-name"], .bot-name, h3, h4').first()
