...
export class TeamsPage extends SettingsBasePage {
  ...
  async fillTeamForm(teamData: {
    ...
    if (teamData.collaborationModel) {
      const modeSelector = this.page.locator('[data-testid="mode-select"], [role="combobox"]:has-text("Mode")')
  ...
  async addMember(botName: string, role?: string): Promise<void> {
    ...
    // Select bot
    const botSelector = this.page.locator('[data-testid="bot-select"], [role="combobox"]:has-text("Bot")')
