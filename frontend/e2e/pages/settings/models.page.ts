...
export class ModelsPage extends SettingsBasePage {
  ...
  constructor(page: Page) {
    super(page)
    this.createButton = page.locator('button:has-text("Create Model"), button:has-text("New Model"), button:has-text("Add Model")')
  ...
  async fillModelForm(modelData: {
    ...
    if (modelData.provider) {
      const providerSelector = this.page.locator('[data-testid="provider-select"], [role="combobox"]:has-text("Provider")')
    ...
    if (modelData.modelId) {
      await this.page.fill('input[name="model_id"], input[placeholder*="model" i]', modelData.modelId)
    }
