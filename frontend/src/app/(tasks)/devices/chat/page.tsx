...
export default function DeviceChatPage() {
  ...
  // Filter teams to only show executor-based teams (not Chat Shell or Dify)
  // These are teams that can use local device execution
  // Include teams where agent_type is undefined/null (assumes executor-based)
  // or agent_type is explicitly an executor type (claude, agno, etc.)
  const executorTeams = teams.filter(team => {
    const agentType = team.agent_type?.toLowerCase()
    // Exclude Chat Shell and Dify - they cannot run on local devices
    return agentType !== 'chat' && agentType !== 'dify'
  })
  ...
  return (
    <div className="flex smart-h-screen bg-base text-text-primary box-border">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {selectedDeviceId && selectedDevice?.status !== 'offline' ? (
          <ChatArea
            teams={executorTeams}
