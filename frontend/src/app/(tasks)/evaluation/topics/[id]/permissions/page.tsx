...
function PermissionsContent() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const [topicData, roleData, permissionsData] = await Promise.all([
        getTopic(topicId),
        getMyRole(topicId),
        listPermissions(topicId, { limit: 100 }),
      ])
  ...
  const handleRevokePermission = async (userId: number) => {
    try {
      await revokePermission(topicId, userId)
      toast({
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="userId">{t('permissions.user')} ID *</Label>
                <Input
                  id="userId"
                  type="number"
                  value={newUserId}
