...
import { respondentListTopics } from '@wecode/api/evaluation'
...
function RespondentTopicsContent() {
  ...
  const loadTopics = useCallback(async () => {
    setLoading(true)
    try {
      const response = await respondentListTopics({
        page,
        limit: 20,
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-text-primary">{t('topics.browse')}</h1>
      ...
      {loading ? (
        ...
        <div className="py-12 text-center">
          ...
          <p className="mb-4 text-sm text-text-secondary">
            {search ? t('errors.not_found') : t('topics.browse_description')}
          </p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {topics.map(topic => (
            <Card
              ...
              onClick={() => router.push(`/evaluation/respondent/topics/${topic.id}`)}
            >
...
export default function RespondentTopicsPage() {
  return (
    <EvaluationPageLayout>
      <RespondentTopicsContent />
    </EvaluationPageLayout>
  )
}
