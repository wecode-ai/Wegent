...
describe('QuickLauncherCards', () => {
  ...
  test('collapses mobile launchers into one compact quick-start row', () => {
    ...
    expect(
      screen.queryByTestId('mobile-quick-launcher-system_function-system-create_skill')
    ).not.toBeInTheDocument()
    expect(screen.getByTestId('quick-launch-more-card')).toBeInTheDocument()
    expect(screen.getByTestId('quick-launch-create-card')).toBeInTheDocument()
  ...
  test('keeps quick create accessible on mobile without launchers', () => {
    mockIsMobile = true
