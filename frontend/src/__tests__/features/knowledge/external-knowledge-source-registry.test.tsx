...
describe('external knowledge source registry — ContextSelector (conversation)', () => {
    ...
    expect(onReplaceContexts).toHaveBeenCalledWith(
      ...
      [
        expect.objectContaining({
          ...
          id: 'external:fake-provider:explicit:organization:lib-1:knowledge_base:source:provider-default',
          name: 'Fake Lib',
          ref: {
            provider: FAKE_PROVIDER,
            mode: 'explicit',
  ...
  it('keeps whole-KB and document refs mutually exclusive for external sources', async () => {
    ...
    await waitFor(() =>
      expect(onChange).toHaveBeenLastCalledWith([
        expect.objectContaining({
          id: 'external:fake-provider:explicit:organization:lib-1:document:document:doc-2:provider-default',
