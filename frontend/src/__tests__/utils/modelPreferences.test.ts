...
describe('modelPreferences', () => {
  ...
  describe('category and purpose as independent axes', () => {
    // Two suffixes landed on this key from two directions: upstream added the model
    // category (llm / image / video...), and the knowledge base added what the choice
    // is for (a summary, or generating a wiki). They are not the same question -- a
    // wiki model is an llm -- so both have to fit, and neither may read the other's.
    const teamId = 300

    it('keeps a purpose-scoped choice out of the conversation slot', () => {
      saveGlobalModelPreference(teamId, createPreference('wiki-model'), undefined, 'wiki')

      expect(getGlobalModelPreference(teamId, undefined, 'wiki')!.modelName).toBe('wiki-model')
      // The chat reads the unscoped key and must not see it.
      expect(getGlobalModelPreference(teamId)).toBeNull()
    })

    it('keeps the two purposes apart', () => {
      saveGlobalModelPreference(teamId, createPreference('for-summaries'), undefined, 'summary')
      saveGlobalModelPreference(teamId, createPreference('for-wikis'), undefined, 'wiki')

      expect(getGlobalModelPreference(teamId, undefined, 'summary')!.modelName).toBe(
        'for-summaries'
      )
      expect(getGlobalModelPreference(teamId, undefined, 'wiki')!.modelName).toBe('for-wikis')
    })

    it('keeps a category apart from a purpose', () => {
      saveGlobalModelPreference(teamId, createPreference('an-image-model'), 'image')
      saveGlobalModelPreference(teamId, createPreference('a-wiki-model'), undefined, 'wiki')

      expect(getGlobalModelPreference(teamId, 'image')!.modelName).toBe('an-image-model')
      expect(getGlobalModelPreference(teamId, undefined, 'wiki')!.modelName).toBe('a-wiki-model')
    })

    it('treats llm as no category at all, so a purpose still reads back', () => {
      // llm contributes no suffix -- that is what lets keys written before categories
      // existed still resolve. A purpose passed alongside it must survive that.
      saveGlobalModelPreference(teamId, createPreference('llm-wiki'), 'llm', 'wiki')

      expect(getGlobalModelPreference(teamId, 'llm', 'wiki')!.modelName).toBe('llm-wiki')
      expect(getGlobalModelPreference(teamId, undefined, 'wiki')!.modelName).toBe('llm-wiki')
    })

    it('composes both when a choice has a category and a purpose', () => {
      saveGlobalModelPreference(teamId, createPreference('both'), 'image', 'wiki')

      expect(getGlobalModelPreference(teamId, 'image', 'wiki')!.modelName).toBe('both')
      // Neither axis alone addresses it.
      expect(getGlobalModelPreference(teamId, 'image')).toBeNull()
      expect(getGlobalModelPreference(teamId, undefined, 'wiki')).toBeNull()
    })

    it('clears only the key it was asked to clear', () => {
      saveGlobalModelPreference(teamId, createPreference('for-summaries'), undefined, 'summary')
      saveGlobalModelPreference(teamId, createPreference('for-wikis'), undefined, 'wiki')

      clearGlobalModelPreference(teamId, undefined, 'wiki')
    ...
    it('survives the cleanup sweep, which must not read a purpose as a task id', () => {
      // The sweep tells a session key from a global one by looking for an underscore
      // after the prefix, and expires session keys older than 30 days while leaving
      // global ones alone. So the age is the whole test: a suffix joined with "_"
      // instead of ":" makes this look like a stale task preference and it is swept.
      // A freshly-written one proves nothing -- it is too young to be expired either
      // way.
      const longAgo = Date.now() - 31 * 24 * 60 * 60 * 1000
      saveGlobalModelPreference(
        teamId,
        createPreference('for-wikis', undefined, false, longAgo),
        undefined,
        'wiki'
      )
