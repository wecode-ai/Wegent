...
export const teamHandlers = [
  ...
  http.post<never, any>('/api/teams', async ({ request }) => {
  ...
  http.put<{ id: string }, any>('/api/teams/:id', async ({ params, request }) => {
    const { id } = params
    const teamData = await request.json()
    const index = MOCK_TEAMS.findIndex(t => t.id === Number(id))
    if (index !== -1) {
      MOCK_TEAMS[index] = { ...MOCK_TEAMS[index], ...teamData, updated_at: new Date().toISOString() }
