...
interface KnowledgeBaseSelectorProps {
  ...
  onDeselect: (knowledge_id: string) => void;
...
export default function KnowledgeBaseSelector({
  ...
  const isSelected = (knowledge_id: string) => {
    return selectedKnowledgeBases.some(kb => kb.knowledge_id === knowledge_id);
  };

  // Handle knowledge base click
  const handleClick = (kb: KnowledgeBase) => {
    if (isSelected(kb.knowledge_id)) {
      onDeselect(kb.knowledge_id);
    } else {
      onSelect(kb);
    }
  };
