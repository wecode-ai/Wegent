...
export function QuickLauncherCards({
  ...
  if (isMobile) {
    ...
    const quickCreateCard = renderQuickCreateCard?.()
    const actionCount = Number(Boolean(moreButton)) + Number(Boolean(quickCreateCard))
    const visibleLauncherCount = Math.max(0, 3 - actionCount)
    const mobileLaunchers = [...systemLaunchers, ...favoriteLaunchers].slice(
      0,
      visibleLauncherCount
    )

    if (mobileLaunchers.length === 0 && !moreButton && !quickCreateCard) {
      return null
    }
