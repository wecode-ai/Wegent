...
import type { ToolRendererProps, ToolRenderResult, GrepToolInput } from '../types'
import { ToolHeader, SkeletonValue, TruncatedIndicator } from '../components'
import { truncateOutput, formatMatchCount } from '../utils'

/**
 * Renderer for Grep tool
 */
export function GrepToolRenderer(props: ToolRendererProps<GrepToolInput>): ToolRenderResult {
  ...
  const stats =
    matchCount !== undefined
      ? `${formatMatchCount(matchCount)} ${t('thinking.units.matches') || 'matches'}`
      : undefined
      ...
      <div className="text-sm space-y-2">
        ...
        {pattern && (
          <div>
            <div className="mb-1 font-medium text-text-secondary text-xs">
              {t('thinking.sections.searchPattern') || 'Pattern'}
            </div>
        ...
        {truncatedOutput ? (
          ...
          <span className="text-text-tertiary text-xs">
            {t('thinking.no_matches') || 'No matches found'}
          </span>
        )}
      </div>
    ),
