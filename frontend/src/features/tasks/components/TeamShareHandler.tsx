 ...
 */
export default function TeamShareHandler({ teams, onTeamSelected, onRefreshTeams }: TeamShareHandlerProps) {
  ...
  const handleConfirmJoin = async () => {
    ...
    try {
      ...
      console.error('Failed to join shared team:', error)
      const errorMessage = error?.message || t('teams.share.join_failed')
  ...
  const renderMessageWithHighlight = (messageKey: string, teamName: string, userName?: string) => {
    ...
    const messageRenderers = {
      ...
      'teams.share.confirm_message': () => userName ? (
        <span>
    ...
    >
      <div className="space-y-4">
        ...
        {isSelfShare ? (
          <>
            <Alert
              message={renderMessageWithHighlight('teams.share.self_share_message', shareInfo.team_name)}
              type="warning"
              showIcon
            />
          </>
          ...
          <>
            <Alert
              message={renderMessageWithHighlight('teams.share.already_joined_message', shareInfo.team_name)}
              type="warning"
              showIcon
            />
          </>
          ...
          <>
            <div className="text-center">
              <p className="text-text-primary text-base">
                {renderMessageWithHighlight('teams.share.confirm_message', shareInfo.team_name, shareInfo.user_name)}
              </p>
            </div>
            
            <Alert
              message={renderMessageWithHighlight('teams.share.join_description', shareInfo.team_name)}
              type="info"
              showIcon
            />
          </>
