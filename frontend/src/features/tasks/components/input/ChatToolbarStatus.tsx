...
const ACTIVE_TASK_STATUS_PATHS = ['/chat', '/devices/chat']
...
export default function ChatToolbarStatus({ className, compact = false }: ChatToolbarStatusProps) {
  ...
  const contentElement = (
    <div className="space-y-3 text-xs" data-testid="chat-toolbar-status-popover">
      ...
      {showQuotaSection && !quotaDetail && legacyQuotaBrief && (
        <QuotaSection
          title={t('common:quota.title')}
          brief={legacyQuotaBrief}
          details={
            <>
              <div className="text-text-muted">
              ...
              <div className="text-text-muted">
                {t('common:quota.detail_rate', {
                  rate: (quota.usage_rate * 100).toFixed(2),
                })}
              </div>
              <div className="text-text-muted">
                {t('common:quota.billing_prefix')}
                <a
                  href="https://space.intra.weibo.com/develop/model-quota"
                  target="_blank"
