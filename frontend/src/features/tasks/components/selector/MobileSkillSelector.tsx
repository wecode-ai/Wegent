...
import { countEnabledSkills, getAutomaticSkillNames } from './skill-selector-utils'
...
export default function MobileSkillSelector({
  ...
  const automaticSkillNames = useMemo(
    () => getAutomaticSkillNames({ skills, teamSkillNames, preloadedSkillNames }),
      ...
      <DrawerContent className="max-h-[85vh] bg-[#f2f2f7] dark:bg-[#1c1c1e]" showHandle={false}>
        ...
        >
          <Button type="button" variant="ghost" className="h-11 w-full justify-start" asChild>
            <Link
              href={`${paths.resourceLibrary.getHref()}?tab=mine&type=skill&scope=personal`}
              data-testid="mobile-skill-manage-default-link"
            >
