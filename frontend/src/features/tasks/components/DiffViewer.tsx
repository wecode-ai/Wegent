...
import { BranchDiffResponse, GitHubDiffFile, GitLabDiffFile } from '@/apis/tasks';
...
function normalizeGitLabFiles(diffs: GitLabDiffFile[]): DiffFile[] {
    ...
    status: diff.new_file
      ...
      : diff.deleted_file
        ...
        : diff.renamed_file
          ? 'renamed'
          : 'modified',
    additions: 0, // GitLab doesn't provide these counts in the diff
    deletions: 0,
    changes: 0,
    diff: diff.diff,
...
function normalizeGitHubFiles(files: GitHubDiffFile[]): DiffFile[] {
...
export default function DiffViewer({
  ...
  useEffect(() => {
    if (fileChanges && fileChanges.length > 0) {
      ...
      if (gitType === 'gitlab' && diffData.diffs) {
        setDiffFiles(normalizeGitLabFiles(diffData.diffs));
      } else if (gitType === 'github' && diffData.files) {
        setDiffFiles(normalizeGitHubFiles(diffData.files));
      }
    }
