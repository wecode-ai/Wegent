  ...
  const handleAddMembers = async () => {
    ...
    if (unregisteredUsersList.length > 0 && registeredUsers.length === 0) {
      ...
      try {
        ...
        toast({
          title: t('addMembers.allUnregistered'),
          description: t('addMembers.useInviteLink'),
          variant: 'default',
        });
      } catch (error) {
    ...
    try {
      ...
      if (successCount > 0) {
        toast({
          title: t('addMembers.success', { count: successCount }),
        });
      }
      ...
      if (alreadyMemberCount > 0 && successCount === 0 && errors.length === 0) {
        ...
        toast({
          title: t('addMembers.allAlreadyMembers'),
          variant: 'default',
        });
      }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {showInviteLink
              ? t('addMembers.inviteLinkDescription', { count: addedCount })
              : t('addMembers.description', { taskTitle })}
          </DialogDescription>
        </DialogHeader>
            ...
            {selectedUsers.length > 0 && (
              <div className="space-y-2">
                <div className="text-sm font-medium">
                  {t('addMembers.selected', { count: selectedUsers.length })}
                </div>
          ...
          {!showInviteLink ? (
            <>
              ...
              <Button onClick={handleAddMembers} disabled={selectedUsers.length === 0 || isAdding}>
                ...
                {isAdding
                  ? t('addMembers.adding')
                  : t('addMembers.add', { count: selectedUsers.length })}
              </Button>
            </>
          ) : (
