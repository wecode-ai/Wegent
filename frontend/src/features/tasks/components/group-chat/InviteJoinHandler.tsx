...
export function InviteJoinHandler() {
  ...
  useEffect(() => {
    ...
    taskMemberApi
      ...
      .then(setInviteInfo)
      .catch(err => setError(err.message || t('invite.invalidLink')))
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (result.already_member) {
        toast({ title: t('invite.alreadyMember') });
      } else {
        toast({ title: t('invite.joinSuccess') });
      }
      ...
      toast({
        title: err instanceof Error ? err.message : t('invite.joinFailed'),
        variant: 'destructive',
      });
    } finally {
      setJoining(false);
