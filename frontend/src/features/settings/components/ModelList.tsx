...
const ModelList: React.FC<ModelListProps> = ({
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: t('errors.load_models_failed'),
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-xl font-semibold text-text-primary mb-1">{t('title')}</h2>
          <p className="text-sm text-text-muted mb-1">{t('description')}</p>
        </div>
        {/* Category Filter */}
        <div className="flex items-center gap-2">
          <span className="text-sm text-text-muted">{t('filter_by_category_type')}:</span>
          <Select
      ...
      <div className="bg-base border border-border rounded-md p-2 w-full max-h-[70vh] flex flex-col overflow-y-auto custom-scrollbar">
        ...
        {!loading && totalModels === 0 && (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-text-muted">{t('no_models')}</p>
            <p className="text-sm text-text-muted mt-1">{t('no_models_hint')}</p>
          </div>
        )}
          ...
          <>
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 p-1">
              ...
              {groupModels.length > 0 && (
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-text-secondary px-2">
                    {t('group_models')} ({groupModels.length})
                  </h3>
                  <div className="space-y-3">
              ...
              {publicModels.length > 0 && (
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-text-secondary px-2">
                    {t('public_models')} ({publicModels.length})
                  </h3>
                  <div className="space-y-3">
        ...
        {!loading && (scope === 'personal' || canCreateInCurrentGroup) && (
          <div className="border-t border-border pt-3 mt-3 bg-base">
            <div className="flex justify-center">
              <UnifiedAddButton onClick={handleCreate}>{t('create')}</UnifiedAddButton>
            </div>
          </div>
        )}
      </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('delete_confirm_message', { name: deleteConfirmModel?.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
