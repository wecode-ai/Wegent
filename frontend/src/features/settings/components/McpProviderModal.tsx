...
export function McpProviderBrowser({ onImportServer }: McpProviderBrowserProps) {
    ...
    try {
      ...
      if (response?.providers && Array.isArray(response.providers)) {
        ...
        setSelectedProvider(current => {
          if (!current) return response.providers[0] ?? null
          return response.providers.find(provider => provider.key === current.key) ?? current
        })
      }
    } catch (_error) {
      toast({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl h-[700px] p-0 overflow-hidden bg-background">
        ...
        <div className="h-[calc(700px-72px)]">
          <McpProviderBrowser onImportServer={onImportServer} />
        </div>
      </DialogContent>
    </Dialog>
  )
