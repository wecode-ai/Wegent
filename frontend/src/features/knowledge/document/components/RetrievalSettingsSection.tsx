...
export function RetrievalSettingsSection({
  ...
  const { retrievers, loading: loadingRetrievers } = useRetrievers(scope, groupName, publicOnly)
  ...
  const embeddingModelNamespace = config.embedding_config?.model_namespace
  ...
  const getModelKey = (name: string, namespace?: string) => `${namespace || 'default'}::${name}`
      ...
      const currentModelExists =
        ...
        embeddingModels.some(
          model =>
            ...
            (model.namespace || 'default') === (embeddingModelNamespace || 'default')
        )
  ...
  }, [loadingModels, embeddingModels, embeddingModelName, embeddingModelNamespace, onChange])
  ...
  const handleEmbeddingModelChange = (value: string) => {
    const model = embeddingModels.find(
      candidate => getModelKey(candidate.name, candidate.namespace) === value
    )
    if (model) {
      onUserChange?.()
      onChange({
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {loadingModels ? (
          ...
          <>
            <SearchableSelect
              value={getModelKey(embeddingModelName || '', embeddingModelNamespace)}
              onValueChange={handleEmbeddingModelChange}
              placeholder={t('document.retrieval.embeddingModelSelect')}
              searchPlaceholder={t('document.retrieval.searchPlaceholder')}
              disabled={isEmbeddingDisabled || noModelsAvailable}
              items={embeddingModels.map(model => ({
                value: getModelKey(model.name, model.namespace),
