...
function DocumentUploadSession({
  ...
  const enqueueFiles = useCallback(
    (files: File[]) => {
      if (!mounted.current || submissionLock.current || filesBusy || !canManageDocuments) return
  ...
  const handleConfirm = async (fileIds?: string[]) => {
    ...
    const submit = async (prompts: UploadMultimodalPrompts | null) => {
      if (!beginSubmit('file')) return
      try {
        const results = await onUploadComplete(attachments, splitterConfig, prompts ?? undefined)
        if (!mounted.current) return
    ...
    const includesVideo = attachments.some(({ file, attachment }) =>
