 ...
 */
function CopyButton({ code, copiedText, copyText }: { code: string; copiedText: string; copyText: string }) {
 ...
 */
function MermaidDiagram({ children, diagramText, clickToExpandText }: { children: ReactNode; diagramText: string; clickToExpandText: string }) {
 ...
 */
function CodeBlock({ language, code, copiedText, copyText }: { language: string; code: string; copiedText: string; copyText: string }) {
            ...
            <ReactMarkdown
              ...
              components={{
                ...
                tr: ({ node: _node, ...props }: MarkdownComponentProps) => (
                  <tr className="hover:bg-surface-hover/50 transition-colors duration-150" {...props} />
                ),
                ...
                h6: ({ node: _node, ...props }: MarkdownComponentProps) => (
                  <h6
                    className="text-sm font-medium mt-4 mb-2 text-text-secondary"
                    {...props}
                  />
                ),
                // Paragraph component
                ...
                code: ({ node: _node, className, children, ...props }: MarkdownComponentProps) => {
                  ...
                  if (className === 'language-mermaid') {
                    return <MermaidDiagram diagramText={t('wiki.diagram')} clickToExpandText={t('wiki.click_to_expand')}>{children}</MermaidDiagram>;
                  }
                  ...
                  if (match) {
                    ...
                    return <CodeBlock language={language} code={codeContent} copiedText={t('wiki.copied')} copyText={t('wiki.copy_code')} />;
                  }
