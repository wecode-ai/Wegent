...
export default function GreyTestButton() {
    ...
    try {
      if (isGreyUser) {
        ...
        if (response.success) {
          setIsGreyUser(response.is_grey_user)
          toast({
            title: t('common:grey.leaveSuccess'),
          })
        }
      } else {
        // Join grey test
        ...
        if (response.success) {
          setIsGreyUser(response.is_grey_user)
          toast({
            title: t('common:grey.joinSuccess'),
          })
        }
      }
    } catch (error) {
  ...
  const buttonText = isGreyUser ? t('common:grey.leaveButton') : t('common:grey.joinButton')
  const confirmTitle = isGreyUser
    ? t('common:grey.leaveConfirmTitle')
    : t('common:grey.joinConfirmTitle')
  const confirmMessage = isGreyUser
    ? t('common:grey.leaveConfirmMessage')
    : t('common:grey.joinConfirmMessage')
