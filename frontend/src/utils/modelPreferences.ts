...
const LEGACY_MODEL_ID_KEY = 'last_selected_model_id';
const LEGACY_MODEL_TYPE_KEY = 'last_selected_model_type';
...
export function saveGlobalModelPreference(teamId: number, preference: ModelPreference): void {
  ...
  try {
    ...
    localStorage.setItem(LEGACY_MODEL_ID_KEY, preference.modelName);
    if (preference.modelType) {
      localStorage.setItem(LEGACY_MODEL_TYPE_KEY, preference.modelType);
    }
  } catch (error) {
 ...
 * 3. Legacy storage (for backward compatibility)
 */
export function getModelPreference(teamId: number, taskId?: number | null): ModelPreference | null {
  ...
  try {
    ...
    // Priority 3: Legacy storage (backward compatibility)
    const legacyModelId = localStorage.getItem(LEGACY_MODEL_ID_KEY);
    const legacyModelType = localStorage.getItem(LEGACY_MODEL_TYPE_KEY);
    if (legacyModelId && legacyModelId !== 'undefined' && legacyModelId !== 'null') {
      return {
        modelName: legacyModelId,
...
export function cleanupModelPreferences(): void {
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (!key || !key.startsWith(GLOBAL_MODEL_PREF_PREFIX)) continue;
    ...
    // Remove expired session preferences
